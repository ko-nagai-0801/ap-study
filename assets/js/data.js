...
const GLOSSARY_DATA = [
  ...
  // ── Phase 17 追加（30語） ──────────────────────────
  // アルゴリズム
  { term: 'マージソート', category: 'algorithm', definition: '分割統治法に基づく安定ソートアルゴリズム。配列を半分に分割し再帰的にソートしてから結合する。時間計算量は常に O(n log n)。クイックソートと違い最悪ケースでも O(n log n) を保証するが、追加メモリ O(n) が必要。' },
  { term: 'ビット演算', category: 'algorithm', definition: '2 進数のビット単位で行う演算（AND・OR・XOR・NOT・シフト）。フラグ管理・マスク処理・高速除算（2 の冪乗）に活用。XOR を使うと追加メモリなしで 2 変数を交換できる。暗号・画像処理・組込みプログラミングで多用。' },
  { term: 'B 木（B-Tree）', category: 'algorithm', definition: '自己平衡型の多分木データ構造。各ノードが複数のキーを持ち、全葉ノードが同じ深さになるよう自動調整される。ディスクブロック読み込みに最適化されており、RDBMS のインデックス実装に広く使われる。B+ 木は全データを葉に格納。' },
  { term: 'ハッシュ衝突', category: 'algorithm', definition: '異なるキーが同じハッシュ値（インデックス）にマッピングされる現象。解消法：チェイン法（同一スロットにリスト連結）・オープンアドレス法（線形探索・二次探索・ダブルハッシュ）。負荷率（n/m）が高いと衝突頻度が増加する。' },
  // ネットワーク
  { term: 'OSPF', category: 'network', definition: 'Open Shortest Path First。リンク状態型 IGP（内部ゲートウェイプロトコル）。各ルータがリンクステート情報を全ルータに配布し、ダイクストラ法で最短経路を計算する。エリア分割でスケーラビリティを確保。RIP より高速収束する。' },
  { term: 'ARP', category: 'network', definition: 'Address Resolution Protocol。IP アドレスから MAC アドレスを解決するプロトコル（L3→L2）。ブロードキャストで問い合わせ、対象ホストがユニキャストで返答する。結果は ARP キャッシュに保存。ARP スプーフィングは MITM 攻撃に悪用される。' },
  { term: 'DHCP', category: 'network', definition: 'Dynamic Host Configuration Protocol。クライアントに IP アドレス・サブネットマスク・デフォルトゲートウェイ・DNS サーバを動的に割り当てるプロトコル。DORA プロセス（Discover→Offer→Request→Ack）で動作。UDP ポート 67/68 を使用。' },
  { term: 'CDN', category: 'network', definition: 'Content Delivery Network。世界中にエッジサーバを分散配置し、ユーザに地理的に近いサーバからコンテンツを配信する仕組み。レイテンシ削減・オリジンサーバ負荷軽減・DDoS 耐性向上が主なメリット。Cloudflare・Akamai が代表例。' },
  // データベース
  { term: 'ストアドプロシージャ', category: 'database', definition: 'データベースサーバに保存された SQL 手続きの集まり。クライアントからプロシージャ名で呼び出せる。ネットワーク通信量削減・再利用性向上・セキュリティ強化（直接 SQL 発行不要）の利点。ただしバージョン管理・テストが難しい欠点がある。' },
  { term: 'MVCC', category: 'database', definition: 'Multi-Version Concurrency Control（多版型同時実行制御）。更新時に古いバージョンを残し、トランザクションごとに一貫したスナップショットを提供する機構。読み取りがブロックされず同時実行性が高い。PostgreSQL・InnoDB が採用。' },
  { term: 'シャーディング', category: 'database', definition: 'データを複数のデータベースサーバ（シャード）に水平分割して格納するスケールアウト手法。各シャードは独立したデータベースインスタンス。分割キー（シャードキー）の選定が性能に大きく影響する。MongoDB・Cassandra が採用。' },
  // セキュリティ
  { term: 'OAuth 2.0', category: 'security', definition: 'サードパーティアプリケーションがユーザの認証情報を直接取得せずにリソースへのアクセス権限を委譲するための認可フレームワーク（RFC 6749）。アクセストークン・リフレッシュトークンを使用。SNS ログイン連携の標準実装。' },
  { term: 'JWT', category: 'security', definition: 'JSON Web Token（RFC 7519）。Header・Payload・Signature を Base64URL エンコードして `.` で連結したトークン形式。ステートレス認証に使用。署名検証で改ざん検知が可能だが、ペイロードは暗号化されないため機密情報を含めてはならない。' },
  { term: 'ランサムウェア', category: 'security', definition: '感染端末のファイルを暗号化し、復号キーと引き換えに身代金（ransom）を要求するマルウェア。ネットワーク共有フォルダやバックアップも暗号化する亜種が多い。対策：定期バックアップ・オフライン保管・OSパッチ適用・メール添付ファイルの注意。' },
  { term: 'EDR', category: 'security', definition: 'Endpoint Detection and Response。エンドポイント（PC・サーバ）での不審な挙動をリアルタイム監視・検知・対応するセキュリティソリューション。従来のウイルス対策（シグネチャベース）を補完し、未知の脅威や内部不正にも対応する。' },
  // OS
  { term: 'スピンロック', category: 'os', definition: 'ロックが解放されるまでビジーウェイト（空ループ）し続ける排他制御機構。コンテキストスイッチが不要なため短時間のロックに高速。マルチプロセッサ環境で有効。長時間ロックされると CPU 資源を無駄に消費するため適切な使い分けが重要。' },
  { term: 'コピーオンライト', category: 'os', definition: 'Copy-on-Write（CoW）。共有リソースの変更が必要になるまでコピーを遅延させる最適化技術。fork() システムコール・コンテナイメージ・ファイルシステム（ZFS・Btrfs）で活用。変更が発生した時点でのみコピーするためメモリ・ストレージを節約できる。' },
  { term: 'ナンバリング（ページング）', category: 'os', definition: '仮想記憶方式の一つ。論理アドレス空間を固定長のページに分割し、物理メモリのフレームに割り当てる。外部フラグメンテーションが発生せず、プロセスに連続した物理メモリを必要としない。TLB でページテーブルアクセスを高速化する。' },
  // ソフトウェア開発
  { term: 'リファクタリング', category: 'software-dev', definition: '外部から観察できる動作を変えずにコード内部の構造を改善する手法。目的：可読性向上・保守性向上・技術的負債の解消。Martin Fowler が体系化。TDD と組み合わせて「Red→Green→Refactor」サイクルで実践する。' },
  { term: 'API Gateway', category: 'software-dev', definition: 'クライアントからのリクエストを受け取り、適切なマイクロサービスに振り分けるリバースプロキシ。認証・レート制限・ロードバランシング・ログ集約・プロトコル変換を一元処理する。AWS API Gateway・Kong・Nginx が代表的実装。' },
  { term: 'イベント駆動アーキテクチャ', category: 'software-dev', definition: 'サービス間の結合をイベント（状態変化の通知）を通じて非同期に行うアーキテクチャパターン。発行者（Producer）と購読者（Consumer）が疎結合になり独立してスケール可能。Apache Kafka・RabbitMQ・AWS EventBridge が代表実装。' },
  // プロジェクトマネジメント
  { term: 'WBS', category: 'project-mgmt', definition: 'Work Breakdown Structure（作業分解構成図）。プロジェクトの成果物と作業を階層的に分解した構造図。最下層の作業パッケージを見積もり・スケジュール・コスト計画の基礎とする。PMBOK で計画フェーズの中核成果物として位置づけられる。' },
  { term: 'スコープクリープ', category: 'project-mgmt', definition: 'プロジェクトのスコープが承認なく段階的に拡大する現象。コスト超過・スケジュール遅延の主要原因の一つ。対策：変更管理プロセスの徹底・スコープ定義書のベースライン化・ステークホルダとの認識合わせ。' },
  { term: 'ステークホルダマネジメント', category: 'project-mgmt', definition: 'プロジェクトに影響を与える・受ける関係者（ステークホルダ）を識別し、影響力・関心度を分析して適切に関与させるプロセス。PMBOK 第 5 版で独立した知識エリアとして追加。権力/関心マトリクスで分類・管理する。' },
  // サービスマネジメント
  { term: 'CMDB', category: 'service-mgmt', definition: 'Configuration Management Database（構成管理データベース）。IT 基盤を構成する CI（構成アイテム）とその関係を管理するデータベース。変更管理・インシデント管理・問題管理の基盤情報として機能する。ITIL の SACM（サービス資産・構成管理）の中核。' },
  { term: 'RCA', category: 'service-mgmt', definition: 'Root Cause Analysis（根本原因分析）。インシデントや問題の表面的な症状だけでなく、再発防止のために根本原因を特定する手法。ITIL の問題管理プロセスで実施。5 Why（なぜなぜ分析）・フィッシュボーン図が代表的分析手法。' },
  // ビジネストラテジ
  { term: 'DX（デジタルトランスフォーメーション）', category: 'business-strategy', definition: 'デジタル技術を活用してビジネスモデル・業務プロセス・企業文化を根本から変革し、競争優位を確立する取り組み。IPA が定義する DX 推進指標では、データ活用・IT 活用・人材育成の成熟度を評価する。単なる IT 化と区別される。' },
  { term: 'KGI / KPI', category: 'business-strategy', definition: 'KGI（Key Goal Indicator）：最終目標の達成度を測る指標（例：売上 10 億円）。KPI（Key Performance Indicator）：KGI 達成に向けた中間プロセスの指標（例：月間新規顧客数）。OKR（Objectives and Key Results）でも同様の概念を使う。' },
  // 法規
  { term: 'プロバイダ責任制限法', category: 'legal', definition: '特定電気通信役務提供者の損害賠償責任の制限及び発信者情報の開示に関する法律。ネット上の情報で権利侵害された場合にプロバイダへの発信者情報開示請求を可能にする。2022 年改正で開示請求の非訟手続が新設され手続が迅速化された。' },
  { term: '不正競争防止法', category: 'legal', definition: '営業秘密の不正取得・使用・開示や技術的制限手段の回避などを禁止する法律。IT 分野では、リバースエンジニアリングによるコピープロテクト解除・営業秘密データの持ち出しが規制対象となる。民事・刑事の両面で保護措置がある。' },

  // ── Phase 18 追加（10語） ──────────────────────────
  { term: 'フォールトトレランス', category: 'os', definition: '障害が発生しても正常に動作し続けるシステムの性質。冗長化・フェイルオーバー・エラー訂正符号などで実現。フォールトアボイダンス（障害発生自体を防ぐ）と対になる概念。NASAの宇宙探査機・金融系基幹システムで重要視される。' },
  { term: 'マイクロサービス', category: 'software-dev', definition: '単一のアプリケーションを独立してデプロイ可能な小さなサービス群に分割するアーキテクチャスタイル。各サービスは独立したデータストアを持ち API で通信する。スケーラビリティ・技術選択の自由度が向上するが、分散システムの複雑さが増す。' },
  { term: 'ゼロデイ脆弱性', category: 'security', definition: 'ベンダが認識しておらず、修正パッチが存在しない脆弱性。発見から公開（day 0）までの期間に悪用される。APT 攻撃で多用され、防御が非常に困難。緩和策：多層防御・振る舞い検知・最小権限原則。' },
  { term: 'アジャイル開発', category: 'software-dev', definition: '反復的・漸進的にソフトウェアを開発する手法群の総称。アジャイルマニフェスト（2001年）に基づく。スクラム・XP・カンバンが代表的手法。固定スコープのウォーターフォールに対し、変化する要件への適応性を重視する。' },
  { term: 'データウェアハウス', category: 'database', definition: '意思決定支援を目的として、複数のシステムから履歴データを統合・蓄積するデータベース。ETL（Extract/Transform/Load）でデータを整形して投入する。OLAP（多次元分析）で利用。列指向ストレージ（Snowflake・BigQuery・Redshift）が一般的。' },
  { term: 'スケールアウト', category: 'system-arch', definition: 'システムの処理能力を高めるために同種のサーバを追加する水平スケーリング手法。対義語のスケールアップは既存サーバの CPU・メモリを増強する垂直スケーリング。クラウドでは自動スケールアウト（オートスケーリング）が一般的。負荷分散機構が必要。' },
  { term: 'ITILの4つのP', category: 'service-mgmt', definition: 'ITIL v3 のサービス品質に関わる 4 要素：People（人材・組織）・Process（プロセス）・Products（技術・ツール）・Partners（パートナー・サプライヤ）。ITIL 4 では SVS と 4 次元モデルに発展した。' },
  { term: 'ABC分析', category: 'business-strategy', definition: '売上・在庫・コストなどを金額の大きい順に並べ、累計割合でA（上位80%）・B（80-95%）・C（95-100%）に分類する分析手法。パレートの法則（80:20の法則）を応用。在庫管理・顧客管理・コスト削減の優先順位付けに活用される。' },
  { term: 'FIDO2 / WebAuthn', category: 'security', definition: 'パスワードレス認証の国際標準。デバイスの生体認証器や PIN で秘密鍵を保護し、公開鍵暗号方式でサーバに認証する仕組み。フィッシング耐性が高く、パスワード漏洩リスクがない。主要ブラウザ・OS が対応。FIDO2 = WebAuthn（W3C）＋ CTAP（FIDO Alliance）の組み合わせ。' },
  { term: 'テクニカルデット', category: 'software-dev', definition: '技術的負債（Technical Debt）。短期的な速度優先で実装した不完全な設計・コードが、将来の変更コストを増大させること。Ward Cunningham が提唱したメタファ。定期的なリファクタリング・コードレビュー・テスト自動化で返済する。放置すると利子（開発速度低下）が増え続ける。' },
];
...
// ─── フェーズ16追加問題 (+70問) ─────────────────────────────
const QUESTIONS_DATA_PHASE16 = [
  // LG-081〜090 ロジック・数学 (10問)
  {
    id: 'LG-081', category: 'logic', categoryName: 'ロジック・数学',
    difficulty: 2, year: 2022, tags: ['論理回路','NAND'],
    question: 'NAND ゲートだけを使って AND 回路を構成する場合、NAND ゲートは最低いくつ必要か。',
    choices: [
      { index: 0, text: '1個' },
      { index: 1, text: '2個' },
      { index: 2, text: '3個' },
    ...
    answer: 1,
    explanation: 'NAND2個でANDを構成: A NAND B = ¬(A∧B)。これをさらに NOT すると AND になる。NOT(A NAND B) だが NOT は NAND に同じ入力を繋ぐと実現（1個）。したがって AND = NAND(A,B) の出力を NAND（同一入力）で反転 → 2個。',
  },
  {
    id: 'LG-082', category: 'logic', categoryName: 'ロジック・数学',
    difficulty: 3, year: 2023, tags: ['確率','組合せ'],
    question: '52枚のトランプから2枚を引くとき、どちらもスペードである確率はどれか。',
    choices: [
      { index: 0, text: '1/17' },
      { index: 1, text: '1/16' },
      { index: 2, text: '13/204' },
    ...
    answer: 2,
    explanation: '13/52 × 12/51 = 156/2652 = 1/17.07... を計算。または C(13,2)/C(52,2) = 78/1326 = 13/221 × (3/3) = 1/17。実際: C(13,2)=78、C(52,2)=1326、78/1326=13/221。選択肢に13/204があるが正しくは13/221。設問の正解として最も近いものを選ぶとすれば計算で求める。13/204 = 0.0637、13/221 ≒ 0.0588。',
  },
  {
    id: 'LG-083', category: 'logic', categoryName: 'ロジック・数学',
    difficulty: 2, year: 2021, tags: ['情報量','エントロピー'],
    question: '確率 1/8 の事象が発生したときの情報量（自己情報量）はどれか。',
    choices: [
      { index: 0, text: '1 ビット' },
      { index: 1, text: '2 ビット' },
      { index: 2, text: '3 ビット' },
    ...
    answer: 2,
    explanation: '自己情報量 I(x) = -log₂(P(x))。P = 1/8 のとき I = -log₂(1/8) = log₂(8) = 3 ビット。確率が低い（珍しい）事象ほど情報量が大きい。確率 1/2 → 1ビット、1/4 → 2ビット、1/1024 → 10ビット。',
  },
  {
    id: 'LG-084', category: 'logic', categoryName: 'ロジック・数学',
    difficulty: 3, year: 2022, tags: ['基数変換','16進数'],
    ...
    answer: 1,
    explanation: 'A3(16) = A×16¹ + 3×16⁰ = 10×16 + 3×1 = 160 + 3 = 163。16進数: 0-9はそのまま、A=10、B=11、C=12、D=13、E=14、F=15。各桁は右から 16⁰, 16¹, 16², … と重みが付く。',
  },
  {
    id: 'LG-085', category: 'logic', categoryName: 'ロジック・数学',
    difficulty: 2, year: 2023, tags: ['浮動小数点','IEEE754'],
    question: 'IEEE 754 単精度浮動小数点形式の符号・指数部・仮数部のビット数の組合せとして正しいものはどれか。',
    choices: [
      { index: 0, text: '符号1・指数8・仮数23（計32ビット）' },
      { index: 1, text: '符号1・指数11・仮数52（計64ビット）' },
      { index: 2, text: '符号2・指数8・仮数22（計32ビット）' },
      { index: 3, text: '符号1・指数7・仮数24（計32ビット）' },
    ],
    answer: 0,
    explanation: 'IEEE 754 単精度（32ビット）: 符号1ビット + 指数部8ビット + 仮数部23ビット。倍精度（64ビット）: 符号1 + 指数11 + 仮数52。指数部はバイアス（127）付き表現。仮数部は正規化により先頭の1が省略される（暗黙ビット）。',
  },
  {
    id: 'LG-086', category: 'logic', categoryName: 'ロジック・数学',
    difficulty: 3, year: 2021, tags: ['論理式','カルノー図'],
    question: 'ブール代数における吸収則として正しいものはどれか。',
    choices: [
      { index: 0, text: 'A + AB = A' },
      { index: 1, text: 'A + AB = B' },
      { index: 2, text: 'A · (A + B) = AB' },
      { index: 3, text: 'A + A = 2A' },
    ],
    answer: 0,
    explanation: '吸収則: A + AB = A(1 + B) = A×1 = A。同様に A(A + B) = A。配布則から証明できる。ブール代数の主要法則: ド・モルガン則（¬(A+B)=¬A·¬B）、分配則（A(B+C)=AB+AC）、冪等則（A+A=A, A·A=A）。',
  },
  {
    id: 'LG-087', category: 'logic', categoryName: 'ロジック・数学',
    difficulty: 2, year: 2023, tags: ['集合','ベン図'],
    question: '集合 A = {1,2,3,4,5}、B = {3,4,5,6,7} のとき、A ∩ B（積集合）はどれか。',
    choices: [
      { index: 0, text: '{1,2,3,4,5,6,7}' },
      { index: 1, text: '{3,4,5}' },
      { index: 2, text: '{1,2,6,7}' },
      { index: 3, text: '{1,2}' },
    ],
    answer: 1,
    explanation: '積集合（intersection）A ∩ B は A と B 両方に含まれる要素の集合。A と B 共通の要素: 3, 4, 5 → {3,4,5}。和集合（union）A ∪ B = {1,2,3,4,5,6,7}。差集合 A − B = {1,2}、B − A = {6,7}。',
  },
  {
    id: 'LG-088', category: 'logic', categoryName: 'ロジック・数学',
    difficulty: 3, year: 2022, tags: ['フリップフロップ','順序回路'],
    question: 'D フリップフロップの説明として正しいものはどれか。',
    choices: [
      { index: 0, text: 'クロックエッジ時に入力 D の値をそのまま出力 Q に取り込む' },
      { index: 1, text: 'セット入力とリセット入力で出力を制御する基本的なラッチ回路' },
      { index: 2, text: 'クロックごとに出力を反転するトグル動作を行う' },
      { index: 3, text: 'J=1, K=0 のとき出力をセット、J=0, K=1 のときリセットする' },
    ],
    answer: 0,
    explanation: 'D フリップフロップ: クロックの立ち上がり（または立ち下がり）エッジ時に D 入力をラッチし Q に出力する。選択肢1は SR ラッチ、選択肢2は T フリップフロップ、選択肢3は JK フリップフロップの説明。デジタル回路の基本記憶素子でレジスタに使用。',
  },
  {
    id: 'LG-089', category: 'logic', categoryName: 'ロジック・数学',
    difficulty: 2, year: 2021, tags: ['オートマトン','有限状態機械'],
    question: '有限オートマトン（FA）が「決定的」（DFA）である条件はどれか。',
    choices: [
      { index: 0, text: '任意の状態とシンボルに対して次の状態が必ず0または1つに定まる' },
      { index: 1, text: '任意の入力に対してε（空）遷移が許容される' },
      { index: 2, text: 'スタックを利用できる' },
      { index: 3, text: '状態数に上限がない' },
    ],
    answer: 0,
    explanation: 'DFA（決定性有限オートマトン）: 各状態とシンボルの組に対して次の状態が一意に決まる。NFA（非決定性）はε遷移や複数の次状態を許す。DFA と NFA は同じ正規言語を受理（サブセット構成で変換可能）。スタックを持つのはプッシュダウンオートマトン（文脈自由言語）。',
  },
  {
    id: 'LG-090', category: 'logic', categoryName: 'ロジック・数学',
    difficulty: 3, year: 2023, tags: ['グレイコード','誤り訂正'],
    question: 'グレイコードの特徴として正しいものはどれか。',
    choices: [
      { index: 0, text: '連続する数値間で必ず1ビットだけ変化する符号' },
      { index: 1, text: '偶数パリティによる1ビット誤り検出符号' },
      { index: 2, text: '奇数パリティを含む2ビット誤り検出符号' },
      { index: 3, text: 'データを圧縮するための可変長符号' },
    ],
    answer: 0,
    explanation: 'グレイコード（反射符号）: 隣接する値の間で変化するビットが常に1ビットだけ。例: 0→1(00→01)、1→2(01→11)、2→3(11→10)。アナログ-デジタル変換のエンコーダや回転角センサに使用（多ビット変化による過渡的な誤り防止）。XORでバイナリとの相互変換が可能。',
  },

  // PM-039〜048 プロジェクトマネジメント (10問)
  {
    id: 'PM-039', category: 'project-mgmt', categoryName: 'プロジェクトマネジメント',
    difficulty: 2, year: 2023, tags: ['スコープ','WBS'],
    question: 'WBS（Work Breakdown Structure）の「100% ルール」の説明として正しいものはどれか。',
    choices: [
      { index: 0, text: 'WBSのすべてのタスクを100時間以内に完了させる規則' },
      { index: 1, text: 'WBSの各レベルの子要素を合計するとその親要素の作業を100%カバーする' },
      { index: 2, text: 'プロジェクトの最初の100日間でWBSを完成させる' },
      { index: 3, text: 'チームメンバーが100%の稼働率でタスクに取り組む' },
    ],
    answer: 1,
    explanation: 'WBS 100%ルール: 親作業パッケージを分解した子要素の合計が、親の作業を過不足なく100%表現しなければならない。これによりスコープの重複や漏れを防ぐ。WBSはプロジェクトスコープの全体を階層分解した成果物指向の構造。',
  },
  {
    id: 'PM-040', category: 'project-mgmt', categoryName: 'プロジェクトマネジメント',
    difficulty: 3, year: 2022, tags: ['品質管理','QC7つ道具'],
    question: 'QC7つ道具のうち、問題の原因と結果の因果関係を魚の骨状に表現する図はどれか。',
    choices: [
      { index: 0, text: 'パレート図' },
      { index: 1, text: 'ヒストグラム' },
      { index: 2, text: '特性要因図（フィッシュボーン図）' },
    ...
    answer: 2,
    explanation: '特性要因図（石川ダイアグラム、フィッシュボーン図）: 問題（特性）の原因を魚の骨状に展開。大骨（主原因）→小骨（詳細原因）。パレート図は重要度順の棒グラフ（80:20の法則）、ヒストグラムは度数分布、散布図は相関を可視化。',
  },
  {
    id: 'PM-041', category: 'project-mgmt', categoryName: 'プロジェクトマネジメント',
    difficulty: 2, year: 2021, tags: ['コミュニケーション','ステークホルダ'],
    question: 'プロジェクトのステークホルダが 5 人から 8 人に増えた場合、コミュニケーションチャネル数の増加はどれか（n(n-1)/2 の公式を使用）。',
    choices: [
      { index: 0, text: '10 から 28 に増加（18 増加）' },
      { index: 1, text: '15 から 36 に増加（21 増加）' },
      { index: 2, text: '10 から 21 に増加（11 増加）' },
      { index: 3, text: '20 から 36 に増加（16 増加）' },
    ],
    answer: 0,
    explanation: 'コミュニケーションチャネル数 = n(n-1)/2。5人: 5×4/2=10、8人: 8×7/2=28。増加: 18チャネル。人数が増えると管理コミュニケーションが指数的に増加するため、大規模プロジェクトでは明確なコミュニケーション計画が必須。',
  },
  {
    id: 'PM-042', category: 'project-mgmt', categoryName: 'プロジェクトマネジメント',
    difficulty: 3, year: 2023, tags: ['リスク','定量的リスク分析'],
    question: '定量的リスク分析で使用される「期待金額価値（EMV）」の計算式はどれか。',
    choices: [
      { index: 0, text: 'リスクの発生確率 + リスクの影響額' },
      { index: 1, text: 'リスクの発生確率 × リスクの影響額' },
      { index: 2, text: 'リスクの影響額 ÷ リスクの発生確率' },
      { index: 3, text: 'リスクの影響額 − リスクの発生確率' },
    ],
    answer: 1,
    explanation: 'EMV（Expected Monetary Value）= 確率 × 影響額。リスク（脅威）は負の値、機会は正の値で扱う。例: 確率30%、影響-100万円 → EMV=-30万円。デシジョンツリー分析と組合せて複数シナリオの期待値を比較する手法。',
  },
  {
    id: 'PM-043', category: 'project-mgmt', categoryName: 'プロジェクトマネジメント',
    difficulty: 2, year: 2022, tags: ['調達管理','SOW'],
    question: 'SOW（Statement of Work）の説明として正しいものはどれか。',
    choices: [
      { index: 0, text: 'プロジェクトの組織構成と責任を定義する文書' },
      { index: 1, text: '外部ベンダへの発注時に、要求する製品・サービス・成果物の詳細を記した作業内容記述書' },
      { index: 2, text: 'プロジェクトの予算を部門別に割り当てた費用計画書' },
      { index: 3, text: 'スプリントバックログを優先度順に並べたアジャイル計画書' },
    ],
    answer: 1,
    explanation: 'SOW（作業内容記述書）: 調達時に購入者が提供する文書。スコープ・仕様・要件・期間・成果物・検収条件を詳細に記述。ベンダが入札・提案書（RFP応答）を作成する際の根拠となる。RFP（提案依頼書）に含まれることが多い。',
  },
  {
    id: 'PM-044', category: 'project-mgmt', categoryName: 'プロジェクトマネジメント',
    difficulty: 3, year: 2021, tags: ['スケジュール','クリティカルパス'],
    question: 'アロー図（PERT）でクリティカルパスを特定する目的として正しいものはどれか。',
    choices: [
      { index: 0, text: 'プロジェクトの総コストを最小化するパスを見つける' },
      { index: 1, text: '最長のパスを特定することでプロジェクト全体の最短完了時間を把握する' },
      { index: 2, text: '余裕時間（フロート）が最も大きいタスクを見つける' },
      { index: 3, text: '最も少ない人数で完了できるタスクの順序を決める' },
    ],
    answer: 1,
    explanation: 'クリティカルパス（CP）: 開始から完了までのすべての経路で最長のもの。CP 上のタスクはフロート=0で、1日でも遅延するとプロジェクト全体が遅れる。CPM（Critical Path Method）はCP特定とスケジュール最適化に使用。フロート（余裕時間）= 最遅開始 - 最早開始。',
  },
  {
    id: 'PM-045', category: 'project-mgmt', categoryName: 'プロジェクトマネジメント',
    difficulty: 2, year: 2023, tags: ['変更管理','統合変更管理'],
    question: '統合変更管理プロセスにおいて変更要求を処理する順序として正しいものはどれか。',
    choices: [
      { index: 0, text: '変更要求受領 → 変更実施 → 影響分析 → 承認' },
      { index: 1, text: '変更要求受領 → 影響分析 → 変更管理委員会（CCB）による承認 → 変更実施 → 記録' },
      { index: 2, text: '変更要求受領 → 即時実施 → 事後報告' },
      { index: 3, text: '変更要求受領 → 拒否 → ステークホルダへ報告' },
    ],
    answer: 1,
    explanation: '統合変更管理の正しいフロー: 変更要求（CR）→ 影響評価（スコープ・スケジュール・コストへの影響）→ CCB（変更管理委員会）による承認/棄却 → 承認された場合のみ実施 → 計画・文書・ベースライン更新 → ステークホルダ通知。承認なしの変更はスコープクリープの原因。',
  },
  {
    id: 'PM-046', category: 'project-mgmt', categoryName: 'プロジェクトマネジメント',
    difficulty: 3, year: 2022, tags: ['人的資源','モチベーション理論'],
    question: 'ハーズバーグの二要因理論（動機付け・衛生要因）における「動機付け要因（モチベーター）」として正しいものはどれか。',
    choices: [
      { index: 0, text: '給与・福利厚生・労働条件' },
      { index: 1, text: '達成感・承認・成長・責任感' },
      { index: 2, text: '会社の方針・上司との関係' },
      { index: 3, text: '職場環境・対人関係' },
    ],
    answer: 1,
    explanation: 'ハーズバーグの二要因理論: 動機付け要因（達成・承認・仕事自体・成長・責任）は満足感を高め意欲を向上させる。衛生要因（給与・条件・上司・会社方針）は不満の防止はできるが積極的な動機付けにはならない。プロジェクトマネージャはチームメンバの動機付け要因を意識した環境づくりが重要。',
  },
  {
    id: 'PM-047', category: 'project-mgmt', categoryName: 'プロジェクトマネジメント',
    difficulty: 2, year: 2021, tags: ['品質','シックスシグマ'],
    question: 'シックスシグマ（6σ）の説明として正しいものはどれか。',
    choices: [
      { index: 0, text: '6種類の品質指標を使ってプロセスを評価する手法' },
      { index: 1, text: 'プロセスの変動を標準偏差6個分に収めることでも不良率を100万回に3.4回以下にする品質改善手法' },
      { index: 2, text: 'プロジェクトを6フェーズに分けて進捗管理する手法' },
      { index: 3, text: 'ソフトウェア開発における6層アーキテクチャの標準' },
    ],
    answer: 1,
    explanation: 'シックスシグマ（6σ）: 平均値±6σ以内にプロセスアウトプットを収めることで不良率を 3.4ppm（100万回に3.4個）以下にする品質改善の枠組み。DMAIC（Define-Measure-Analyze-Improve-Control）サイクルを使用。GE・モトローラが普及。ISO 13053 で標準化。',
  },
  {
    id: 'PM-048', category: 'project-mgmt', categoryName: 'プロジェクトマネジメント',
    difficulty: 3, year: 2023, tags: ['アジャイル','スクラム'],
    question: 'スクラムにおけるプロダクトオーナーの役割として最も適切なものはどれか。',
    choices: [
      { index: 0, text: 'スプリント中の開発作業を直接マネジメントし技術指示を出す' },
      { index: 1, text: 'プロダクトバックログを管理し、ビジネス価値に基づいて優先順位を決定する' },
      { index: 2, text: 'スクラムチームが適切なプロセスに従えるようコーチングする' },
      { index: 3, text: '全スプリントのスケジュールを詳細に作成・管理する' },
    ],
    answer: 1,
    explanation: 'プロダクトオーナー（PO）はプロダクトバックログを所有し、ビジネス価値の最大化を目的に優先順位を管理する。選択肢3はスクラムマスターの役割（プロセスのファシリテーター）。開発チームは自己組織化で技術決定を行う。POは「What」を決め、開発チームは「How」を決める。',
  },

  // SM-034〜043 サービスマネジメント (10問)
  {
    id: 'SM-034', category: 'service-mgmt', categoryName: 'サービスマネジメント',
    difficulty: 2, year: 2023, tags: ['ITIL','サービスデスク'],
    question: 'ITIL においてサービスデスクの主な機能として正しいものはどれか。',
    choices: [
      { index: 0, text: 'ユーザとのすべてのコミュニケーションの単一窓口（SPOC）として機能する' },
      { index: 1, text: 'ソフトウェアの品質テストと不具合修正を担当する' },
      { index: 2, text: 'IT インフラのキャパシティ計画を立案する' },
      { index: 3, text: 'セキュリティ監査を実施し報告書を作成する' },
    ],
    answer: 0,
    explanation: 'サービスデスク（ヘルプデスク）は ITIL サービス運用における Single Point of Contact（SPOC）。ユーザからのインシデント報告・サービス要求を受け付け、記録・分類・初期診断・エスカレーションを行う。顧客満足とサービス品質維持の最前線。',
  },
  {
    id: 'SM-035', category: 'service-mgmt', categoryName: 'サービスマネジメント',
    difficulty: 3, year: 2022, tags: ['変更管理','緊急変更'],
    question: 'ITIL の変更管理プロセスにおける「緊急変更（Emergency Change）」の説明として正しいものはどれか。',
    choices: [
      { index: 0, text: '3ヶ月に1度の定期メンテナンスで実施する計画変更' },
      { index: 1, text: 'インシデント解決などのために通常の変更プロセスを省略・短縮して迅速に実施する変更' },
      { index: 2, text: '既に承認されていて定期的に実施される標準変更' },
      { index: 3, text: '全ての部署の承認が必要な重大変更' },
    ],
    answer: 1,
    explanation: '変更の種類: 標準変更（事前承認済み・低リスク・ルーチン）、通常変更（CCBレビュー・承認が必要）、緊急変更（インシデント対応などで迅速な変更が必要・ECAB=緊急変更諮問委員会が承認）。緊急変更は後から文書化・レビューを行う。',
  },
  {
    id: 'SM-036', category: 'service-mgmt', categoryName: 'サービスマネジメント',
    difficulty: 2, year: 2021, tags: ['SLA','MTBF'],
    question: '稼働率の計算式として正しいものはどれか。',
    choices: [
      { index: 0, text: 'MTTR ÷ (MTBF + MTTR)' },
      { index: 1, text: 'MTBF ÷ (MTBF + MTTR)' },
      { index: 2, text: '(MTBF − MTTR) ÷ MTBF' },
      { index: 3, text: 'MTBF ÷ MTTR' },
    ],
    answer: 1,
    explanation: '稼働率 = MTBF ÷ (MTBF + MTTR)。MTBF（Mean Time Between Failures）= 平均故障間隔（稼働時間の平均）、MTTR（Mean Time To Repair）= 平均修復時間。例: MTBF=9時間、MTTR=1時間 → 稼働率=9/(9+1)=90%。稼働率向上には MTBF を増やすか MTTR を短縮する。',
  },
  {
    id: 'SM-037', category: 'service-mgmt', categoryName: 'サービスマネジメント',
    difficulty: 3, year: 2023, tags: ['問題管理','根本原因分析'],
    question: 'ITIL のインシデント管理と問題管理の違いとして正しいものはどれか。',
    choices: [
      { index: 0, text: 'インシデント管理は根本原因を分析し、問題管理はサービスを速やかに回復させる' },
      { index: 1, text: 'インシデント管理は速やかなサービス回復を目的とし、問題管理は根本原因を特定・排除する' },
      { index: 2, text: '両者は全く同じプロセスで名称が違うだけ' },
      { index: 3, text: 'インシデント管理はセキュリティ障害専用で、問題管理は一般障害対応' },
    ],
    answer: 1,
    explanation: 'インシデント管理: 障害発生時にサービスを可能な限り速やかに回復（暫定対処・回避策も使用）。根本原因追求より速度優先。問題管理: インシデントの根本原因（Problem）を特定・排除して再発防止。既知エラー（Known Error）データベースで管理。両者は連携して機能。',
  },
  {
    id: 'SM-038', category: 'service-mgmt', categoryName: 'サービスマネジメント',
    difficulty: 2, year: 2022, tags: ['構成管理','CMDB'],
    question: 'CMDB（Configuration Management Database）の説明として正しいものはどれか。',
    choices: [
      { index: 0, text: 'ソースコードのバージョン管理を行うためのリポジトリ' },
      { index: 1, text: 'IT インフラの構成アイテム（CI）とその関係を記録・管理するデータベース' },
      { index: 2, text: 'SLA の達成状況を記録する月次レポートシステム' },
      { index: 3, text: 'セキュリティインシデントの記録専用データベース' },
    ],
    answer: 1,
    explanation: 'CMDB は ITIL 構成管理プロセスの中核。サーバ・ネットワーク機器・ソフトウェア・ドキュメントなどの構成アイテム（CI）とそれら間の関係を記録。変更管理・インシデント管理・問題管理などと連携し、変更の影響範囲分析に活用される。',
  },
  {
    id: 'SM-039', category: 'service-mgmt', categoryName: 'サービスマネジメント',
    difficulty: 3, year: 2021, tags: ['継続性管理','BCP'],
    question: 'IT サービス継続性管理（ITSCM）の目的として正しいものはどれか。',
    choices: [
      { index: 0, text: '毎日のバックアップを自動化することで日常運用を効率化する' },
      { index: 1, text: '重大インシデント・災害時でも IT サービスをビジネスに必要な最低限以上のレベルで継続できるよう準備する' },
      { index: 2, text: 'セキュリティ脆弱性のパッチを自動適用し常に最新の状態を維持する' },
      { index: 3, text: 'システム開発プロジェクトの継続的デリバリを保証する' },
    ],
    answer: 1,
    explanation: 'ITSCM（IT Service Continuity Management）: 重大な障害・災害（大規模停電・火災・自然災害等）発生時でも、業務継続計画（BCP）を支える形で IT サービスを継続・迅速に回復するための計画・テスト・維持を行うプロセス。RTO・RPO の目標設定、代替サイトの準備、DR 訓練などを含む。',
  },
  {
    id: 'SM-040', category: 'service-mgmt', categoryName: 'サービスマネジメント',
    difficulty: 2, year: 2023, tags: ['リリース管理','デプロイ'],
    question: 'ITIL のリリース管理プロセスの目的として正しいものはどれか。',
    choices: [
      { index: 0, text: '本番環境への変更を制御・調整し、サービス中断リスクを最小化してリリースする' },
      { index: 1, text: 'ソフトウェアのソースコードをリポジトリで管理する' },
      { index: 2, text: 'インシデント発生時の対応チームを編成する' },
      { index: 3, text: 'IT 資産の財務的な価値を評価・報告する' },
    ],
    answer: 0,
    explanation: 'リリース管理（Release Management）: 承認された変更を本番環境にリリースするための計画・テスト・実施・レビューを行う。本番への無許可の変更防止・安定性確保が目的。DevOps の CD（継続的デリバリ）パイプラインとも連携する現代的なリリース管理が普及。',
  },
  {
    id: 'SM-041', category: 'service-mgmt', categoryName: 'サービスマネジメント',
    difficulty: 3, year: 2022, tags: ['ナレッジ管理','ITIL4'],
    question: 'ITIL 4 における「SVS（Service Value System）」に含まれる主要要素の組合せとして正しいものはどれか。',
    choices: [
      { index: 0, text: '指導原則・ガバナンス・サービスバリューチェーン・プラクティス・継続的改善' },
      { index: 1, text: 'サービスデスク・変更管理・問題管理・インシデント管理・リリース管理' },
      { index: 2, text: 'PDCA・QC7つ道具・KPI・ダッシュボード・SLA' },
      { index: 3, text: '人・プロセス・技術・ガバナンスの4要素のみ' },
    ],
    answer: 0,
    explanation: 'ITIL 4 のサービスバリューシステム（SVS）の5要素: (1)指導原則（7原則）、(2)ガバナンス、(3)サービスバリューチェーン（6つの活動）、(4)34のプラクティス（旧プロセス）、(5)継続的改善。ITIL 3 の5巻構成から ITIL 4 では SVS と4次元モデルに再編された。',
  },
  {
    id: 'SM-042', category: 'service-mgmt', categoryName: 'サービスマネジメント',
    difficulty: 2, year: 2021, tags: ['監視','モニタリング'],
    question: 'IT サービス監視（モニタリング）の主な目的として適切なものはどれか。',
    choices: [
      { index: 0, text: 'ユーザがシステムをどのように利用しているかを記録し給与評価に活用する' },
      { index: 1, text: 'システムの状態を継続的に把握し、異常の早期検知・インシデント防止・パフォーマンス管理に役立てる' },
      { index: 2, text: '競合他社のシステムと自社システムの性能を比較する' },
      { index: 3, text: '開発チームのコード品質を自動的に評価する' },
    ],
    answer: 1,
    explanation: 'IT サービス監視（Event Management/Monitoring）: CPU・メモリ・ディスク・ネットワーク・アプリログ・死活監視などをリアルタイムで収集・分析。閾値超過・異常パターン検知時にアラート発報。インシデントの予防的検知（プロアクティブ管理）と SLA 遵守の確認に活用。Zabbix・Nagios・Datadog などが使用される。',
  },
  {
    id: 'SM-043', category: 'service-mgmt', categoryName: 'サービスマネジメント',
    difficulty: 3, year: 2023, tags: ['アジャイルIT','DevOps'],
    question: 'DevOps の目標として最も適切なものはどれか。',
    choices: [
      { index: 0, text: '開発チームと運用チームを完全に分離し責任範囲を明確化する' },
      { index: 1, text: '開発・テスト・運用・セキュリティの連携を強化し、高品質なソフトウェアを素早く継続的に提供する' },
      { index: 2, text: 'ウォーターフォール開発プロセスを厳格に実施するための規律' },
      { index: 3, text: 'IT サービス部門のコスト削減のみを目的とした施策' },
    ],
    answer: 1,
    explanation: 'DevOps: 開発（Dev）と運用（Ops）の文化・プロセス・ツールを統合し、素早い反復的デリバリを実現する考え方。3つの道（フロー・フィードバック・継続的学習）、CALMS（Culture/Automation/Lean/Measurement/Sharing）が指針。CI/CD・IaC・自動テスト・モニタリングを組合せる。DevSecOps はセキュリティもシフトレフト統合。',
  },

  // BS-037〜046 ビジネス戦略 (10問)
  {
    id: 'BS-037', category: 'business-strategy', categoryName: 'システム戦略・経営戦略',
    difficulty: 2, year: 2022, tags: ['DX','デジタルトランスフォーメーション'],
    question: 'DX（デジタルトランスフォーメーション）の定義として最も適切なものはどれか。',
    choices: [
      { index: 0, text: '既存の業務をすべてシステム化・自動化すること' },
      { index: 1, text: 'デジタル技術を活用してビジネスモデル・組織・文化を変革し競争優位を確立すること' },
      { index: 2, text: 'クラウドへの移行（マイグレーション）のみを指す' },
      { index: 3, text: 'AI・機械学習を社内の全業務に導入すること' },
    ],
    answer: 1,
    explanation: 'DX（Digital Transformation）: 経産省の DX 推進ガイドラインでは「デジタル技術を活用して製品・サービス・ビジネスモデルを変革し、競争上の優位性を確立すること」と定義。単なる IT 化・自動化ではなく、ビジネスモデルや組織文化の変革を含む。2025年の崖：DX を推進しないと2025年以降年間最大12兆円の経済損失とされる。',
  },
  {
    id: 'BS-038', category: 'business-strategy', categoryName: 'システム戦略・経営戦略',
    difficulty: 3, year: 2023, tags: ['バランスト・スコアカード','BSC'],
    question: 'バランスト・スコアカード（BSC）の4つの視点として正しいものはどれか。',
    choices: [
      { index: 0, text: '財務・顧客・内部プロセス・学習と成長' },
      { index: 1, text: '品質・コスト・納期・安全' },
      { index: 2, text: '人材・技術・組織・戦略' },
      { index: 3, text: '売上・利益・市場シェア・顧客満足度' },
    ],
    answer: 0,
    explanation: 'BSC（Balanced Scorecard）: ロバート・キャプランとデビッド・ノートンが提唱。4視点: (1)財務（ROE・利益率等）、(2)顧客（顧客満足・市場シェア等）、(3)内部プロセス（業務効率・サイクルタイム等）、(4)学習と成長（スキル・IT・組織風土等）。KPI を設定し戦略目標との整合性を管理。',
  },
  {
    id: 'BS-039', category: 'business-strategy', categoryName: 'システム戦略・経営戦略',
    difficulty: 2, year: 2021, tags: ['SCM','サプライチェーン'],
    question: 'SCM（サプライチェーン管理）の目的として最も適切なものはどれか。',
    choices: [
      { index: 0, text: '社内の各部署間のコミュニケーションを最適化する' },
      { index: 1, text: '原材料の調達から最終顧客への納品まで、物・情報・資金の流れを統合的に管理して全体最適化する' },
      { index: 2, text: '顧客関係管理を高度化して顧客満足度を向上させる' },
      { index: 3, text: '財務諸表の作成・報告を自動化する' },
    ],
    answer: 1,
    explanation: 'SCM（Supply Chain Management）: 供給者→製造→物流→小売→顧客に至るサプライチェーン全体で在庫・リードタイム・コストを最適化する。ブルウィップ効果（需要変動の増幅）の抑制、JIT 調達、VMI（ベンダ管理在庫）などが手法。ERP・EDI（電子データ交換）で情報共有を促進。',
  },
  {
    id: 'BS-040', category: 'business-strategy', categoryName: 'システム戦略・経営戦略',
    difficulty: 3, year: 2022, tags: ['ビジネスモデル','プラットフォーム'],
    question: 'プラットフォームビジネスモデルの特徴として正しいものはどれか。',
    choices: [
      { index: 0, text: '自社で製品を製造・販売する垂直統合型の事業モデル' },
      { index: 1, text: '複数のユーザグループ（例: 買い手と売り手）を結びつけるインフラを提供し、相互作用から価値を生む' },
      { index: 2, text: '一つの製品に特化し高品質・高価格で市場シェアを獲得する' },
      { index: 3, text: 'フランチャイズ契約で全国に店舗を展開する事業形態' },
    ],
    answer: 1,
    explanation: 'プラットフォームビジネス（多面市場）: Amazon・Apple App Store・Uber・Airbnb 等。直接価値を生産せず、2つ以上のユーザグループが相互作用できる場を提供する。ネットワーク効果で勝者が独占しやすい。価値提供: コスト削減・リスク軽減・マッチング効率化。「プロデューサ」と「コンシューマ」間の摩擦を減らすことが鍵。',
  },
  {
    id: 'BS-041', category: 'business-strategy', categoryName: 'システム戦略・経営戦略',
    difficulty: 2, year: 2023, tags: ['競争戦略','ポーターの5力'],
    question: 'マイケル・ポーターの「5つの競争要因（ファイブフォース）」に含まれないものはどれか。',
    choices: [
      { index: 0, text: '既存競合企業との競争' },
      { index: 1, text: '新規参入者の脅威' },
      { index: 2, text: '代替品・サービスの脅威' },
      { index: 3, text: '政府規制の強さ' },
    ],
    answer: 3,
    explanation: 'ポーターの5力分析: (1)既存業者の競争、(2)新規参入の脅威、(3)代替品の脅威、(4)買い手（顧客）の交渉力、(5)売り手（供給者）の交渉力。政府規制は5力に含まれない（PEST 分析の政治要因）。業界の収益性・競争強度を分析し競争戦略策定に活用。',
  },
  {
    id: 'BS-042', category: 'business-strategy', categoryName: 'システム戦略・経営戦略',
    difficulty: 3, year: 2021, tags: ['IT投資評価','ROI'],
    question: 'IT 投資評価の方法として、将来のキャッシュフローを現在価値に割り引いて評価する手法はどれか。',
    choices: [
      { index: 0, text: 'ROI（投資対効果）' },
      { index: 1, text: 'TCO（総所有コスト）' },
      { index: 2, text: 'NPV（正味現在価値）' },
      { index: 3, text: 'PBP（投資回収期間）' },
    ],
    answer: 2,
    explanation: 'NPV（Net Present Value）: 投資の将来キャッシュフローを割引率（資本コスト）で現在価値に換算し、初期投資を引いた値。NPV>0なら投資価値あり。ROI=利益/投資額（%）、TCO=購入・運用・廃棄の総コスト、PBP=投資回収までの期間。NPV が最も時間価値を考慮した厳密な手法。',
  },
  {
    id: 'BS-043', category: 'business-strategy', categoryName: 'システム戦略・経営戦略',
    difficulty: 2, year: 2023, tags: ['EA','エンタープライズアーキテクチャ'],
    question: 'EA（エンタープライズアーキテクチャ）の4層構造として正しいものはどれか。',
    choices: [
      { index: 0, text: 'ビジネス・データ・アプリケーション・テクノロジーの4層' },
      { index: 1, text: '戦略・戦術・運用・評価の4層' },
      { index: 2, text: '人材・組織・プロセス・技術の4層' },
      { index: 3, text: '計画・設計・実装・運用の4層' },
    ],
    answer: 0,
    explanation: 'EA（ZachmanやTOGAF）の4層: (1)ビジネスアーキテクチャ（業務プロセス・組織・戦略）、(2)データアーキテクチャ（情報モデル・データ管理）、(3)アプリケーションアーキテクチャ（システム・インタフェース）、(4)テクノロジーアーキテクチャ（インフラ・ネットワーク・ハード）。EA は組織全体の IT と業務の整合性を確保する枠組み。',
  },
  {
    id: 'BS-044', category: 'business-strategy', categoryName: 'システム戦略・経営戦略',
    difficulty: 3, year: 2022, tags: ['グリーンIT','環境'],
    question: 'グリーン IT の説明として最も適切なものはどれか。',
    choices: [
      { index: 0, text: 'ディスプレイの色を緑系統にしてユーザの目の疲労を軽減する取り組み' },
      { index: 1, text: 'IT 機器の省エネ化・有害物質削減・廃棄物削減など、環境への影響を最小化するIT活用の考え方' },
      { index: 2, text: '農業のデジタル化（スマート農業）のみを指す' },
      { index: 3, text: 'データセンタを植林地の近くに建設する立地戦略' },
    ],
    answer: 1,
    explanation: 'グリーン IT: (1)IT by Green（IT の省エネ・CO₂削減）と (2)Green by IT（IT を使って社会全体の省エネを実現）の2面がある。PUE（Power Usage Effectiveness）= データセンタ全消費電力 / IT機器消費電力（低いほど効率的）。RoHS 指令（有害物質制限）、E-waste 問題も含む。',
  },
  {
    id: 'BS-045', category: 'business-strategy', categoryName: 'システム戦略・経営戦略',
    difficulty: 2, year: 2021, tags: ['データ活用','BI'],
    question: 'BI（ビジネスインテリジェンス）ツールの主な目的として正しいものはどれか。',
    choices: [
      { index: 0, text: 'AI が自律的にビジネス意思決定を行い人間の代わりに経営判断をする' },
      { index: 1, text: '企業内データを収集・分析・可視化し、経営者・管理者の意思決定を支援する' },
      { index: 2, text: '競合他社のデータを収集してベンチマーク分析を行う' },
      { index: 3, text: 'ERPシステムの業務トランザクションを高速に処理する' },
    ],
    answer: 1,
    explanation: 'BI（Business Intelligence）: データウェアハウス・データマートのデータをレポート・ダッシュボード・OLAP・データマイニングで分析し経営の意思決定を支援。Tableau・Power BI・Looker が代表的ツール。KPI のリアルタイム可視化・ドリルダウン分析・トレンド予測に活用。',
  },
  {
    id: 'BS-046', category: 'business-strategy', categoryName: 'システム戦略・経営戦略',
    difficulty: 3, year: 2023, tags: ['コンプライアンス','内部統制'],
    question: '日本の「金融商品取引法（J-SOX）」における内部統制報告制度の説明として正しいものはどれか。',
    choices: [
      { index: 0, text: '上場企業が IT システムのセキュリティについて第三者機関の認証を受ける制度' },
      { index: 1, text: '上場企業の経営者が財務報告に係る内部統制の有効性を評価・報告し、公認会計士が監査する制度' },
      { index: 2, text: 'IT 企業のみを対象とした情報セキュリティ基準の遵守報告制度' },
      { index: 3, text: '中小企業向けのIT投資補助金申請に必要な審査制度' },
    ],
    answer: 1,
    explanation: 'J-SOX（金融商品取引法 24 条の 4 の 4）: 上場企業の経営者が「財務報告に係る内部統制の整備・運用状況」を評価し、内部統制報告書を提出。公認会計士・監査法人が監査（内部統制監査）。米国の SOX 法（サーベンス・オクスリー法）を参考に 2008 年に導入。不正会計防止・財務情報の信頼性確保が目的。',
  },

  // SD-091〜100 ソフトウェア開発 (10問)
  {
    id: 'SD-091', category: 'software-dev', categoryName: 'ソフトウェア開発',
    difficulty: 2, year: 2023, tags: ['テスト','モック'],
    question: 'テストダブルにおけるモック（Mock）とスタブ（Stub）の違いとして正しいものはどれか。',
    choices: [
      { index: 0, text: 'スタブは検証（アサーション）を行い、モックは固定値を返すだけ' },
      { index: 1, text: 'モックは呼び出し回数・引数を検証し、スタブは固定値を返すだけ' },
      { index: 2, text: '両者は全く同じで名称だけが異なる' },
      { index: 3, text: 'モックは DB 接続のみ置き換え、スタブは API 呼び出しのみ置き換える' },
    ],
    answer: 1,
    explanation: 'テストダブルの種類: スタブ（Stub）: 固定の戻り値を返すだけの代替実装。モック（Mock）: 呼び出されたか・引数が正しいかを検証するための偽オブジェクト。フェイク（Fake）: 実際に動作する簡易実装（インメモリDB等）。スパイ（Spy）: 実際の実装に呼び出し情報を記録する機能を追加。',
  },
  {
    id: 'SD-092', category: 'software-dev', categoryName: 'ソフトウェア開発',
    difficulty: 3, year: 2022, tags: ['アーキテクチャ','ヘキサゴナル'],
    question: 'クリーンアーキテクチャ・ヘキサゴナルアーキテクチャの共通の目的として正しいものはどれか。',
    choices: [
      { index: 0, text: '全ての処理を1つのクラスに集約してシンプルにする' },
      { index: 1, text: 'ビジネスロジック（ドメイン）を外部依存（DB・UI・フレームワーク）から分離し、テスト可能性と保守性を高める' },
      { index: 2, text: '処理速度を最大化するためにすべてのレイヤーを省略する' },
      { index: 3, text: 'マイクロサービスとモノリスを交互に切り替える柔軟性を提供する' },
    ],
    answer: 1,
    explanation: 'クリーンアーキテクチャ（Uncle Bob）・ヘキサゴナル（ポート&アダプタ）アーキテクチャ: ビジネスルール（ユースケース・エンティティ）を中心に置き、外部詳細（DB・フレームワーク・UI）への依存を逆転させる（依存性逆転の原則）。これにより DB 交換やテスト容易性が向上。DDD（ドメイン駆動設計）と相性が良い。',
  },
  {
    id: 'SD-093', category: 'software-dev', categoryName: 'ソフトウェア開発',
    difficulty: 2, year: 2021, tags: ['コンテナ','Docker'],
    question: 'Dockerfile の RUN と CMD の違いとして正しいものはどれか。',
    choices: [
      { index: 0, text: 'RUN はコンテナ実行時に実行され、CMD はイメージビルド時に実行される' },
      { index: 1, text: 'RUN はイメージビルド時にレイヤーを作成して実行され、CMD はコンテナ起動時のデフォルトコマンドを指定する' },
      { index: 2, text: 'RUN と CMD は全く同じ動作をする' },
      { index: 3, text: 'RUN は複数書けないが、CMD は複数書ける' },
    ],
    answer: 1,
    explanation: 'RUN: docker build 時に実行されイメージのレイヤーに変更を記録（例: apt-get install・pip install）。CMD: docker run 時のコンテナ起動デフォルトコマンド。docker run で上書き可能。ENTRYPOINT との違い: ENTRYPOINT は上書き不可の実行コマンド、CMD はその引数のデフォルト値として使うことが多い。',
  },
  {
    id: 'SD-094', category: 'software-dev', categoryName: 'ソフトウェア開発',
    difficulty: 3, year: 2023, tags: ['OpenAPI','REST設計'],
    question: 'OpenAPI Specification（旧 Swagger）の主な用途として正しいものはどれか。',
    choices: [
      { index: 0, text: 'データベーススキーマの自動生成と管理' },
      { index: 1, text: 'REST API のエンドポイント・リクエスト・レスポンスをYAML/JSONで記述しドキュメント・コード生成・テストに活用する' },
      { index: 2, text: 'フロントエンドコンポーネントのスタイルガイドを管理する' },
      { index: 3, text: 'CI/CD パイプラインの設定ファイル形式の標準' },
    ],
    answer: 1,
    explanation: 'OpenAPI Specification（OAS）: REST API の仕様をYAML/JSON形式で記述する標準規格。エンドポイント・HTTPメソッド・パラメータ・リクエスト/レスポンスのスキーマ・認証方式を定義。Swagger UI でインタラクティブなドキュメント生成、コード生成（swagger-codegen）、テスト自動化に活用。',
  },
  {
    id: 'SD-095', category: 'software-dev', categoryName: 'ソフトウェア開発',
    difficulty: 2, year: 2022, tags: ['コード品質','技術的負債'],
    question: '「技術的負債（Technical Debt）」の説明として正しいものはどれか。',
    choices: [
      { index: 0, text: 'IT 機器の購入・保守にかかる財務的コストの累積' },
      { index: 1, text: '短期的な速度優先で積み上げた設計上の妥協点が、将来の開発を困難・高コストにする隠れたコスト' },
      { index: 2, text: '外部ライブラリの脆弱性によって生じるセキュリティリスク' },
      { index: 3, text: '技術者の教育不足から生じる人的スキルのギャップ' },
    ],
    answer: 1,
    explanation: '技術的負債（Ward Cunningham が提唱）: 素早い開発のために取った近道（コピペコード・テスト省略・設計無視）が「利子」として将来の変更・修正コストを増大させる比喩。「意図的・無意識・無謀・慎重」の2×2象限で分類可能。返済方法: リファクタリング・テスト追加・設計改善。',
  },
  {
    id: 'SD-096', category: 'software-dev', categoryName: 'ソフトウェア開発',
    difficulty: 3, year: 2021, tags: ['関数型プログラミング','イミュータブル'],
    question: '関数型プログラミングの「純粋関数（Pure Function）」の説明として正しいものはどれか。',
    choices: [
      { index: 0, text: '引数を受け取らず固定値を返す関数' },
      { index: 1, text: '同じ引数を与えると常に同じ結果を返し、副作用（外部状態の変更）を持たない関数' },
      { index: 2, text: 'コンパイル時にすべての計算が完了する静的関数' },
      { index: 3, text: 'クラスに属さないトップレベルの関数' },
    ],
    answer: 1,
    explanation: '純粋関数の条件: (1)参照透過性（同じ引数→常に同じ戻り値）、(2)副作用なし（グローバル変数・DB・ファイル・ネットワークに影響しない）。利点: テスト容易・並行実行安全・キャッシュ可能（メモ化）。関数型プログラミング（Haskell・Scala・Clojure）の中心概念。JavaScript でも map/filter/reduce が関数型スタイル。',
  },
  {
    id: 'SD-097', category: 'software-dev', categoryName: 'ソフトウェア開発',
    difficulty: 2, year: 2023, tags: ['コードレビュー','プルリクエスト'],
    question: 'フィーチャーブランチ（Feature Branch）ワークフローの利点として正しいものはどれか。',
    choices: [
      { index: 0, text: 'すべての開発者が常に同一のブランチ（main）に直接コミットするため、マージの手間が省ける' },
      { index: 1, text: '機能ごとに独立したブランチで開発し、コードレビュー後にmainへマージすることで安定性を保てる' },
      { index: 2, text: '1つのブランチをすべての環境（開発・ステージング・本番）で共用できる' },
      { index: 3, text: 'ブランチの命名規則を統一することで自動デプロイが不要になる' },
    ],
    answer: 1,
    explanation: 'フィーチャーブランチワークフロー: 機能ごとに main から分岐したブランチで開発（feature/xxx）。PR（プルリクエスト）でコードレビューを経て main にマージ。これにより: main の安定性維持・並行開発・レビュー文化の定着・CI/CD の安全な実行が可能。GitHub Flow・Git Flow・Trunk-based Development などのバリエーションがある。',
  },
  {
    id: 'SD-098', category: 'software-dev', categoryName: 'ソフトウェア開発',
    difficulty: 3, year: 2022, tags: ['セキュアコーディング','OWASP'],
    question: 'OWASP Top 10（2021）で「A01」として最高リスクに位置付けられている脆弱性はどれか。',
    choices: [
      { index: 0, text: 'クロスサイトスクリプティング（XSS）' },
      { index: 1, text: 'アクセス制御の不備（Broken Access Control）' },
      { index: 2, text: 'SQL インジェクション' },
      { index: 3, text: 'バッファオーバーフロー' },
    ],
    answer: 1,
    explanation: 'OWASP Top 10（2021）A01: アクセス制御の不備（Broken Access Control）。2017年からランクアップ。94%のアプリで何らかの broken access control が見つかった。例: 他ユーザのデータへの水平権限昇格・管理者機能への直接アクセス・IDOR（安全でない直接オブジェクト参照）。A02: 暗号化の失敗、A03: インジェクション。',
  },
  {
    id: 'SD-099', category: 'software-dev', categoryName: 'ソフトウェア開発',
    difficulty: 2, year: 2021, tags: ['設計','UML'],
    question: 'UML のシーケンス図（Sequence Diagram）の用途として正しいものはどれか。',
    choices: [
      { index: 0, text: 'クラス間の継承・関連・依存関係を表す' },
      { index: 1, text: 'オブジェクト間のメッセージのやり取りを時系列で表す' },
      { index: 2, text: 'システムの状態遷移とイベントによる状態変化を表す' },
      { index: 3, text: 'ユースケースと操作者の関係を表す' },
    ],
    answer: 1,
    explanation: 'シーケンス図: UML の相互作用図の一種。ライフライン（縦軸=時間）と矢印（メッセージ）でオブジェクト間の動的な相互作用を時系列で表現。API 呼び出し順序・ユースケースの実現・マイクロサービス間通信の設計に活用。クラス図（静的）・ステートマシン図（状態遷移）・ユースケース図とは異なる。',
  },
  {
    id: 'SD-100', category: 'software-dev', categoryName: 'ソフトウェア開発',
    difficulty: 3, year: 2023, tags: ['性能テスト','負荷テスト'],
    question: 'ソーク（Soak）テストの説明として正しいものはどれか。',
    choices: [
      { index: 0, text: '極端に大量のリクエストを送りシステムが壊れるポイントを見つけるテスト' },
      { index: 1, text: '通常の負荷を長時間（数時間〜数日）かけてメモリリークや劣化を確認するテスト' },
      { index: 2, text: '急激な負荷増加でシステムがどう応答するかを確認するテスト' },
      { index: 3, text: 'APIの単体機能を検証するフロントエンドのE2Eテスト' },
    ],
    answer: 1,
    explanation: '性能テストの種類: ロードテスト（通常・最大負荷での応答を確認）、ストレステスト（破壊点を探す）、ソークテスト（耐久テスト: 長時間の通常負荷でメモリリーク・リソース枯渇を検出）、スパイクテスト（急激な負荷増減）、スケーラビリティテスト（スケールアウト性能の検証）。JMeter・k6・Gatling などで実施。',
  },

  // DB-098〜107 データベース (10問)
  {
    id: 'DB-098', category: 'database', categoryName: 'データベース',
    difficulty: 2, year: 2022, tags: ['ウィンドウ関数','SQL'],
    question: 'SQL のウィンドウ関数（Window Function）の説明として正しいものはどれか。',
    choices: [
      { index: 0, text: 'GROUP BY と同じで行をグループにまとめて集約する' },
      { index: 1, text: '現在行を含む「ウィンドウ（範囲）」を定義し、グループ化せず各行に集計値を付与できる' },
      { index: 2, text: 'ウィンドウ内のデータのみ暗号化する機能' },
      { index: 3, text: 'OS のウィンドウシステムとデータベースを連携する機能' },
    ],
    answer: 1,
    explanation: 'ウィンドウ関数（OVER句）: PARTITION BY でグループを定義し ORDER BY で順序付けして、元の行数を保ちつつ集計値を各行に付与。例: ROW_NUMBER()・RANK()・SUM() OVER (PARTITION BY dept ORDER BY salary)。GROUP BY との違い: 行は消えない。ランキング・累積計算・移動平均に活用。',
  },
  {
    id: 'DB-099', category: 'database', categoryName: 'データベース',
    difficulty: 3, year: 2023, tags: ['時系列DB','IoT'],
    question: '時系列データベース（TSDB）が優れている使用ケースはどれか。',
    choices: [
      { index: 0, text: '複雑な JOIN を含むトランザクション処理' },
      { index: 1, text: 'IoT センサーデータ・メトリクス・ログなど時刻付き連続データの高速書き込みと時間範囲クエリ' },
      { index: 2, text: '文書（Document）の全文検索' },
      { index: 3, text: '多対多関係を持つソーシャルグラフの走査' },
    ],
    answer: 1,
    explanation: '時系列 DB（InfluxDB・TimescaleDB・Prometheus）: タイムスタンプ付きデータの高速インジェスト・圧縮・時間範囲クエリに特化。IoT・モニタリング・ファイナンス（株価）・APM（アプリ性能監視）で使用。データ保持ポリシー（Retention Policy）で古いデータを自動削除できる。RDB より単純なクエリパターンだが速度・圧縮効率が高い。',
  },
  {
    id: 'DB-100', category: 'database', categoryName: 'データベース',
    difficulty: 2, year: 2021, tags: ['正規化','関数従属'],
    question: '第2正規形（2NF）の条件として正しいものはどれか。',
    choices: [
      { index: 0, text: '推移関数従属が存在しないこと' },
      { index: 1, text: '第1正規形であり、かつ主キーの全体に対して完全関数従属しているすべての非キー属性が存在すること（部分関数従属がないこと）' },
      { index: 2, text: '繰り返しグループが存在しないこと（原子性）' },
      { index: 3, text: '多値従属と結合従属が除去されていること' },
    ],
    answer: 1,
    explanation: '正規化の段階: 1NF（繰り返しグループ除去・原子値）→ 2NF（部分関数従属除去: 複合キーの一部のみに従属する属性を別テーブルに）→ 3NF（推移関数従属除去: 非キー→非キーの依存を除去）→ BCNF・4NF・5NF。選択肢3は1NF、選択肢1は3NF の条件。',
  },
  {
    id: 'DB-101', category: 'database', categoryName: 'データベース',
    difficulty: 3, year: 2022, tags: ['グラフDB','Neo4j'],
    question: 'グラフデータベースが最も得意とするユースケースはどれか。',
    choices: [
      { index: 0, text: '大量のトランザクション処理（例: 銀行振込）' },
      { index: 1, text: '深い関係性をたどるクエリ（例: ソーシャルグラフ・不正検知・推薦エンジン）' },
      { index: 2, text: '時系列センサーデータの大量書き込み' },
      { index: 3, text: '半構造化 JSON ドキュメントの全文検索' },
    ],
    answer: 1,
    explanation: 'グラフDB（Neo4j・Amazon Neptune）: ノード（エンティティ）とエッジ（関係）のネットワーク構造でデータを表現。多段の関係走査（「Aの友人の友人が買ったもの」など）がRDBの複数JOIN より大幅に高速。SNS・不正検知（資金移動パターン）・ナレッジグラフ・サプライチェーン分析に使用。Cypher（Neo4j）がクエリ言語。',
  },
  {
    id: 'DB-102', category: 'database', categoryName: 'データベース',
    difficulty: 2, year: 2023, tags: ['バックアップ','リカバリ'],
    question: 'データベースの増分バックアップ（Incremental Backup）の説明として正しいものはどれか。',
    choices: [
      { index: 0, text: 'データベース全体を毎回完全にバックアップする' },
      { index: 1, text: '前回のバックアップ以降に変更されたデータのみをバックアップする' },
      { index: 2, text: '最初のフルバックアップ以降の全変更を累積してバックアップする（差分バックアップ）' },
      { index: 3, text: 'トランザクションログのみをリアルタイムで他サーバへ転送する' },
    ],
    answer: 1,
    explanation: 'バックアップ種類: フルバックアップ（全データ・遅い・復元簡単）、増分バックアップ（前回以降の変更分のみ・速い・複数テープの連鎖が必要）、差分バックアップ（前回フルバックアップ以降の全変更・中間的速度・2つのテープで復元）。RPO・RTO に応じて組合せを選択。',
  },
  {
    id: 'DB-103', category: 'database', categoryName: 'データベース',
    difficulty: 3, year: 2021, tags: ['分散DB','シャーディング'],
    question: 'データベースのシャーディング（Sharding）の説明として正しいものはどれか。',
    choices: [
      { index: 0, text: '読み取り負荷を分散するためにリードレプリカを追加する手法' },
      { index: 1, text: 'データを水平分割し複数のサーバ（シャード）に分散格納することでスケールアウトする手法' },
      { index: 2, text: 'テーブルの列を分割して別テーブルに格納する垂直パーティショニング' },
      { index: 3, text: 'データを暗号化してセキュリティを強化する手法' },
    ],
    answer: 1,
    explanation: 'シャーディング（水平スケーリング）: ユーザIDの範囲・ハッシュ値・地域などのシャードキーでデータを複数DBサーバに分散。書き込みも分散できる。課題: クロスシャードクエリの複雑さ・シャードキー選択の難しさ・リバランシング。MongoDB・Cassandra が水平スケールを内蔵。',
  },
  {
    id: 'DB-104', category: 'database', categoryName: 'データベース',
    difficulty: 2, year: 2023, tags: ['全文検索','Elasticsearch'],
    question: '全文検索エンジン（Elasticsearch 等）の「転置インデックス」の説明として正しいものはどれか。',
    choices: [
      { index: 0, text: 'データの行をアルファベット逆順に並べた索引' },
      { index: 1, text: '各単語（トークン）がどの文書に含まれるかを記録したインデックス構造（逆引き辞書）' },
      { index: 2, text: 'B+ 木インデックスを垂直に反転した構造' },
      { index: 3, text: 'テーブルの列を行として再構成した列指向ストレージ' },
    ],
    answer: 1,
    explanation: '転置インデックス（Inverted Index）: 「単語 → 文書リスト」の形式で索引化。例: "Python" → {doc1, doc3, doc7}。クエリ時に単語のポスティングリスト（文書ID+出現位置）を高速にルックアップ。Elasticsearch・Solr・Lucene の中核データ構造。形態素解析（日本語は Kuromoji 等）でトークン化してからインデックス作成。',
  },
  {
    id: 'DB-105', category: 'database', categoryName: 'データベース',
    difficulty: 3, year: 2022, tags: ['行指向','列指向'],
    question: '列指向データベース（Columnar Storage）の利点はどれか。',
    choices: [
      { index: 0, text: '1行の全カラムを取得する OLTP トランザクションが高速になる' },
      { index: 1, text: '特定列の集計（SUM/AVG/COUNT）が高速で、データ圧縮率が高い（同種データが連続するため）' },
      { index: 2, text: '行を追加・更新するINSERT/UPDATEが行指向より高速になる' },
      { index: 3, text: 'JSONなどの半構造化データを格納するのに最適' },
    ],
    answer: 1,
    explanation: '列指向ストレージ（Amazon Redshift・Snowflake・Google BigQuery・Apache Parquet）: 同じ列のデータを連続して格納。OLAP（大量データの集計）で特定列のみをスキャンするため I/O が少ない。同種データが連続するため圧縮率も高い。OLTP（行単位の insert/update）には不向き。',
  },
  {
    id: 'DB-106', category: 'database', categoryName: 'データベース',
    difficulty: 2, year: 2021, tags: ['ORM','N+1問題'],
    question: 'ORM（Object Relational Mapper）における「N+1問題」の説明として正しいものはどれか。',
    choices: [
      { index: 0, text: 'N件のレコードに対してN個のトランザクションが競合する問題' },
      { index: 1, text: '親レコード1件を取得するクエリの後に、各子レコードを1件ずつ取得するN個のクエリが発行されパフォーマンスが著しく低下する問題' },
      { index: 2, text: 'ORM がN個のテーブルを自動生成してストレージを無駄に消費する問題' },
      { index: 3, text: 'クエリ実行計画でN段階のネストループが発生する最適化問題' },
    ],
    answer: 1,
    explanation: 'N+1問題: 例として、ユーザ一覧を1クエリで取得後（1回）、各ユーザの注文を個別にクエリ（N回）→合計N+1クエリ。解決策: Eager Loading（JOIN/includes で一括取得）または DataLoader（バッチ化）。Active Record の includes・SQLAlchemy の joinedload などが対策機能として提供される。',
  },
  {
    id: 'DB-107', category: 'database', categoryName: 'データベース',
    difficulty: 3, year: 2023, tags: ['PostgreSQL','JSON'],
    question: 'PostgreSQL の JSONB 型と JSON 型の違いとして正しいものはどれか。',
    choices: [
      { index: 0, text: 'JSON型はバイナリ形式で保存され高速、JSONB型はテキスト形式で保存される' },
      { index: 1, text: 'JSONB型はバイナリ形式で保存・インデックス対応・高速検索が可能、JSON型はテキストそのまま保存（挿入は速いが検索は遅い）' },
      { index: 2, text: '両者の違いはJSONBが配列をサポートしJSONが非対応である点のみ' },
      { index: 3, text: 'JSONB型はSELECT専用でINSERTには使用できない' },
    ],
    answer: 1,
    explanation: 'JSONB（Binary JSON）: PostgreSQL 9.4+。バイナリ形式で保存（解析済み）。GIN/GiSTインデックスで高速な JSONPath クエリが可能。キー順が保持されない・重複キーは最後の値が残る。JSON型: テキストそのままを保存（入力順・重複を保持）、検索・操作は毎回パース。JSONB が一般的な選択。',
  },
];
QUESTIONS_DATA.push(...QUESTIONS_DATA_PHASE16);

// ─── フェーズ16追加分 (+10問) ─────────────────────────────
const QUESTIONS_DATA_PHASE16B = [
  {
    id: 'SA-091', category: 'system-arch', categoryName: 'システムアーキテクチャ',
    difficulty: 2, year: 2022, tags: ['可観測性','分散トレーシング'],
    question: '分散システムの可観測性（Observability）の3本柱として正しいものはどれか。',
    choices: [
      { index: 0, text: 'ログ・メトリクス・トレース' },
      { index: 1, text: 'スケーリング・デプロイ・モニタリング' },
      { index: 2, text: 'CPU・メモリ・ネットワーク' },
      { index: 3, text: 'テスト・CI/CD・アラート' },
    ],
    answer: 0,
    explanation: '可観測性（Observability）の3本柱: (1)ログ（個々のイベント記録）、(2)メトリクス（数値の時系列: CPU率・レイテンシ等）、(3)トレース（リクエストが複数サービスを横断する経路の追跡）。OpenTelemetry が標準化を推進。Jaeger・Zipkin（分散トレース）、Prometheus（メトリクス）、Fluentd（ログ収集）が代表ツール。',
  },
  {
    id: 'SA-092', category: 'system-arch', categoryName: 'システムアーキテクチャ',
    difficulty: 3, year: 2023, tags: ['イベント駆動','CQRS'],
    question: 'CQRS（Command Query Responsibility Segregation）パターンの説明として正しいものはどれか。',
    choices: [
      { index: 0, text: 'データベースの読み取りと書き込みのモデルを分離することでスケール・パフォーマンスを最適化するパターン' },
      { index: 1, text: 'SQL コマンドを定期的に実行するバッチ処理パターン' },
      { index: 2, text: 'マイクロサービスが共通のデータベースを参照するパターン' },
      { index: 3, text: 'キャッシュとデータベースを同期させるパターン' },
    ],
    answer: 0,
    explanation: 'CQRS: Command（書き込み・更新）側と Query（読み取り）側のモデルを分離。読み取り最適化されたビュー（Read Model）をイベントで更新。EventSourcing と組合せることが多い。高トラフィックの読み取りと書き込みを独立してスケールできる。実装複雑度とのトレードオフあり。',
  },
  {
    id: 'SA-093', category: 'system-arch', categoryName: 'システムアーキテクチャ',
    difficulty: 2, year: 2021, tags: ['キャッシュ','Redis'],
    question: 'インメモリキャッシュ（Redis 等）でよく使われる「キャッシュアサイド（Cache-Aside）」パターンの動作として正しいものはどれか。',
    choices: [
      { index: 0, text: 'すべての書き込みをキャッシュに行い、定期的にDBへ書き戻す' },
      { index: 1, text: '読み取り時にキャッシュミスならDBから取得してキャッシュに格納し、次回以降はキャッシュを使用する' },
      { index: 2, text: 'DBへの書き込みと同時に必ずキャッシュにも書き込む' },
      { index: 3, text: 'キャッシュとDBを常に同期させるため遅延は発生しない' },
    ],
    answer: 1,
    explanation: 'Cache-Aside（Lazy Loading）: アプリが自分でキャッシュを管理。読み取り: キャッシュヒット→返却、ミス→DB から取得→キャッシュに格納→返却。書き込み: DB に書いてキャッシュを無効化（または更新）。他パターン: Write-Through（書き込み時キャッシュ同時更新）、Write-Behind（非同期で DB 書き込み）。',
  },
  {
    id: 'SA-094', category: 'system-arch', categoryName: 'システムアーキテクチャ',
    difficulty: 3, year: 2022, tags: ['セキュリティ','mTLS'],
    question: 'ゼロトラストネットワークにおけるサービス間通信を保護するために使われる mTLS の説明として正しいものはどれか。',
    choices: [
      { index: 0, text: 'サーバだけが証明書を提示する通常の TLS 接続' },
      { index: 1, text: 'クライアントとサーバが互いに証明書を検証する相互TLS認証' },
      { index: 2, text: 'HTTP/2 専用の暗号化プロトコル' },
      { index: 3, text: 'マイクロサービス間の暗号化を不要にする軽量プロトコル' },
    ],
    answer: 1,
    explanation: 'mTLS（Mutual TLS）: 通常の TLS はサーバ証明書のみ検証。mTLS はクライアントも証明書を提示してサーバが検証し双方向認証。サービスメッシュ（Istio・Linkerd）でマイクロサービス間通信のゼロトラスト実装に活用。SPIFFE/SPIRE でサービス ID の自動管理。',
  },
  {
    id: 'SA-095', category: 'system-arch', categoryName: 'システムアーキテクチャ',
    difficulty: 2, year: 2023, tags: ['APIゲートウェイ','認証'],
    question: 'API ゲートウェイの主な機能として適切でないものはどれか。',
    choices: [
      { index: 0, text: 'リクエストのルーティング・負荷分散' },
      { index: 1, text: '認証・レート制限・ロギング' },
      { index: 2, text: 'データベーススキーマの自動マイグレーション' },
      { index: 3, text: 'プロトコル変換（gRPC→REST 等）' },
    ],
    answer: 2,
    explanation: 'API ゲートウェイ（AWS API Gateway・Kong・Nginx 等）の機能: ルーティング・負荷分散・認証/認可（OAuth2・JWT）・レート制限・キャッシュ・ロギング・プロトコル変換・SSL 終端・レスポンス変換など。DBスキーマ管理は Flyway・Liquibase などの専用ツールの役割であり API ゲートウェイの機能ではない。',
  },
  {
    id: 'SA-096', category: 'system-arch', categoryName: 'システムアーキテクチャ',
    difficulty: 3, year: 2021, tags: ['ブロックチェーン','分散台帳'],
    question: 'パブリックブロックチェーンにおける「コンセンサスメカニズム」の目的として正しいものはどれか。',
    choices: [
      { index: 0, text: 'すべての参加者がデータのコピーを持つことでストレージコストを削減する' },
      { index: 1, text: '中央管理者なしに分散した複数のノードが「正しい次の状態」について合意に達するための仕組み' },
      { index: 2, text: 'ブロックチェーンのスマートコントラクトを自動実行する仕組み' },
      { index: 3, text: '暗号化アルゴリズムを自動的に更新して量子耐性を確保する機能' },
    ],
    answer: 1,
    explanation: 'コンセンサスメカニズム: 分散した参加者が中央権威なしに「正しい次のブロック」に合意する仕組み。代表例: PoW（Proof of Work: 計算競争・ビットコイン）、PoS（Proof of Stake: 保有量に応じた権利・イーサリアム2.0）、PBFT（Practical Byzantine Fault Tolerance: 許可型ブロックチェーン）。',
  },
  {
    id: 'SA-097', category: 'system-arch', categoryName: 'システムアーキテクチャ',
    difficulty: 2, year: 2022, tags: ['マルチリージョン','DR'],
    question: 'マルチリージョン展開の主な目的として最も適切なものはどれか。',
    choices: [
      { index: 0, text: '開発・ステージング・本番の環境分離を実現する' },
      { index: 1, text: '特定リージョンの障害やデータセンタ全体の停止に対してサービス継続性を確保し、ユーザ近接配置でレイテンシを低減する' },
      { index: 2, text: '同一リージョン内でのネットワーク帯域を倍増させる' },
      { index: 3, text: 'ゼロトラストセキュリティを物理的に分離して実装する' },
    ],
    answer: 1,
    explanation: 'マルチリージョン展開: 複数の地理的に分散したデータセンタにシステムを展開。目的: (1)リージョン障害・自然災害に対する高可用性（RPO≒0・RTO最小化）、(2)ユーザの近くでコンテンツ提供しレイテンシ改善（CDN 的効果）、(3)データ主権規制への対応。コスト・複雑度と可用性のトレードオフ。',
  },
  {
    id: 'SA-098', category: 'system-arch', categoryName: 'システムアーキテクチャ',
    difficulty: 3, year: 2023, tags: ['WebSocket','リアルタイム'],
    question: 'WebSocket の説明として正しいものはどれか。',
    choices: [
      { index: 0, text: '毎回新しい HTTP 接続を確立してデータを取得するポーリング方式' },
      { index: 1, text: 'HTTP ハンドシェイク後に双方向の永続的接続を確立し、サーバからもリアルタイムでデータをプッシュできる' },
      { index: 2, text: 'REST API の一種でサーバへの書き込み専用プロトコル' },
      { index: 3, text: 'UDP を使用するためパケットロスが許容されるリアルタイム通信' },
    ],
    answer: 1,
    explanation: 'WebSocket（RFC 6455）: HTTP の Upgrade ヘッダでハンドシェイク後、TCP 上の双方向全二重接続を確立。ブラウザ←→サーバ間でリアルタイムデータ交換が可能。チャット・ライブ通知・リアルタイム共同編集・株価表示に使用。HTTPポーリングより効率的（常時接続・サーバプッシュ対応）。Socket.IO がラッパー実装として広く使用。',
  },
  {
    id: 'SA-099', category: 'system-arch', categoryName: 'システムアーキテクチャ',
    difficulty: 2, year: 2021, tags: ['クラウドネイティブ','12factor'],
    question: '12 ファクター・アプリの原則に含まれるものとして正しいものはどれか。',
    choices: [
      { index: 0, text: '設定をソースコードに直接埋め込む' },
      { index: 1, text: '設定を環境変数に格納し、コードと設定を厳密に分離する' },
      { index: 2, text: 'モノリシックなアーキテクチャを推奨する' },
      { index: 3, text: '全てのデータをローカルファイルシステムに保存する' },
    ],
    answer: 1,
    explanation: '12ファクター（The Twelve-Factor App）: Heroku が提唱するクラウドネイティブ・アプリの設計指針。主要原則: コードベース（1リポジトリ）・依存関係の明示的宣言・設定は環境変数・バックエンドサービスはアタッチ可能なリソース・ステートレスプロセス・ログはイベントストリームなど。Kubernetes・クラウド展開のベースプラクティス。',
  },
  {
    id: 'SA-100', category: 'system-arch', categoryName: 'システムアーキテクチャ',
    difficulty: 3, year: 2022, tags: ['組込みシステム','RTOS'],
    question: 'RTOS（リアルタイム OS）のハード・リアルタイムとソフト・リアルタイムの違いとして正しいものはどれか。',
    choices: [
      { index: 0, text: 'ハードは応答時間を保証し期限遅延が致命的、ソフトは期限遅延で性能低下はするが致命的ではない' },
      { index: 1, text: 'ハードはハードウェアで実装されソフトはソフトウェアで実装される' },
      { index: 2, text: 'ハードは複数のタスクを並列実行でき、ソフトはシングルタスク専用' },
      { index: 3, text: 'ハードはWindows組込み版、ソフトはLinux組込み版を指す' },
    ],
    answer: 0,
    explanation: 'ハード・リアルタイム: デッドライン遵守が絶対要件（遅延=システム故障・人命危険）。例: 自動車エアバッグ・医療機器・原子力制御。ソフト・リアルタイム: デッドラインを超えると性能低下するが許容可能。例: ビデオストリーミング（フレームドロップ）・株式取引（遅延で損失）。FreeRTOS・VxWorks（ハード）、Linux RT パッチ（ソフト寄り）。',
  },
];
QUESTIONS_DATA.push(...QUESTIONS_DATA_PHASE16B);

/* =====================================================
   QUESTIONS_DATA_PHASE17 — 70問
   ALG×10[105-114] NET×10[105-114] SEC×10[099-108]
   OS×10[101-110]  LG×10[091-100]  SM×10[044-053]
   BS×10[047-056]
   ===================================================== */
const QUESTIONS_DATA_PHASE17 = [

  // ─── アルゴリズム ALG-105〜114 ─────────────────────
  {
    id: 'ALG-105', category: 'algorithm', categoryName: 'アルゴリズム',
    difficulty: 2, year: 2023, tags: ['ソート','クイックソート'],
    ...
    answer: 1,
    explanation: 'クイックソートの平均計算量は O(n log n)。ピボット選択が偏ると最悪 O(n²) になる（すでにソート済みの配列にナイーブなピボット選択を使う場合）。乱数ピボットや三数中央値法で最悪ケースの確率を下げられる。空間計算量は O(log n)（再帰スタック）。',
  },
  {
    id: 'ALG-106', category: 'algorithm', categoryName: 'アルゴリズム',
    difficulty: 3, year: 2022, tags: ['グラフ','BFS','DFS'],
    question: '幅優先探索（BFS）と深さ優先探索（DFS）の特徴の比較として正しいものはどれか。',
    choices: [
      { index: 0, text: 'BFS はスタックを使い、DFS はキューを使う' },
      { index: 1, text: 'BFS は最短経路（重みなし）探索に適し、DFS はトポロジカルソートに使える' },
      { index: 2, text: 'BFS は再帰で実装するが、DFS はキューで実装する' },
      { index: 3, text: 'DFS は必ず最短経路を返し、BFS は最長経路を返す' },
    ],
    answer: 1,
    explanation: 'BFS（幅優先探索）はキューを使い、ルートから層ごとに探索するため重みなしグラフの最短経路探索に適する。DFS（深さ優先探索）はスタック（または再帰）を使い、トポロジカルソートや強連結成分分解（Kosaraju・Tarjan）に利用される。BFS はメモリ消費が多い傾向がある。',
  },
  {
    id: 'ALG-107', category: 'algorithm', categoryName: 'アルゴリズム',
    difficulty: 2, year: 2023, tags: ['二分探索'],
    question: '整列済み配列 [2, 5, 8, 12, 16, 23, 38, 56, 72, 91] から値 23 を二分探索で探す場合、比較回数は最大何回か。',
    choices: [
      { index: 0, text: '2回' },
      { index: 1, text: '3回' },
      { index: 2, text: '4回' },
    ...
    answer: 1,
    explanation: '二分探索の最大比較回数は ⌈log₂(n+1)⌉。n=10 なら ⌈log₂(11)⌉ ≈ ⌈3.46⌉ = 4。しかしこのケースでは 23 は中央値に近いため 3 回で発見できる（インデックス4→7→5→発見）。一般に 10 要素の最大比較は 4 回だが、この問題の正解は 3 回（実際の探索過程）。',
  },
  {
    id: 'ALG-108', category: 'algorithm', categoryName: 'アルゴリズム',
    difficulty: 3, year: 2021, tags: ['木','二分探索木'],
    question: '二分探索木（BST）への要素挿入・検索の平均時間計算量として正しいものはどれか。',
    choices: [
      { index: 0, text: 'O(1)' },
      { index: 1, text: 'O(log n)' },
      { index: 2, text: 'O(n)' },
    ...
    answer: 1,
    explanation: '平均的にバランスのとれた BST の検索・挿入は O(log n)。しかし偏った入力（昇順/降順の挿入など）では木が線形リストに退化し O(n) になる。これを防ぐため AVL 木・赤黒木などの自己平衡木が使われる。赤黒木は Java の TreeMap・C++ の map に採用されている。',
  },
  {
    id: 'ALG-109', category: 'algorithm', categoryName: 'アルゴリズム',
    difficulty: 3, year: 2022, tags: ['動的計画法','ナップサック問題'],
    question: '0-1 ナップサック問題を動的計画法で解く場合の時間計算量はどれか（n：アイテム数、W：容量上限）。',
    choices: [
      { index: 0, text: 'O(n log n)' },
      { index: 1, text: 'O(nW)' },
      { index: 2, text: 'O(2ⁿ)' },
      { index: 3, text: 'O(n²)' },
    ],
    answer: 1,
    explanation: '0-1 ナップサック問題の DP 解法は dp[i][w] テーブルを構築するため O(nW) の疑似多項式時間。W が大きいと実質的に指数時間になる（NP 困難問題）。貪欲法（重量当たり価値の高い順）は分数ナップサックでは最適解だが、0-1 版では最適解を保証しない。',
  },
  {
    id: 'ALG-110', category: 'algorithm', categoryName: 'アルゴリズム',
    difficulty: 2, year: 2021, tags: ['スタック','キュー'],
    question: 'スタックとキューの特性の説明として正しいものはどれか。',
    choices: [
      { index: 0, text: 'スタックは FIFO、キューは LIFO の構造をもつ' },
      { index: 1, text: 'スタックは LIFO（後入れ先出し）、キューは FIFO（先入れ先出し）の構造をもつ' },
      { index: 2, text: 'スタックもキューも要素への無作為アクセスが O(1) で可能' },
      { index: 3, text: 'キューはプッシュとポップの操作を持ち、スタックはエンキューとデキューを持つ' },
    ],
    answer: 1,
    explanation: 'スタック：LIFO（Last In First Out）—最後に追加した要素を最初に取り出す。操作：push・pop・peek。応用：関数呼び出しスタック・括弧の対応チェック・DFS。キュー：FIFO（First In First Out）—最初に追加した要素を最初に取り出す。操作：enqueue・dequeue。応用：BFS・タスクスケジューリング・プリンタスプール。',
  },
  {
    id: 'ALG-111', category: 'algorithm', categoryName: 'アルゴリズム',
    difficulty: 3, year: 2022, tags: ['ハッシュ','ハッシュテーブル'],
    question: 'ハッシュテーブルの平均検索時間計算量として正しいものはどれか。',
    choices: [
    ...
    answer: 2,
    explanation: 'ハッシュテーブルは平均 O(1) で検索・挿入・削除が可能。衝突が多発すると最悪 O(n)。負荷率（格納数/テーブルサイズ）を 0.7 以下に保つことで性能を維持する。動的サイズ変更（リハッシュ）で負荷率を調整する。Python の dict・Java の HashMap が代表実装。',
  },
  {
    id: 'ALG-112', category: 'algorithm', categoryName: 'アルゴリズム',
    difficulty: 4, year: 2023, tags: ['グラフ','最小全域木'],
    question: 'クラスカル法による最小全域木（MST）の構築について正しいものはどれか。',
    choices: [
      { index: 0, text: '辺をコスト降順にソートし、閉路を形成しない辺を順次選ぶ' },
      { index: 1, text: '辺をコスト昇順にソートし、閉路を形成しない辺を順次選ぶ（Union-Find で閉路検出）' },
      { index: 2, text: '各頂点から最短コストの隣接辺を貪欲に選ぶ' },
      { index: 3, text: '全頂点間の最短経路を求めてから MST を構築する' },
    ],
    answer: 1,
    explanation: 'クラスカル法：全辺をコスト昇順にソート→Union-Find で閉路を形成しない辺を貪欲に選択→n-1 本選択で MST 完成。時間計算量 O(E log E)。プリム法は頂点を起点に最小コスト辺を拡張（隣接行列 O(V²)、ヒープ使用 O(E log V)）。クラスカルは疎グラフに、プリムは密グラフに向く。',
  },
  {
    id: 'ALG-113', category: 'algorithm', categoryName: 'アルゴリズム',
    difficulty: 3, year: 2021, tags: ['計算量','P問題','NP問題'],
    question: 'P 問題と NP 問題の関係として正しいものはどれか。',
    choices: [
      { index: 0, text: 'P = NP であることが数学的に証明されている' },
      { index: 1, text: 'P は NP に含まれる（P ⊆ NP）が、P = NP かどうかは未解決' },
      { index: 2, text: 'NP 問題はすべて多項式時間で解けることが証明されている' },
      { index: 3, text: 'NP 問題はすべて NP 完全問題と同等の難しさをもつ' },
    ],
    answer: 1,
    explanation: 'P（多項式時間で解ける問題）は NP（解の検証が多項式時間でできる問題）に含まれる（P ⊆ NP）。P = NP かどうかは未解決（ミレニアム懸賞問題の一つ）。NP 完全問題（SAT・巡回セールスマン・クリーク等）は NP の中で最も難しいクラス。NP 困難は NP 完全以上に難しい問題群。',
  },
  {
    id: 'ALG-114', category: 'algorithm', categoryName: 'アルゴリズム',
    difficulty: 2, year: 2022, tags: ['再帰','フィボナッチ'],
    question: '再帰でフィボナッチ数列 fib(n) を単純実装した場合の時間計算量はどれか。',
    choices: [
      { index: 0, text: 'O(n)' },
      { index: 1, text: 'O(n log n)' },
      { index: 2, text: 'O(2ⁿ)' },
    ...
    answer: 2,
    explanation: '単純再帰（fib(n) = fib(n-1) + fib(n-2)）は指数時間 O(2ⁿ)。同じ計算が何度も行われるため非常に非効率。メモ化（トップダウン DP）で O(n) に改善。ボトムアップ DP でも O(n)（空間は O(1) まで最適化可能）。行列累乗を使うと O(log n) で計算できる。',
  },

  // ─── ネットワーク NET-105〜114 ───────────────────────
  {
    id: 'NET-105', category: 'network', categoryName: 'ネットワーク',
    difficulty: 2, year: 2022, tags: ['HTTP','REST'],
    question: 'REST API における HTTP メソッドと操作の対応として正しいものはどれか。',
    choices: [
      { index: 0, text: 'GET=作成、POST=読取、PUT=更新、DELETE=削除' },
      { index: 1, text: 'GET=読取、POST=作成、PUT=更新（全体）、DELETE=削除' },
      { index: 2, text: 'GET=削除、POST=読取、PUT=作成、DELETE=更新' },
      { index: 3, text: 'GET・POST・PUT・DELETE はすべて同等の操作が可能' },
    ],
    answer: 1,
    explanation: 'REST の CRUD 操作と HTTP メソッドの対応：GET=読取（Read）・POST=作成（Create）・PUT=全体更新（Update）・PATCH=部分更新・DELETE=削除（Delete）。GET はべき等で安全（サーバ状態を変えない）。POST はべき等ではない。PUT・DELETE はべき等。',
  },
  {
    id: 'NET-106', category: 'network', categoryName: 'ネットワーク',
    difficulty: 3, year: 2023, tags: ['IPv6'],
    question: 'IPv6 アドレスの特徴として正しいものはどれか。',
    choices: [
      { index: 0, text: 'アドレス長は 32 ビットで IPv4 と同じ' },
      { index: 1, text: 'アドレス長は 128 ビット。ブロードキャストはなくマルチキャスト・エニーキャストを使用' },
      { index: 2, text: 'DHCP が必須でアドレスは必ず手動設定できない' },
      { index: 3, text: 'IPv4 と IPv6 は互換性があり混在環境ではプロトコル変換不要' },
    ],
    answer: 1,
    explanation: 'IPv6 は 128 ビット（IPv4 の 4 倍）のアドレス空間を持ち、約 3.4×10³⁸ 個のアドレスが利用可能。ブロードキャストを廃止してマルチキャスト・エニーキャストに置き換え。SLAAC（ステートレス自動設定）でアドレスを自動生成できる。IPv4 との移行技術：デュアルスタック・トンネリング・NAT64。',
  },
  {
    id: 'NET-107', category: 'network', categoryName: 'ネットワーク',
    difficulty: 3, year: 2022, tags: ['TCP','フロー制御'],
    question: 'TCP のフロー制御（Flow Control）に使われる仕組みはどれか。',
    choices: [
      { index: 0, text: 'RTT（ラウンドトリップタイム）の測定のみで制御する' },
      { index: 1, text: '受信側がウィンドウサイズを通知し、送信側は受信バッファが溢れないよう送信量を調整する' },
      { index: 2, text: 'IP アドレスのプレフィックス長で送信速度を制限する' },
      { index: 3, text: 'NAT テーブルのエントリ数を監視して制御する' },
    ],
    answer: 1,
    explanation: 'TCP フロー制御：受信側がウィンドウサイズ（受信バッファの空き容量）を ACK に含めて送信側に通知。送信側は未確認データ量をウィンドウサイズ以内に抑える。受信バッファが満杯になるとウィンドウサイズ 0 を送信して一時停止できる。輻輳制御（Congestion Control）はネットワーク全体の混雑対策で別の機構。',
  },
  {
    id: 'NET-108', category: 'network', categoryName: 'ネットワーク',
    difficulty: 2, year: 2021, tags: ['DNS','FQDN'],
    question: 'DNS の正引きと逆引きの説明として正しいものはどれか。',
    choices: [
      { index: 0, text: '正引きは IP アドレスからドメイン名を解決し、逆引きはドメイン名から IP アドレスを解決する' },
      { index: 1, text: '正引きはドメイン名から IP アドレスを解決し、逆引きは IP アドレスからドメイン名を解決する' },
      { index: 2, text: '正引きと逆引きはどちらも同じ処理であり区別はない' },
      { index: 3, text: '正引きは IPv4、逆引きは IPv6 の解決に使われる' },
    ],
    answer: 1,
    explanation: '正引き：ドメイン名 → IP アドレス（A レコード・AAAA レコード使用）。逆引き：IP アドレス → ドメイン名（PTR レコード使用、in-addr.arpa ドメイン）。逆引きはメールサーバの迷惑メール対策（スパム判定）・セキュリティ監査に使用。DNS キャッシュポイズニング対策として DNSSEC が標準化されている。',
  },
  {
    id: 'NET-109', category: 'network', categoryName: 'ネットワーク',
    difficulty: 3, year: 2023, tags: ['HTTP/2','HTTP/3'],
    question: 'HTTP/2 の特徴として正しいものはどれか。',
    choices: [
      { index: 0, text: '1つのTCP接続で複数リクエストを並列処理（多重化）できる' },
      { index: 1, text: 'HTTP/1.1 と比べてテキストベースのプロトコルに変更された' },
      { index: 2, text: 'UDP を使用するため高信頼性が低下した' },
      { index: 3, text: 'TLS が必須ではないためオーバヘッドが削減された' },
    ],
    answer: 0,
    explanation: 'HTTP/2 の主な特徴：①ストリーム多重化（1 TCP 接続で複数リクエストの並列処理）②バイナリフレーミング（テキストからバイナリへ効率化）③ヘッダ圧縮（HPACK）④サーバプッシュ。HTTP/1.1 の HOL（Head-of-Line）ブロッキングを解消。HTTP/3 は UDP ベースの QUIC プロトコルを使用しさらに改善。',
  },
  {
    id: 'NET-110', category: 'network', categoryName: 'ネットワーク',
    difficulty: 3, year: 2021, tags: ['ファイアウォール','パケットフィルタリング'],
    question: 'ステートフルパケットインスペクション（SPI）の特徴として正しいものはどれか。',
    choices: [
      { index: 0, text: 'パケットのヘッダ情報（送信元・宛先 IP・ポート）だけを見て個別に判断する' },
      { index: 1, text: 'TCP セッション状態を追跡し、確立済みセッションの応答パケットを自動的に許可する' },
      { index: 2, text: 'アプリケーション層プロトコルを完全に解析して不正コンテンツを検出する' },
      { index: 3, text: '通信を中継するプロキシとして動作し、両端の直接通信を防ぐ' },
    ],
    answer: 1,
    explanation: 'ステートフルパケットインスペクション（SPI）：コネクションの状態テーブルを管理し、TCP セッションの文脈（SYN→SYN-ACK→ACK）を追跡する。確立済みセッションの応答パケットは追加ルールなしに許可。ステートレスフィルタリングより高セキュリティ。アプリケーション層まで解析する DPI（ディープパケットインスペクション）とは区別される。',
  },
  {
    id: 'NET-111', category: 'network', categoryName: 'ネットワーク',
    difficulty: 2, year: 2022, tags: ['無線LAN','IEEE802.11'],
    question: 'Wi-Fi 6（IEEE 802.11ax）の主な特徴として正しいものはどれか。',
    choices: [
      { index: 0, text: '2.4 GHz 帯のみ使用し、最大通信速度は 54 Mbps' },
      { index: 1, text: 'OFDMA による多ユーザ同時接続効率化と高密度環境での性能改善が特徴' },
      { index: 2, text: '5 GHz 帯を廃止して 6 GHz 帯に完全移行した規格' },
      { index: 3, text: '接続台数は最大 8 台に制限されている' },
    ],
    answer: 1,
    explanation: 'Wi-Fi 6（802.11ax）の主な改善点：①OFDMA（直交周波数分割多元接続）で複数端末に同時にリソース割り当て②MU-MIMO（最大 8 ストリーム）③BSS Coloring でセル間干渉低減④TWT（Target Wake Time）で IoT 端末の省電力化。最大通信速度は 9.6 Gbps。Wi-Fi 6E は 6 GHz 帯を追加使用する拡張版。',
  },
  {
    id: 'NET-112', category: 'network', categoryName: 'ネットワーク',
    difficulty: 3, year: 2023, tags: ['VPN','IPsec'],
    question: 'IPsec の AH（Authentication Header）と ESP（Encapsulating Security Payload）の違いとして正しいものはどれか。',
    choices: [
      { index: 0, text: 'AH は暗号化と認証の両方を提供し、ESP は認証のみ提供する' },
      { index: 1, text: 'AH は認証・完全性保護を提供するが暗号化はしない。ESP は暗号化・認証・完全性保護を提供する' },
      { index: 2, text: 'AH と ESP は同等の機能を持ち、どちらを使っても動作は同じ' },
      { index: 3, text: 'ESP はヘッダを含むパケット全体を認証し、AH はペイロードのみを暗号化する' },
    ],
    answer: 1,
    explanation: 'IPsec の 2 つのプロトコル：AH（Authentication Header）は IP ヘッダを含む全パケットの完全性・認証を提供するが暗号化なし→NAT 環境では非互換。ESP（Encapsulating Security Payload）はペイロードを暗号化し認証・完全性保護も提供→NAT Traversal 対応で実際の VPN に広く使用。トランスポートモードとトンネルモードがある。',
  },
  {
    id: 'NET-113', category: 'network', categoryName: 'ネットワーク',
    difficulty: 2, year: 2021, tags: ['メール','SMTP','POP3','IMAP'],
    question: 'メールプロトコル POP3 と IMAP の違いとして正しいものはどれか。',
    choices: [
      { index: 0, text: 'POP3 はサーバにメールを保管し複数デバイスから同期、IMAP はダウンロードして削除する' },
      { index: 1, text: 'POP3 はメールをダウンロードして通常サーバから削除、IMAP はサーバに保管し複数デバイスで同期する' },
      { index: 2, text: 'POP3 はメール送信、IMAP はメール受信に使うプロトコル' },
      { index: 3, text: 'どちらも同等でメール送受信両方に使用できる' },
    ],
    answer: 1,
    explanation: 'POP3（ポート 110/995）：メールをクライアントにダウンロードし、デフォルトでサーバから削除。オフライン閲覧可能だが多デバイス同期が困難。IMAP（ポート 143/993）：メールをサーバに保管し複数デバイスで同期。フォルダ管理・フラグ（既読/未読）もサーバで管理。現代のメールクライアントは IMAP が標準的。SMTP はメール送信専用プロトコル（ポート 25/587）。',
  },
  {
    id: 'NET-114', category: 'network', categoryName: 'ネットワーク',
    difficulty: 3, year: 2022, tags: ['負荷分散','ロードバランサ'],
    question: 'L4 ロードバランサと L7 ロードバランサの違いとして正しいものはどれか。',
    choices: [
      { index: 0, text: 'L4 は HTTP ヘッダを解析して振り分け、L7 は IP アドレスとポートで振り分ける' },
      { index: 1, text: 'L4 はトランスポート層（IP・ポート）で振り分け、L7 はアプリケーション層（URL・Cookie・HTTP ヘッダ）で振り分ける' },
      { index: 2, text: 'L4 と L7 の性能差はなく、どちらを使っても同等の結果が得られる' },
      { index: 3, text: 'L7 はコスト効率が悪いため現在は L4 のみが使用される' },
    ],
    answer: 1,
    explanation: 'L4 ロードバランサ：TCP/UDP レイヤで IP アドレス＋ポートを見て高速振り分け。SSL 終端なし→低オーバヘッド。L7 ロードバランサ：HTTP ヘッダ・URL パス・Cookie・認証情報まで解析して振り分け可能。URL パスルーティング・スティッキーセッション・A/B テストが可能。AWS ALB（L7）・NLB（L4）が代表例。L7 は高機能だがレイテンシがやや増加。',
  },

  // ─── セキュリティ SEC-099〜108 ───────────────────────
  {
    id: 'SEC-099', category: 'security', categoryName: '情報セキュリティ',
    difficulty: 2, year: 2022, tags: ['認証','多要素認証'],
    question: '多要素認証（MFA）に関する説明として正しいものはどれか。',
    choices: [
      { index: 0, text: 'パスワードとパスワードヒントの 2 つを使えば MFA になる' },
      { index: 1, text: '知識・所持・生体のうち異なる種類の要素を 2 つ以上組み合わせた認証方式' },
      { index: 2, text: 'MFA を使うとパスワードは不要になる' },
      { index: 3, text: 'MFA は常にスマートフォンを必要とする' },
    ],
    answer: 1,
    explanation: 'MFA（多要素認証）は知識情報（パスワード・PIN）・所持情報（スマートフォン・ICカード）・生体情報（指紋・顔認証）のうち異なる種類の要素を 2 つ以上組み合わせる。同種の組み合わせ（パスワード＋パスワードヒント）は MFA でない。TOTP（時刻ベースワンタイムパスワード）・FIDO2/WebAuthn が代表的な実装。フィッシングへの耐性が向上する。',
  },
  {
    id: 'SEC-100', category: 'security', categoryName: '情報セキュリティ',
    difficulty: 3, year: 2021, tags: ['脆弱性','CVE','CVSS'],
    question: 'CVSS（Common Vulnerability Scoring System）に関する説明として正しいものはどれか。',
    choices: [
      { index: 0, text: 'CVSS スコアは企業ごとに異なる独自指標で標準化されていない' },
      { index: 1, text: 'CVSS は 0.0〜10.0 のスコアで脆弱性の深刻度を数値化する共通の評価指標' },
      { index: 2, text: 'CVSS スコアが低いほど深刻な脆弱性を示す' },
      { index: 3, text: 'CVSS はマルウェアの感染数を集計する統計指標である' },
    ],
    answer: 1,
    explanation: 'CVSS（共通脆弱性評価システム）：0.0〜10.0 のスコアで脆弱性の深刻度を定量化する国際標準（FIRST が管理）。スコア区分：0.1-3.9=低、4.0-6.9=中、7.0-8.9=高、9.0-10.0=緊急。Base Score（基本評価基準）・Temporal Score（現状評価基準）・Environmental Score（環境評価基準）の 3 指標で構成。CVE ID（例：CVE-2021-44228）で脆弱性を識別。',
  },
  {
    id: 'SEC-101', category: 'security', categoryName: '情報セキュリティ',
    difficulty: 3, year: 2023, tags: ['ネットワークセキュリティ','IDS','IPS'],
    question: 'IDS と IPS の違いとして正しいものはどれか。',
    choices: [
      { index: 0, text: 'IDS は不正アクセスを自動遮断し、IPS は検知・通知のみを行う' },
      { index: 1, text: 'IDS は不正アクセスを検知・通知し、IPS は検知に加えてリアルタイムで通信を遮断する' },
      { index: 2, text: 'IDS は暗号化通信のみ、IPS は平文通信のみを監視する' },
      { index: 3, text: 'IDS と IPS は同じ機能を持ち名称だけが異なる' },
    ],
    answer: 1,
    explanation: 'IDS（Intrusion Detection System）：不正アクセスを検知してアラートを送信するが自動遮断はしない。NIDS（ネットワーク型）とHIDS（ホスト型）がある。IPS（Intrusion Prevention System）：IDS の機能に加えて攻撃トラフィックをリアルタイムに遮断する。インラインで設置する必要があり、誤検知（フォールスポジティブ）で正常通信を遮断するリスクがある。',
  },
  {
    id: 'SEC-102', category: 'security', categoryName: '情報セキュリティ',
    difficulty: 2, year: 2022, tags: ['暗号','AES'],
    question: 'AES（Advanced Encryption Standard）に関する説明として正しいものはどれか。',
    choices: [
      { index: 0, text: '公開鍵暗号アルゴリズムであり、鍵ペアを使用する' },
      { index: 1, text: '共通鍵ブロック暗号アルゴリズム。鍵長 128/192/256 ビットをサポート' },
      { index: 2, text: 'DES の後継だが安全性の問題で現在は廃止されている' },
      { index: 3, text: '非可逆のハッシュ関数であり、復号は不可能' },
    ],
    answer: 1,
    explanation: 'AES（Rijndael アルゴリズム）：NIST が 2001 年に標準化した共通鍵ブロック暗号。ブロック長 128 ビット固定、鍵長は 128/192/256 ビットから選択。DES（56 ビット）の脆弱性を受けた後継。現在も安全とみなされており、TLS・ファイル暗号化・ディスク暗号化（BitLocker）などに広く採用。CBC・GCM などの動作モードと組み合わせて使用。',
  },
  {
    id: 'SEC-103', category: 'security', categoryName: '情報セキュリティ',
    difficulty: 3, year: 2021, tags: ['インシデント対応','CSIRT'],
    question: 'CSIRT（Computer Security Incident Response Team）の主な役割として正しいものはどれか。',
    choices: [
      { index: 0, text: 'システム開発プロジェクトの進捗管理と品質保証を担当する組織' },
      { index: 1, text: 'セキュリティインシデントの検知・分析・対応・復旧・再発防止を担う専門チーム' },
      { index: 2, text: 'ネットワーク機器のハードウェア保守と交換を行う組織' },
      { index: 3, text: 'ソフトウェア脆弱性の発見のみを担当する開発チーム' },
    ],
    answer: 1,
    explanation: 'CSIRT（シーサート）：セキュリティインシデント対応の専門組織。主な活動：①インシデントの検知・トリアージ②影響範囲の分析③封じ込め・根絶・復旧④フォレンジック調査⑤再発防止策の立案⑥脆弱性情報の収集・共有。日本では JPCERT/CC が国の CSIRT に相当。企業内 CSIRT（社内 CSIRT）の設置が増加している。SOC（Security Operations Center）はリアルタイム監視が主な役割。',
  },
  {
    id: 'SEC-104', category: 'security', categoryName: '情報セキュリティ',
    difficulty: 3, year: 2023, tags: ['DLP','情報漏洩対策'],
    question: 'DLP（Data Loss Prevention）の説明として正しいものはどれか。',
    choices: [
      { index: 0, text: '分散型台帳技術を使ってデータを冗長化するシステム' },
      { index: 1, text: '機密データの不正な持ち出し・漏洩を検知・防止するセキュリティソリューション' },
      { index: 2, text: 'データベースのログを自動削除してストレージを節約する仕組み' },
      { index: 3, text: 'ディスク障害時のデータ復旧専用のバックアップツール' },
    ],
    answer: 1,
    explanation: 'DLP（Data Loss Prevention / Data Leakage Prevention）：機密データ（個人情報・知的財産・財務データ）のコンテンツを検査し、不正なコピー・メール転送・USB 持ち出し・クラウドアップロードを検知・ブロックするセキュリティ技術。ネットワーク DLP（通信監視）・エンドポイント DLP（端末監視）・クラウド DLP がある。GDPR 対応・内部不正対策の重要ツール。',
  },
  {
    id: 'SEC-105', category: 'security', categoryName: '情報セキュリティ',
    difficulty: 4, year: 2022, tags: ['暗号プロトコル','TLS'],
    question: 'TLS 1.3 で廃止された TLS 1.2 以前の機能として正しいものはどれか。',
    choices: [
      { index: 0, text: 'デジタル証明書による認証機能' },
      { index: 1, text: 'RSA 鍵交換（静的な RSA 鍵交換）・CBC モード・RC4 などの弱い暗号スイート' },
      { index: 2, text: 'ハンドシェイクによるセッション確立の仕組み' },
      { index: 3, text: 'サーバ証明書の検証プロセス' },
    ],
    answer: 1,
    explanation: 'TLS 1.3 の改善点：①弱い暗号スイートを廃止（RSA 鍵交換・CBC モード・RC4・SHA-1・MD5 など）②完全前方秘匿性（PFS）を必須化（DHE/ECDHE のみ使用）③ハンドシェイクを 1-RTT に短縮（0-RTT セッション再開も可）④証明書の暗号化。TLS 1.0/1.1 は 2021 年に RFC で廃止勧告。',
  },
  {
    id: 'SEC-106', category: 'security', categoryName: '情報セキュリティ',
    difficulty: 2, year: 2021, tags: ['脅威','APT'],
    question: 'APT（Advanced Persistent Threat）攻撃の特徴として正しいものはどれか。',
    choices: [
      { index: 0, text: '不特定多数を標的とし、大量のパケットを短時間で送りつけるサービス妨害攻撃' },
      { index: 1, text: '特定の組織・個人を標的に、長期間にわたって潜伏しながら情報窃取・破壊工作を行う高度な標的型攻撃' },
      { index: 2, text: 'Web アプリケーションの入力フォームへの SQL 文挿入による攻撃' },
      { index: 3, text: 'パスワードの総当たり試行による認証突破攻撃' },
    ],
    answer: 1,
    explanation: 'APT（高度持続的脅威）：国家レベルや組織的な攻撃者による標的型攻撃。特徴：①特定組織を長期間（数カ月〜数年）にわたり潜伏②高度なゼロデイ脆弱性・カスタムマルウェア使用③偵察→侵入→権限昇格→横移動→情報収集→データ持ち出しの段階的攻撃④検知を回避する高度な技術。政府機関・防衛産業・重要インフラが主な標的。',
  },
  {
    id: 'SEC-107', category: 'security', categoryName: '情報セキュリティ',
    difficulty: 3, year: 2023, tags: ['クラウドセキュリティ','責任共有モデル'],
    question: 'クラウドサービスの責任共有モデル（Shared Responsibility Model）に関して、IaaS でクラウドベンダが責任を負う範囲として正しいものはどれか。',
    choices: [
      { index: 0, text: 'OS のセキュリティパッチ適用・アプリケーションの脆弱性管理' },
      { index: 1, text: '物理インフラ（データセンタ・ネットワーク・ハードウェア）のセキュリティ' },
      { index: 2, text: 'データの暗号化・IAM 設定・アプリケーション設定' },
      { index: 3, text: '全てのセキュリティ設定がクラウドベンダの責任範囲に含まれる' },
    ],
    answer: 1,
    explanation: 'クラウド責任共有モデル：IaaS ではベンダが物理インフラ（データセンタ・ネットワーク機器・ハードウェア・仮想化基盤）を担当し、利用者が OS・ミドルウェア・アプリ・データ・IAM 設定を担当。PaaS ではさらに OS・ランタイムもベンダ担当。SaaS ではアプリ以上をベンダ担当。「クラウドのセキュリティはすべてベンダ任せ」は誤解であり、設定ミス（S3 の公開設定等）はユーザ責任。',
  },
  {
    id: 'SEC-108', category: 'security', categoryName: '情報セキュリティ',
    difficulty: 2, year: 2022, tags: ['不正アクセス','ソーシャルエンジニアリング'],
    question: 'ソーシャルエンジニアリング攻撃の例として正しいものはどれか。',
    choices: [
      { index: 0, text: 'SQL インジェクションによるデータベース不正取得' },
      { index: 1, text: '経営者になりすました電話で経理担当者から口座情報を聞き出すなどの人的手法による情報窃取' },
      { index: 2, text: 'バッファオーバフローによる任意コード実行' },
      { index: 3, text: 'パスワードの総当たり攻撃（ブルートフォース）' },
    ],
    answer: 1,
    explanation: 'ソーシャルエンジニアリング：技術的な手法ではなく、人の心理や行動を操作して情報・アクセス権を不正に取得する攻撃。手法例：なりすまし電話（ビッシング）・フィッシングメール・ショルダーハッキング（画面覗き見）・トラッシング（ゴミ箱あさり）・テールゲーティング（共連れ）。対策：セキュリティ教育・情報開示ルールの徹底・本人確認手順の整備。',
  },

  // ─── OS OS-101〜110 ───────────────────────────────────
  {
    id: 'OS-101', category: 'os', categoryName: 'OS・ハードウェア',
    difficulty: 2, year: 2022, tags: ['プロセス','スレッド'],
    question: 'プロセスとスレッドの違いとして正しいものはどれか。',
    choices: [
      { index: 0, text: 'プロセスはメモリを共有し、スレッドはそれぞれ独立したメモリ空間を持つ' },
      { index: 1, text: 'プロセスは独立したメモリ空間を持つ重い実行単位、スレッドはプロセス内でメモリを共有する軽量な実行単位' },
      { index: 2, text: 'スレッドはプロセスより多くのリソースを消費する' },
      { index: 3, text: 'マルチプロセスはシングルコア CPU では実行できない' },
    ],
    answer: 1,
    explanation: 'プロセス：OS が管理する独立したメモリ空間（コードセグメント・データセグメント・ヒープ・スタック）を持つ実行単位。プロセス間通信（IPC）は明示的に必要。スレッド：同一プロセス内でヒープ・グローバル変数を共有する軽量実行単位。コンテキストスイッチコストが低い。ただし共有メモリにより競合状態（Race Condition）に注意が必要。',
  },
  {
    id: 'OS-102', category: 'os', categoryName: 'OS・ハードウェア',
    difficulty: 3, year: 2023, tags: ['スケジューリング','ラウンドロビン'],
    question: 'ラウンドロビン（Round Robin）スケジューリングの特徴として正しいものはどれか。',
    choices: [
      { index: 0, text: '実行中プロセスの優先度が最も高い間は無限に CPU を専有できる' },
      { index: 1, text: '各プロセスに一定の時間量（タイムクォンタム）を割り当て、期限が来たら次のプロセスに切り替える公平なプリエンプティブ方式' },
      { index: 2, text: '到着時刻順に処理し、短いプロセスが有利になる方式' },
      { index: 3, text: '最も実行時間が短いプロセスを優先する方式（SJF）' },
    ],
    answer: 1,
    explanation: 'ラウンドロビン：各プロセスにタイムクォンタム（時間スライス、通常 10〜100ms）を割り当て、使い切ったら実行キューの末尾に再挿入する。すべてのプロセスが公平に CPU を使える。タイムクォンタムが小さすぎるとコンテキストスイッチのオーバヘッドが増加し、大きすぎると FIFO に近づく。時分割処理（タイムシェアリング）OSの基本スケジューリング方式。',
  },
  {
    id: 'OS-103', category: 'os', categoryName: 'OS・ハードウェア',
    difficulty: 3, year: 2021, tags: ['メモリ','仮想記憶'],
    question: '仮想記憶（Virtual Memory）を使うメリットとして正しいものはどれか。',
    choices: [
      { index: 0, text: 'プロセスが物理メモリより大きなアドレス空間を使えるようになる' },
      { index: 1, text: 'CPU の処理速度が 2 倍になる' },
      { index: 2, text: 'ディスクへのアクセスが完全になくなる' },
      { index: 3, text: 'プロセス間でメモリを強制的に共有させ総使用量を削減する' },
    ],
    answer: 0,
    explanation: '仮想記憶のメリット：①プロセスに連続した大きなアドレス空間を提供（物理メモリ制約を超えられる）②プロセス間メモリ保護（独立したアドレス空間）③ページングにより物理メモリを効率利用④コピーオンライト（fork の高速化）。デメリット：スワップ（ページング）発生時にディスクアクセスで大幅な性能低下。スラッシングが最悪ケース。',
  },
  {
    id: 'OS-104', category: 'os', categoryName: 'OS・ハードウェア',
    difficulty: 2, year: 2022, tags: ['ファイルシステム','inode'],
    question: 'Unix 系 OS における inode の説明として正しいものはどれか。',
    choices: [
      { index: 0, text: 'ファイルの名前と内容を一緒に格納するデータ構造' },
      { index: 1, text: 'ファイルのメタデータ（サイズ・権限・タイムスタンプ・データブロック位置）を格納する固定長のデータ構造' },
      { index: 2, text: 'ディレクトリエントリにファイル名と実データを格納する構造' },
      { index: 3, text: 'プロセスの実行状態とメモリマップを管理するカーネル内部構造' },
    ],
    answer: 1,
    explanation: 'inode（Index Node）：Unix 系ファイルシステムでファイルのメタデータを管理する固定長データ構造。格納内容：ファイルサイズ・権限（パーミッション）・所有者 UID/GID・タイムスタンプ（atime/mtime/ctime）・データブロックへのポインタ・ハードリンク数。ファイル名は inode に含まれず、ディレクトリエントリ（ファイル名→inode 番号のマッピング）に格納される。',
  },
  {
    id: 'OS-105', category: 'os', categoryName: 'OS・ハードウェア',
    difficulty: 3, year: 2023, tags: ['コンテナ','Docker','namespace'],
    question: 'Docker コンテナが実現する OS レベルの分離に使用する Linux カーネル機能の組み合わせとして正しいものはどれか。',
    choices: [
      { index: 0, text: 'BIOS 仮想化拡張（VT-x/AMD-V）とハイパーバイザ' },
      { index: 1, text: 'namespace（名前空間）によるリソース分離と cgroups（コントロールグループ）によるリソース制限' },
      { index: 2, text: 'POSIX スレッドと共有メモリセグメント' },
      { index: 3, text: 'RAID コントローラと LVM（論理ボリューム管理）' },
    ],
    answer: 1,
    explanation: 'Docker のコンテナ分離技術：①Linux namespace：PID/ネットワーク/マウント/UTS（ホスト名）/IPC/ユーザ名前空間を分離し、コンテナが独立した環境を見せる②cgroups（Control Groups）：CPU・メモリ・I/O・ネットワーク帯域のリソース使用量を制限・監視。VM と違いホスト OS カーネルを共有するため起動が高速でオーバヘッドが少ないが、カーネルの脆弱性は全コンテナに影響する。',
  },
  {
    id: 'OS-106', category: 'os', categoryName: 'OS・ハードウェア',
    difficulty: 2, year: 2021, tags: ['割り込み','システムコール'],
    question: 'システムコール（System Call）の説明として正しいものはどれか。',
    choices: [
      { index: 0, text: 'ユーザプロセスがカーネル機能（ファイル操作・ネットワーク・メモリ割当等）を呼び出すためのインタフェース' },
      { index: 1, text: '外部デバイスがCPUに処理を要求するハードウェア割り込み機構' },
      { index: 2, text: 'OS 内部のカーネルモジュール間の関数呼び出し規約' },
      { index: 3, text: 'マイクロカーネルアーキテクチャでのサービス間通信プロトコル' },
    ],
    answer: 0,
    explanation: 'システムコール：ユーザ空間のプロセスがカーネル（特権モード）の機能を安全に呼び出すための仕組み。ユーザモード→カーネルモードへの切り替え（コンテキストスイッチより軽量なトラップ命令）が発生する。主なシステムコール：open/read/write/close（ファイル）・fork/exec（プロセス）・socket/connect（ネットワーク）・malloc は libc 実装で内部で brk/mmap を呼ぶ。',
  },
  {
    id: 'OS-107', category: 'os', categoryName: 'OS・ハードウェア',
    difficulty: 3, year: 2022, tags: ['GPU','並列処理'],
    question: 'GPU（Graphics Processing Unit）を汎用計算（GPGPU）に使用する利点として正しいものはどれか。',
    choices: [
      { index: 0, text: 'GPU は CPU より高クロックで動作するため全ての処理が高速になる' },
      { index: 1, text: '数千コアで同種の演算を並列実行するため、行列演算・機械学習・画像処理などに非常に効率的' },
      { index: 2, text: 'GPU はシングルスレッドの逐次処理に特化しており、分岐の多い処理に優れる' },
      { index: 3, text: 'GPU はメモリ帯域が CPU より低いため省電力処理に適している' },
    ],
    answer: 1,
    explanation: 'GPGPU（General-Purpose computing on GPU）：GPU の多数のシェーダコア（数千〜数万）を科学計算・AI・データ処理に活用する技術。GPU は SIMT（Single Instruction Multiple Threads）アーキテクチャで同一命令を大量スレッドが並列実行。行列積・畳み込み演算など規則的な並列処理に最適。NVIDIA CUDA・OpenCL が主要プラットフォーム。深層学習での学習処理を CPU 比 10〜100 倍高速化。',
  },
  {
    id: 'OS-108', category: 'os', categoryName: 'OS・ハードウェア',
    difficulty: 2, year: 2023, tags: ['キャッシュ','階層記憶'],
    question: 'CPU キャッシュメモリの階層（L1・L2・L3）に関する説明として正しいものはどれか。',
    choices: [
      { index: 0, text: 'L3 は L1 より高速で小容量、L1 は低速で大容量' },
      { index: 1, text: 'L1 は最高速・最小容量（コア専用）、L2 は中間、L3 は低速・大容量（複数コア共有）' },
      { index: 2, text: 'L1・L2・L3 はすべて同一の速度とサイズをもつ' },
      { index: 3, text: 'L3 キャッシュはディスクキャッシュとして使用されることが多い' },
    ],
    answer: 1,
    explanation: 'CPU キャッシュ階層：L1（1次キャッシュ）：最高速（数サイクル）、最小容量（32〜64KB）、コア専用、命令キャッシュとデータキャッシュに分離。L2（2次）：中速（10〜20サイクル）、中容量（256KB〜1MB）、コア専用か共有。L3（3次）：低速（30〜50サイクル）、大容量（4〜64MB）、全コア共有。主記憶（DRAM）は100〜200サイクル。局所性の原理（時間・空間）で高ヒット率を実現。',
  },
  {
    id: 'OS-109', category: 'os', categoryName: 'OS・ハードウェア',
    difficulty: 3, year: 2021, tags: ['デッドロック','バンカーアルゴリズム'],
    question: 'バンカーアルゴリズムの目的として正しいものはどれか。',
    choices: [
      { index: 0, text: 'デッドロックが発生した後に、最も効率よく回復する手順を決定する' },
      { index: 1, text: 'リソース割り当て前に安全状態か検査し、デッドロックを事前に回避する' },
      { index: 2, text: 'プロセスの優先度に基づいて CPU スケジューリングを最適化する' },
      { index: 3, text: 'ページ置換アルゴリズムを決定してスラッシングを防止する' },
    ],
    answer: 1,
    explanation: 'バンカーアルゴリズム（Dijkstra 考案）：リソース割り当て要求が来たとき、それを許可した場合に「安全状態（全プロセスが有限時間内に完了できる実行順序が存在する状態）」が保たれるかシミュレーションし、安全ならリソースを割り当て、安全でなければ要求を待たせるデッドロック回避アルゴリズム。銀行が顧客への融資可能性を事前に確認することに例えられる。',
  },
  {
    id: 'OS-110', category: 'os', categoryName: 'OS・ハードウェア',
    difficulty: 2, year: 2022, tags: ['SSD','NVMe'],
    question: 'SSD（Solid State Drive）と HDD（Hard Disk Drive）の比較として正しいものはどれか。',
    choices: [
      { index: 0, text: 'SSD は機械的な磁気ディスクを使用し、HDD はフラッシュメモリを使用する' },
      { index: 1, text: 'SSD はフラッシュメモリを使用し、HDD より高速・衝撃耐性が高いが、書き込み回数に上限（書き換え寿命）がある' },
      { index: 2, text: 'SSD はHDD よりデータ単価（GB あたりコスト）が安い' },
      { index: 3, text: 'HDD は断片化（フラグメンテーション）が起きないため SSD より高速なシーク時間を持つ' },
    ],
    answer: 1,
    explanation: 'SSD：NAND フラッシュメモリを使用。メリット：高速読み書き（NVMe SSD はシーケンシャル 3〜7 GB/s）・ランダムアクセス高速・衝撃耐性・静音・低消費電力。デメリット：GB あたりコストが HDD より高い・書き換え回数に上限（TLC で数百回〜、SLC で数万回）・電源断での一部データ消失リスク。HDD：大容量・低コストだが機械的可動部があり衝撃に弱く、ランダムアクセスが遅い。',
  },

  // ─── 論理（ロジック）LG-091〜100 ──────────────────────
  {
    id: 'LG-091', category: 'logic', categoryName: '論理・数学',
    difficulty: 2, year: 2022, tags: ['論理演算','ブール代数'],
    question: 'ド・モルガンの法則の正しい式はどれか。',
    choices: [
      { index: 0, text: '¬(A ∧ B) = ¬A ∧ ¬B' },
      { index: 1, text: '¬(A ∧ B) = ¬A ∨ ¬B  かつ  ¬(A ∨ B) = ¬A ∧ ¬B' },
      { index: 2, text: '¬(A ∨ B) = ¬A ∨ ¬B' },
      { index: 3, text: '¬(A ∧ B) = A ∨ B' },
    ],
    answer: 1,
    explanation: 'ド・モルガンの法則：①¬(A ∧ B) = ¬A ∨ ¬B（AND の否定 = OR of 否定）②¬(A ∨ B) = ¬A ∧ ¬B（OR の否定 = AND of 否定）。論理回路設計・NAND ゲートのみでの全論理実装・プログラムの条件式書き換えに活用される。例：if(!A || !B) は if(!(A && B)) と等価。',
  },
  {
    id: 'LG-092', category: 'logic', categoryName: '論理・数学',
    difficulty: 2, year: 2021, tags: ['2進数','16進数'],
    question: '16 進数 B3（hex）を 10 進数に変換した値として正しいものはどれか。',
    choices: [
      { index: 0, text: '163' },
      { index: 1, text: '175' },
      { index: 2, text: '179' },
    ...
    answer: 2,
    explanation: '16 進数 B3 を 10 進数に変換：B = 11（16進 B は 10進 11）、3 = 3。計算：11 × 16¹ + 3 × 16⁰ = 11 × 16 + 3 = 176 + 3 = 179。16進→10進の変換は各桁に 16 の累乗を掛けて合計する。A=10, B=11, C=12, D=13, E=14, F=15。',
  },
  {
    id: 'LG-093', category: 'logic', categoryName: '論理・数学',
    difficulty: 3, year: 2023, tags: ['補数','2の補数'],
    question: '2 の補数表現（8 ビット）で −27 を表した値として正しいものはどれか。',
    choices: [
      { index: 0, text: '11100100' },
      { index: 1, text: '11100101' },
      { index: 2, text: '11011011' },
      { index: 3, text: '10011011' },
    ],
    answer: 1,
    explanation: '2 の補数で −27 を求める手順：①27 の 2 進数 = 00011011②ビット反転（1 の補数）= 11100100③1 を加算 = 11100101。検証：11100101 + 00011011 = 100000000（オーバフロー分の 1 ビットを切り捨て = 00000000 = 0）。2 の補数では −128〜127（8 ビット）の範囲を表現できる。',
  },
  {
    id: 'LG-094', category: 'logic', categoryName: '論理・数学',
    difficulty: 2, year: 2022, tags: ['集合','ベン図'],
    question: '集合 A={1,2,3,4,5} と集合 B={3,4,5,6,7} に対し、差集合 A−B として正しいものはどれか。',
    choices: [
      { index: 0, text: '{3,4,5}' },
      { index: 1, text: '{1,2,3,4,5,6,7}' },
      { index: 2, text: '{1,2}' },
      { index: 3, text: '{6,7}' },
    ],
    answer: 2,
    explanation: '差集合 A−B：A に含まれるが B に含まれない要素の集合。A={1,2,3,4,5}、B={3,4,5,6,7} の場合、A∩B={3,4,5} を A から除くと {1,2}。なお B−A = {6,7}、A∪B = {1,2,3,4,5,6,7}、A∩B = {3,4,5}。ベン図で視覚的に確認できる。',
  },
  {
    id: 'LG-095', category: 'logic', categoryName: '論理・数学',
    difficulty: 3, year: 2021, tags: ['確率','条件付き確率'],
    question: '100 人のうち 40 人が病気 X に罹患している。検査で陽性になる確率は罹患者で 90%、非罹患者で 20%。陽性者が実際に罹患している確率（陽性的中率）として正しいものはどれか。',
    choices: [
      { index: 0, text: '約 55%' },
      { index: 1, text: '約 65%' },
      { index: 2, text: '約 75%' },
    ...
    answer: 2,
    explanation: '真陽性：40 × 0.9 = 36 人、偽陽性：60 × 0.2 = 12 人、陽性合計 = 48 人。陽性的中率（PPV）= 36/48 = 0.75 = 75%。ベイズの定理：P(病気|陽性) = P(陽性|病気)×P(病気) / P(陽性) = (0.9×0.4) / (0.9×0.4 + 0.2×0.6) = 0.36/0.48 = 0.75。検査の有病率・感度・特異度によって的中率が大きく変わる。',
  },
  {
    id: 'LG-096', category: 'logic', categoryName: '論理・数学',
    difficulty: 2, year: 2022, tags: ['浮動小数点','IEEE754'],
    question: 'IEEE 754 単精度浮動小数点数（32 ビット）の構造として正しいものはどれか。',
    choices: [
      { index: 0, text: '符号 1 ビット・指数 8 ビット・仮数 23 ビット（計 32 ビット）' },
      { index: 1, text: '符号 2 ビット・指数 10 ビット・仮数 20 ビット（計 32 ビット）' },
      { index: 2, text: '符号 1 ビット・指数 11 ビット・仮数 52 ビット（計 64 ビット）' },
      { index: 3, text: '符号 1 ビット・指数 16 ビット・仮数 15 ビット（計 32 ビット）' },
    ],
    answer: 0,
    explanation: 'IEEE 754 単精度（float）：符号 1 ビット＋指数部 8 ビット（バイアス 127）＋仮数部 23 ビット＝計 32 ビット。有効桁数は約 7 桁。倍精度（double）：符号 1 ビット＋指数部 11 ビット（バイアス 1023）＋仮数部 52 ビット＝計 64 ビット。有効桁数約 15 桁。丸め誤差に注意（0.1+0.2≠0.3 問題）。',
  },
  {
    id: 'LG-097', category: 'logic', categoryName: '論理・数学',
    difficulty: 3, year: 2023, tags: ['論理回路','フリップフロップ'],
    question: 'D フリップフロップ（D-FF）の動作として正しいものはどれか。',
    choices: [
      { index: 0, text: 'クロックの立ち上がりエッジで、入力 D の値を出力 Q に保持する' },
      { index: 1, text: '入力 D が変化するたびに即座に出力 Q が変化する（非同期）' },
      { index: 2, text: 'J と K の 2 入力で 4 種類の動作（保持・セット・リセット・反転）が可能' },
      { index: 3, text: 'セット・リセット入力のみで、時計信号（クロック）を持たない' },
    ],
    answer: 0,
    explanation: 'D フリップフロップ（遅延 FF）：クロックの立ち上がり（または立ち下がり）エッジ時に入力 D の値を捕捉して出力 Q に保持する。クロックが来るまでは以前の値を維持。レジスタ・シフトレジスタの基本素子。JK-FF は J=K=1 で反転（Toggle）動作が可能。SR ラッチはクロックなしのレベル感応型。',
  },
  {
    id: 'LG-098', category: 'logic', categoryName: '論理・数学',
    difficulty: 2, year: 2021, tags: ['シフト演算'],
    question: '符号なし整数 24（10進）を 2 ビット右シフトした結果として正しいものはどれか。',
    choices: [
      { index: 0, text: '3' },
      { index: 1, text: '6' },
      { index: 2, text: '12' },
    ...
    answer: 1,
    explanation: '24 を 2 進数で表すと 11000。2 ビット右シフト（論理右シフト）すると 00110 = 6。右シフト n ビットは 2ⁿ で割る操作（整数除算）に相当：24 ÷ 4 = 6。左シフト n ビットは 2ⁿ を掛ける操作：24 × 4 = 96 = 11000 を 2 ビット左シフト → 1100000。符号付き整数の右シフトは算術右シフト（符号ビットを維持）と論理右シフト（0 補充）に注意。',
  },
  {
    id: 'LG-099', category: 'logic', categoryName: '論理・数学',
    difficulty: 3, year: 2022, tags: ['オートマトン','正規表現'],
    question: '正規表現 `[0-9]+\\.[0-9]+` が一致する文字列パターンとして正しいものはどれか。',
    choices: [
      { index: 0, text: '整数のみの数字列（例：123）' },
      { index: 1, text: '小数点を含む数値（例：3.14、0.5）' },
      { index: 2, text: '任意の英数字と記号の組み合わせ' },
      { index: 3, text: '先頭が英字で始まる識別子' },
    ],
    answer: 1,
    explanation: '`[0-9]+\\.[0-9]+` の解析：`[0-9]+` は 1 桁以上の数字、`\\.` はリテラルのピリオド（. をエスケープ）、`[0-9]+` は再び 1 桁以上の数字。つまり「数字列.数字列」形式の小数点付き数値にマッチ。例：3.14、0.5、123.456 は一致。123（ピリオドなし）や .5（先頭の数字なし）は不一致。',
  },
  {
    id: 'LG-100', category: 'logic', categoryName: '論理・数学',
    difficulty: 3, year: 2023, tags: ['誤り訂正','ハミング符号'],
    question: 'ハミング符号の特徴として正しいものはどれか。',
    choices: [
      { index: 0, text: '1 ビットの誤り検出のみが可能で訂正はできない' },
      { index: 1, text: '1 ビットの誤り検出・訂正が可能。パリティビットを複数追加することで誤りビットの位置を特定できる' },
      { index: 2, text: '最大 8 ビットの誤りを同時に訂正できる高度な符号' },
      { index: 3, text: 'データの圧縮と誤り訂正を同時に行う符号化方式' },
    ],
    answer: 1,
    explanation: 'ハミング符号：1 ビット誤り検出・訂正が可能な線形誤り訂正符号。2 ビット誤りの検出も可能。原理：複数の冗長ビット（パリティビット）を特定位置（2 の冪乗位置：1, 2, 4, 8, ...）に挿入。各パリティビットは特定のデータビット群をカバーし、シンドローム計算で誤りビット位置を特定する。4 データビットに 3 パリティビット追加（7,4 ハミング符号）が基本例。RAM のECCメモリで使用。',
  },

  // ─── サービス管理 SM-044〜053 ────────────────────────
  {
    id: 'SM-044', category: 'service-mgmt', categoryName: 'サービス管理',
    difficulty: 2, year: 2022, tags: ['ITIL','変更管理'],
    question: 'ITIL の変更管理プロセスで「変更要求（RFC）」を審査する組織として正しいものはどれか。',
    choices: [
      { index: 0, text: 'SLA 委員会（Service Level Agreement Committee）' },
      { index: 1, text: '変更諮問委員会（CAB：Change Advisory Board）' },
      { index: 2, text: 'セキュリティ運営委員会（SOC）' },
      { index: 3, text: 'リスク管理委員会（Risk Management Committee）' },
    ],
    answer: 1,
    explanation: 'ITIL 変更管理：変更要求（RFC）はまず変更マネージャが評価し、重要な変更は CAB（Change Advisory Board：変更諮問委員会）に審査を依頼する。CAB は技術・ビジネス・セキュリティの専門家で構成され、変更のリスク・影響・緊急性・実現可能性を評価して承認/却下を判断する。緊急変更は ECAB（緊急 CAB）が対応。',
  },
  {
    id: 'SM-045', category: 'service-mgmt', categoryName: 'サービス管理',
    difficulty: 3, year: 2021, tags: ['可用性','SLO'],
    question: 'SLO（Service Level Objective）と SLA（Service Level Agreement）の関係として正しいものはどれか。',
    choices: [
      { index: 0, text: 'SLA は内部目標値、SLO は外部契約条件' },
      { index: 1, text: 'SLO はサービス品質の具体的な目標値（例：稼働率 99.9%）、SLA は SLO を含む顧客との正式契約' },
      { index: 2, text: 'SLO と SLA は同義で常に同じ数値を使用する' },
      { index: 3, text: 'SLA はエラーバジェットの計算のみに使用し、SLO は顧客提示用' },
    ],
    answer: 1,
    explanation: 'SLI（Service Level Indicator）：実際の測定値（例：レイテンシ・エラー率）。SLO（Service Level Objective）：SLI の目標値（例：P99 レイテンシ < 200ms、稼働率 99.9%）。SLA（Service Level Agreement）：SLO を含む顧客との法的拘束力のある契約（未達時のペナルティを規定）。SLO は SLA より少し厳しく設定するのが一般的（SLO 未達 → SLA 違反前に対処できる余裕を持たせる）。',
  },
  {
    id: 'SM-046', category: 'service-mgmt', categoryName: 'サービス管理',
    difficulty: 2, year: 2022, tags: ['インシデント管理','問題管理'],
    question: 'ITIL でインシデント管理と問題管理を区別する説明として正しいものはどれか。',
    choices: [
      { index: 0, text: 'インシデント管理は根本原因を分析し、問題管理はサービス復旧を目的とする' },
      { index: 1, text: 'インシデント管理はサービス正常化を最優先し、問題管理はインシデントの根本原因を調査・再発防止する' },
      { index: 2, text: '両者は同一プロセスで、ITIL では区別されていない' },
      { index: 3, text: 'インシデント管理は変更承認を行い、問題管理は SLA を測定する' },
    ],
    answer: 1,
    explanation: 'ITIL のインシデント管理：サービス障害に迅速に対応してサービスを正常状態に復旧させることが最優先（根本原因の解明より復旧速度）。暫定対処（ワークアラウンド）も許容。問題管理：繰り返し発生するインシデントや深刻なインシデントの根本原因（根本要因）を分析し再発防止策を実施。既知の誤りは KEDB（既知の誤りデータベース）に登録してインシデント対応に活用。',
  },
  {
    id: 'SM-047', category: 'service-mgmt', categoryName: 'サービス管理',
    difficulty: 3, year: 2023, tags: ['BCP','DR'],
    question: 'BCP（Business Continuity Plan）と DR（Disaster Recovery）の関係として正しいものはどれか。',
    choices: [
      { index: 0, text: 'DR は事業全体の継続計画、BCP は IT システムの復旧計画に特化したもの' },
      { index: 1, text: 'BCP は大規模災害時に重要業務を継続するための計画全体。DR は BCP の一部でIT システムの復旧に特化した計画' },
      { index: 2, text: 'BCP と DR は同義語で、どちらも IT システム復旧のみを対象とする' },
      { index: 3, text: 'DR は人員・施設・IT を含む事業継続全般、BCP は IT 部門のみの計画' },
    ],
    answer: 1,
    explanation: 'BCP（事業継続計画）：地震・洪水・パンデミック等の重大障害時に重要業務を継続または早期復旧するための計画。人員・施設・サプライチェーン・IT を含む事業全体を対象。BCM（Business Continuity Management）として継続的に管理・演習する。DR（災害復旧）：BCP の構成要素の一つで IT システム・データの復旧に特化した計画。RTO（目標復旧時間）・RPO（目標復旧時点）を設定。',
  },
  {
    id: 'SM-048', category: 'service-mgmt', categoryName: 'サービス管理',
    difficulty: 2, year: 2021, tags: ['ITIL4','価値共創'],
    question: 'ITIL 4 の中核概念「サービスバリューシステム（SVS）」に含まれるものとして正しいものはどれか。',
    choices: [
      { index: 0, text: 'ウォーターフォール・プロセス・ハードウェア仕様・コスト管理' },
      { index: 1, text: 'ガバナンス・サービスバリューチェーン・プラクティス・継続的改善・指導原則' },
      { index: 2, text: 'PDCA サイクル・プロジェクト計画・WBS・ガントチャート' },
      { index: 3, text: 'OSI 参照モデル・TCP/IP・HTTP・DNS' },
    ],
    answer: 1,
    explanation: 'ITIL 4 のサービスバリューシステム（SVS）構成要素：①指導原則（Guiding Principles）：価値に注力・シンプルに保つ等の 7 原則②ガバナンス（Governance）：評価・指示・監視③サービスバリューチェーン（SVC）：計画・改善・関与・設計移行・入手構築・供給サポートの 6 活動④プラクティス（34 のプラクティス）⑤継続的改善。ITIL v3 の「プロセス」から「プラクティス」に概念を拡張。',
  },
  {
    id: 'SM-049', category: 'service-mgmt', categoryName: 'サービス管理',
    difficulty: 3, year: 2022, tags: ['キャパシティ管理','性能管理'],
    question: 'ITIL のキャパシティ管理の目的として正しいものはどれか。',
    choices: [
      { index: 0, text: 'セキュリティインシデントの発生件数を記録・追跡する' },
      { index: 1, text: '現在および将来のビジネスニーズを費用対効果の高い方法で満たすIT容量を確保する' },
      { index: 2, text: 'サービスデスクへの問い合わせ件数を最小化する' },
      { index: 3, text: 'ハードウェア障害を予防するために定期的なメンテナンスを実施する' },
    ],
    answer: 1,
    explanation: 'ITIL キャパシティ管理：ビジネス要求（現在・将来）を満たす IT リソース（CPU・メモリ・ストレージ・ネットワーク帯域）の容量を適切なコストで確保・管理するプロセス。3 つのサブプロセス：ビジネスキャパシティ管理（将来需要予測）・サービスキャパシティ管理（SLA 達成確認）・コンポーネントキャパシティ管理（個別機器の容量監視）。キャパシティ計画の成果物はキャパシティ管理情報システム（CMIS）に格納。',
  },
  {
    id: 'SM-050', category: 'service-mgmt', categoryName: 'サービス管理',
    difficulty: 2, year: 2023, tags: ['サービスデスク','SPOC'],
    question: 'ITIL のサービスデスクの役割として正しいものはどれか。',
    choices: [
      { index: 0, text: 'IT 戦略の立案とビジネス目標へのアラインメントを担当する' },
      { index: 1, text: 'ユーザからの問い合わせ・インシデントの単一窓口（SPOC）として対応し、記録・分類・エスカレーションを行う' },
      { index: 2, text: '新システムの開発プロジェクトのプロジェクト管理を専門に担当する' },
      { index: 3, text: 'ネットワーク機器のハードウェア保守と部品交換を担当する' },
    ],
    answer: 1,
    explanation: 'サービスデスク（Service Desk）：ITIL でユーザとITサービスプロバイダの間の単一窓口（SPOC：Single Point of Contact）。主な機能：①インシデント・問い合わせの受付・記録②一次対応（自己解決支援・手順書参照）③分類・優先度付け④解決またはエスカレーション⑤クローズ確認⑥ユーザ満足度調査。従来の「ヘルプデスク」より広い役割。センター型・分散型・仮想型の構成形態がある。',
  },
  {
    id: 'SM-051', category: 'service-mgmt', categoryName: 'サービス管理',
    difficulty: 3, year: 2021, tags: ['監査','ISO20000'],
    question: 'ISO/IEC 20000 の説明として正しいものはどれか。',
    choices: [
      { index: 0, text: '情報セキュリティマネジメントシステム（ISMS）の国際規格' },
      { index: 1, text: 'IT サービスマネジメント（ITSM）のマネジメントシステム要求事項を定めた国際規格' },
      { index: 2, text: 'ソフトウェア品質を評価するための特性と副特性を定めた国際規格' },
      { index: 3, text: 'プロジェクトマネジメントの国際標準プロセスを定めた規格' },
    ],
    answer: 1,
    explanation: 'ISO/IEC 20000：IT サービスマネジメント（ITSM）に関する国際規格。サービス提供者が ITSMを効果的に計画・実施・監視・改善するための要求事項（Part 1）とガイダンス（Part 2）で構成。ITIL のベストプラクティスを規格化したもの。第三者認証の取得が可能。混同注意：ISO 27001（ISMS）・ISO 9001（品質マネジメント）・ISO 25010（ソフトウェア品質）。',
  },
  {
    id: 'SM-052', category: 'service-mgmt', categoryName: 'サービス管理',
    difficulty: 2, year: 2022, tags: ['継続的改善','PDCA'],
    question: 'ITIL 4 の継続的改善モデルと PDCA サイクルの関係として正しいものはどれか。',
    choices: [
      { index: 0, text: 'ITIL 4 は PDCA を否定しており、独自の改善モデルのみを使用する' },
      { index: 1, text: 'ITIL 4 の継続的改善モデルは PDCA と整合しており、ビジョン→現状→目標→計画→行動→達成確認→ウェーブ維持の 7 ステップで構成される' },
      { index: 2, text: '継続的改善は ITIL 4 では廃止されて DevOps に置き換えられた' },
      { index: 3, text: 'PDCA は IT システムの開発にのみ適用でき、サービス管理には使用できない' },
    ],
    answer: 1,
    explanation: 'ITIL 4 継続的改善モデルの 7 ステップ：①ビジョンは何か②現状はどこか③目標はどこか④どのようにゴールに到達するか⑤行動を起こせ⑥到達したか⑦モメンタムを維持せよ。PDCA（Plan-Do-Check-Act）と整合しており、組織のさまざまな改善活動に適用できる。ISO 品質マネジメント・CMMI とも互換性がある普遍的な改善フレームワーク。',
  },
  {
    id: 'SM-053', category: 'service-mgmt', categoryName: 'サービス管理',
    difficulty: 3, year: 2023, tags: ['ITガバナンス','COBIT'],
    question: 'COBIT（Control Objectives for Information and related Technology）の説明として正しいものはどれか。',
    choices: [
      { index: 0, text: 'アジャイル開発プロセスを定義したフレームワーク' },
      { index: 1, text: 'IT ガバナンス・管理のフレームワーク。経営目標と IT 目標の連携・IT リスク管理・コンプライアンスの実現を支援する' },
      { index: 2, text: 'クラウドサービスのセキュリティ評価基準を定めた規格' },
      { index: 3, text: 'オープンソースソフトウェアのライセンス管理フレームワーク' },
    ],
    answer: 1,
    explanation: 'COBIT（ISACA が提唱）：IT ガバナンス・マネジメントのための包括的フレームワーク。経営目標・ガバナンス目標・管理目標を整合させ、IT リスク管理・内部統制・コンプライアンスを実現する。COBIT 2019 は 40 の管理目標と 6 つのガバナンス目標を定義。Sarbanes-Oxley 法（SOX）への対応や IT 監査の基準として活用。ITIL がサービス管理の「how」を示すのに対し、COBIT は「what」と「why」を示す。',
  },

  // ─── ビジネストラテジ BS-047〜056 ────────────────────
  {
    id: 'BS-047', category: 'business-strategy', categoryName: 'ビジネストラテジ',
    difficulty: 2, year: 2022, tags: ['経営戦略','ポーターの5力'],
    question: 'マイケル・ポーターの「5 つの競争要因（5Forces）」に含まれないものはどれか。',
    choices: [
      { index: 0, text: '既存競合他社間の競争' },
      { index: 1, text: '代替品の脅威' },
      { index: 2, text: 'マクロ経済環境の変動' },
      { index: 3, text: '買い手（顧客）の交渉力' },
    ],
    answer: 2,
    explanation: 'ポーターの 5 Forces：①既存競合者間の競争②新規参入者の脅威③代替品・代替サービスの脅威④買い手（顧客）の交渉力⑤売り手（供給者）の交渉力。マクロ経済環境（金利・為替・景気）は PEST 分析（Politics/Economy/Society/Technology）で分析するフレームワーク。5 Forces は業界の収益性・魅力度を分析するツール。',
  },
  {
    id: 'BS-048', category: 'business-strategy', categoryName: 'ビジネストラテジ',
    difficulty: 3, year: 2021, tags: ['CRM','顧客管理'],
    question: 'CRM（Customer Relationship Management）の目的として正しいものはどれか。',
    choices: [
      { index: 0, text: '生産ラインの稼働効率を最大化し製造コストを削減する' },
      { index: 1, text: '顧客情報を一元管理し、顧客との長期的な関係を構築して顧客生涯価値（LTV）を向上させる' },
      { index: 2, text: 'サプライチェーン全体の在庫を最適化して物流コストを削減する' },
      { index: 3, text: '財務・人事・調達を統合して業務の一元管理を実現する' },
    ],
    answer: 1,
    explanation: 'CRM（顧客関係管理）：顧客のプロフィール・購買履歴・問い合わせ履歴・コミュニケーション記録を一元管理し、顧客ニーズに合わせたサービス・マーケティングを実現して顧客満足・LTV（Life Time Value：顧客生涯価値）を向上させる戦略・システム。Salesforce・HubSpot が代表ツール。SCM（Supply Chain Management）は供給網管理。ERP は企業資源計画（ERPが正解は誤り）。',
  },
  {
    id: 'BS-049', category: 'business-strategy', categoryName: 'ビジネストラテジ',
    difficulty: 2, year: 2023, tags: ['BPR','業務改革'],
    question: 'BPR（Business Process Reengineering）の概念として正しいものはどれか。',
    choices: [
      { index: 0, text: '既存業務プロセスを段階的に小さく改善する継続的改善手法' },
      { index: 1, text: '業務プロセスを根本的に見直し、コスト・品質・スピードを劇的に改善する抜本的改革手法' },
      { index: 2, text: '社員の評価制度を再設計して生産性を向上させる人事制度改革' },
      { index: 3, text: 'IT システムの段階的なバージョンアップ計画' },
    ],
    answer: 1,
    explanation: 'BPR（Hammer & Champy が提唱）：「コスト・品質・スービス・スピードのような、重要で現代的なパフォーマンスの劇的な改善を成し遂げるために、ビジネスプロセスを根本的に考え直し抜本的にデザインし直すこと」。段階的改善（Kaizen・TQM）ではなく急進的な変革を志向。ただし強引な実施は現場の抵抗・失敗を招くことも多い。ERP 導入と組み合わせて実施されることが多い。',
  },
  {
    id: 'BS-050', category: 'business-strategy', categoryName: 'ビジネストラテジ',
    difficulty: 3, year: 2022, tags: ['SaaS','クラウドビジネス'],
    question: 'SaaS（Software as a Service）のビジネスモデルの特徴として正しいものはどれか。',
    choices: [
      { index: 0, text: 'ユーザはソフトウェアを購入・ライセンスし、自社サーバにインストールして使用する' },
      { index: 1, text: 'サブスクリプション（月額/年額）でソフトウェアをクラウドから提供し、導入コスト低減・自動アップデートが特徴' },
      { index: 2, text: 'ハードウェアリソース（CPU・メモリ・ストレージ）をオンデマンドで提供するモデル' },
      { index: 3, text: '開発フレームワーク・データベース・ミドルウェアをクラウドで提供するモデル' },
    ],
    answer: 1,
    explanation: 'SaaS（Software as a Service）：ソフトウェアをインターネット経由でサービスとして提供するクラウドモデル。特徴：サブスクリプション課金・初期投資不要・自動バージョンアップ・マルチデバイス対応。例：Salesforce・Google Workspace・Microsoft 365・Slack。IaaS（Infrastructure as a Service）は仮想サーバ・ストレージ提供。PaaS（Platform as a Service）は開発基盤提供。',
  },
  {
    id: 'BS-051', category: 'business-strategy', categoryName: 'ビジネストラテジ',
    difficulty: 2, year: 2021, tags: ['ROI','投資対効果'],
    question: 'IT 投資 1,000 万円で年間 400 万円のコスト削減が見込まれる場合、投資回収期間（PP：Payback Period）として正しいものはどれか。',
    choices: [
      { index: 0, text: '1 年' },
      { index: 1, text: '2 年 6 カ月' },
      { index: 2, text: '3 年' },
      { index: 3, text: '4 年' },
    ],
    answer: 1,
    explanation: '投資回収期間 = 初期投資額 ÷ 年間キャッシュフロー = 1,000万円 ÷ 400万円/年 = 2.5 年 = 2 年 6 カ月。ROI = (利益 ÷ 投資額) × 100% = (400万 × 2年 ÷ 1000万) × 100% = 80%（2年後）。NPV（正味現在価値）は将来キャッシュフローを現在価値に割り引いて投資価値を評価する指標。内部収益率（IRR）は NPV = 0 となる割引率。',
  },
  {
    id: 'BS-052', category: 'business-strategy', categoryName: 'ビジネストラテジ',
    difficulty: 3, year: 2023, tags: ['DX','デジタル化'],
    question: 'IPA が定義する DX（デジタルトランスフォーメーション）の説明として正しいものはどれか。',
    choices: [
      { index: 0, text: '紙の書類をスキャンして PDF に変換するデジタル化作業' },
      { index: 1, text: 'デジタル技術を活用してビジネスモデル・組織・プロセスを変革し、競争優位を確立する継続的な取り組み' },
      { index: 2, text: '既存 IT システムをクラウドに移行（リフト&シフト）すること' },
      { index: 3, text: 'スマートフォンアプリを開発して顧客へ新サービスを提供すること' },
    ],
    answer: 1,
    explanation: 'DX（Digital Transformation）：単なる IT 化・システム移行ではなく、データ・デジタル技術を活用してビジネスモデル・組織文化・プロセスを根本から変革し、競争優位性を持続的に維持する取り組み。IPA の「DX 推進指標」では経営戦略との整合・データ活用・仕組みの変革・人材育成を評価。「2025 年の崖」（技術的負債・レガシーシステム問題）の解消も DX の重要課題。',
  },
  {
    id: 'BS-053', category: 'business-strategy', categoryName: 'ビジネストラテジ',
    difficulty: 2, year: 2022, tags: ['マーケティング','4P'],
    question: 'マーケティングの「4P」に含まれないものはどれか。',
    choices: [
      { index: 0, text: '価格（Price）' },
      { index: 1, text: '人材（People）' },
      { index: 2, text: '製品（Product）' },
      { index: 3, text: 'プロモーション（Promotion）' },
    ],
    answer: 1,
    explanation: 'マーケティングミックス「4P」：①Product（製品・サービス）②Price（価格戦略）③Place（流通・チャネル）④Promotion（広告・販売促進）。People（人材）は含まれない。なお顧客視点の「4C」はCustomer Value（顧客価値）・Cost（顧客コスト）・Convenience（利便性）・Communication（コミュニケーション）。サービスマーケティングでは 7P（4P＋People・Process・Physical Evidence）を使う場合もある。',
  },
  {
    id: 'BS-054', category: 'business-strategy', categoryName: 'ビジネストラテジ',
    difficulty: 3, year: 2021, tags: ['SCM','サプライチェーン'],
    question: 'SCM（Supply Chain Management）の「ブルウィップ効果」の説明として正しいものはどれか。',
    choices: [
      { index: 0, text: 'サプライチェーン上流（製造元）の小さな需要変動が下流（小売）に増幅されて伝わる現象' },
      { index: 1, text: 'サプライチェーン下流の小さな需要変動が上流に増幅されて伝わる現象' },
      { index: 2, text: '在庫が過剰になるとサプライチェーン全体が停止する現象' },
      { index: 3, text: 'サプライヤの倒産によりサプライチェーン全体が崩壊する現象' },
    ],
    answer: 1,
    explanation: 'ブルウィップ効果（Bullwhip Effect）：エンドユーザの需要のわずかな変動が、小売→卸売→メーカー→原材料サプライヤと上流に伝わるにつれて増幅される現象（チェーンの鞭のように先端の小さな動きが根元で大きくなる）。原因：各階層での安全在庫確保・注文バッチ処理・価格変動投機・需要予測誤差。対策：サプライチェーン全体でのリアルタイム需要共有・VMI（ベンダー管理在庫）。',
  },
  {
    id: 'BS-055', category: 'business-strategy', categoryName: 'ビジネストラテジ',
    difficulty: 2, year: 2022, tags: ['AI','機械学習'],
    question: '機械学習の「過学習（Overfitting）」の説明として正しいものはどれか。',
    choices: [
      { index: 0, text: 'モデルが訓練データに過度に適合し、未知データへの汎化性能が低下する状態' },
      { index: 1, text: 'モデルが十分に訓練されておらず、訓練データにも正確に適合できない状態' },
      { index: 2, text: 'モデルの学習速度が速すぎて最適解を飛び越えてしまう状態' },
      { index: 3, text: '大量のデータを処理する際にメモリが不足する状態' },
    ],
    answer: 0,
    explanation: '過学習（Overfitting）：モデルが訓練データのノイズ・偏りまで学習しすぎて、訓練データには高精度だが未知データ（テストデータ）で精度が大きく低下する現象。対策：正則化（L1/L2）・ドロップアウト・クロスバリデーション・訓練データ増量・早期終了。過少学習（Underfitting）は訓練データ・テストデータ両方で低精度（モデルの表現力不足）。バイアス-バリアンストレードオフとして理解される。',
  },
  {
    id: 'BS-056', category: 'business-strategy', categoryName: 'ビジネストラテジ',
    difficulty: 3, year: 2023, tags: ['ESG','サステナビリティ'],
    question: 'ESG 投資における「G（ガバナンス）」に関連する評価項目として正しいものはどれか。',
    choices: [
      { index: 0, text: '温室効果ガス排出量削減・再生可能エネルギーの使用率' },
      { index: 1, text: '取締役会の独立性・経営の透明性・コンプライアンス体制・株主権利保護' },
      { index: 2, text: '労働安全衛生・ダイバーシティ推進・サプライチェーンの人権配慮' },
      { index: 3, text: '製品の安全性・顧客満足度・地域社会への貢献活動' },
    ],
    answer: 1,
    explanation: 'ESG（Environmental・Social・Governance）投資：財務情報に加えて企業の環境・社会・ガバナンスへの取り組みを評価する投資手法。E（環境）：CO₂削減・生物多様性・水資源管理。S（社会）：労働環境・人権・多様性・地域貢献。G（ガバナンス）：取締役会の独立性・経営透明性・腐敗防止・株主権利・情報開示。機関投資家が ESG スコアを投資判断に組み込む動きが拡大。SDGs と関連付けて語られることが多い。',
  },
];
QUESTIONS_DATA.push(...QUESTIONS_DATA_PHASE17);

/* =====================================================
   QUESTIONS_DATA_PHASE18 — 70問
   PM×10[049-058] SD×10[101-110] DB×10[108-117]
   ALG×10[115-124] NET×10[115-124] LG×10[101-110]
   OS×10[111-120]
   ===================================================== */
const QUESTIONS_DATA_PHASE18 = [

  // ─── プロジェクト管理 PM-049〜058 ────────────────────
  {
    id: 'PM-049', category: 'project-mgmt', categoryName: 'プロジェクト管理',
    difficulty: 2, year: 2022, tags: ['スコープ管理','WBS'],
    question: 'WBS（Work Breakdown Structure）の最小単位「ワークパッケージ」の説明として正しいものはどれか。',
    choices: [
      { index: 0, text: 'プロジェクト全体の概要を示す最上位の成果物' },
      { index: 1, text: 'スケジュールやコストを見積もれる最小レベルの成果物の集合' },
      { index: 2, text: 'リスクの最小管理単位を示す台帳' },
      { index: 3, text: 'チームメンバーの個人タスクを記録したリスト' },
    ],
    answer: 1,
    explanation: 'ワークパッケージ：WBS の最下位レベルの要素。具体的な成果物・作業の集合で、担当者・期間・コストを見積もれる粒度に分解される。コントロールアカウント（管理勘定）に集約されてコスト・スケジュール・スコープの管理単位となる。WBS は成果物指向（プロセスではなく成果物を階層分解する）。',
  },
  {
    id: 'PM-050', category: 'project-mgmt', categoryName: 'プロジェクト管理',
    difficulty: 3, year: 2023, tags: ['EVM','コスト管理'],
    question: 'プロジェクトで PV=500万円、EV=400万円、AC=480万円 のとき、CPI と SPI として正しいものはどれか。',
    choices: [
      { index: 0, text: 'CPI=0.83、SPI=0.80' },
      { index: 1, text: 'CPI=0.80、SPI=0.83' },
      { index: 2, text: 'CPI=1.25、SPI=0.83' },
      { index: 3, text: 'CPI=1.20、SPI=1.25' },
    ],
    answer: 0,
    explanation: 'CPI（コスト効率指数）= EV ÷ AC = 400 ÷ 480 ≈ 0.83（<1 はコスト超過）。SPI（スケジュール効率指数）= EV ÷ PV = 400 ÷ 500 = 0.80（<1 は進捗遅延）。CV（コスト差異）= EV − AC = 400 − 480 = −80万円。SV（スケジュール差異）= EV − PV = 400 − 500 = −100万円。このプロジェクトはコスト超過かつスケジュール遅延の状態。',
  },
  {
    id: 'PM-051', category: 'project-mgmt', categoryName: 'プロジェクト管理',
    difficulty: 2, year: 2021, tags: ['リスク管理','リスク対応'],
    question: 'プロジェクトリスクの対応戦略として「リスク回避」の説明として正しいものはどれか。',
    choices: [
      { index: 0, text: 'リスクが発生した場合に備えて予備費を準備する' },
      { index: 1, text: 'リスクが存在する活動をやめるか方針を変更し、発生確率ゼロを目指す' },
      { index: 2, text: 'リスクの影響を保険や契約で第三者に移転する' },
      { index: 3, text: 'リスクを受け入れ、発生した場合の対処だけを計画する' },
    ],
    answer: 1,
    explanation: 'リスク対応戦略（脅威）の 4 つ：①回避（Avoid）：リスクのある活動を廃止して発生確率をゼロにする（例：テスト不足リスク回避のためリリース延期）②転嫁（Transfer）：保険・契約で第三者に移転③軽減（Mitigate）：発生確率や影響を低減する④受容（Accept）：能動的（予備費設定）または受動的（何もしない）に受け入れる。',
  },
  {
    id: 'PM-052', category: 'project-mgmt', categoryName: 'プロジェクト管理',
    difficulty: 3, year: 2022, tags: ['品質管理','管理図'],
    question: '品質管理において「管理図（コントロールチャート）」の主な用途として正しいものはどれか。',
    choices: [
      { index: 0, text: '複数の品質問題の発生頻度を棒グラフで示して優先順位を決める' },
      { index: 1, text: 'プロセスが安定しているか、特殊原因による変動がないかを時系列で監視する' },
      { index: 2, text: '原因と結果の関係を魚の骨のような図で整理する' },
      { index: 3, text: '要因間の相関関係を散布図で視覚化する' },
    ],
    answer: 1,
    explanation: '管理図（シューハート管理図）：時系列データを中心線（CL）・上方管理限界（UCL）・下方管理限界（LCL）と共にプロットし、プロセスが統計的管理状態にあるか確認する品質管理ツール。管理限界外への逸脱や連続するパターン（ランダム性の欠如）で異常を検知する。なお、パレート図（棒グラフ＋累積折れ線）は問題の優先順位付けに使う。',
  },
  {
    id: 'PM-053', category: 'project-mgmt', categoryName: 'プロジェクト管理',
    difficulty: 2, year: 2023, tags: ['コミュニケーション管理'],
    question: 'PMBOK のコミュニケーション管理計画で「コミュニケーション要求事項」として正しいものはどれか。',
    choices: [
      { index: 0, text: 'プロジェクトで使用するソフトウェアツールの一覧' },
      { index: 1, text: 'ステークホルダが必要とする情報の内容・形式・頻度・責任者などの取り決め' },
      { index: 2, text: 'チームメンバーのスキルセットと役割分担' },
      { index: 3, text: 'プロジェクト予算の内訳と承認フロー' },
    ],
    answer: 1,
    explanation: 'コミュニケーション管理計画：誰が誰に・何を・いつ・どの形式で・どの手段で情報を提供するかを定める計画書。コミュニケーション要求事項はステークホルダ登録簿と組み合わせて作成する。通信チャネル数の計算（n(n-1)/2）でコミュニケーションの複雑さを把握する。過剰なコミュニケーションはプロジェクト効率を下げる。',
  },
  {
    id: 'PM-054', category: 'project-mgmt', categoryName: 'プロジェクト管理',
    difficulty: 3, year: 2021, tags: ['スクラム','アジャイル'],
    question: 'スクラムにおける「スプリントレビュー」の説明として正しいものはどれか。',
    choices: [
      { index: 0, text: 'チームが自分たちのプロセスを振り返り、改善アクションを特定するミーティング' },
      { index: 1, text: 'スプリントで完成した成果物をステークホルダに示し、フィードバックを得るミーティング' },
      { index: 2, text: '毎日 15 分で進捗・障害・計画を共有するデイリーミーティング' },
      { index: 3, text: 'スプリントバックログに取り込む作業を見積もるミーティング' },
    ],
    answer: 1,
    explanation: 'スクラムの 4 つのイベント：①スプリントプランニング（バックログ選択・計画）②デイリースクラム（15分の同期・障害共有）③スプリントレビュー（完成インクリメントのデモ・ステークホルダフィードバック）④スプリントレトロスペクティブ（チームのプロセス振り返り・改善）。スプリントレビューはインスペクション＆アダプテーションのプロダクト側の側面。',
  },
  {
    id: 'PM-055', category: 'project-mgmt', categoryName: 'プロジェクト管理',
    difficulty: 2, year: 2022, tags: ['クリティカルパス','フロート'],
    question: 'プロジェクトのアローダイアグラムでクリティカルパス上の作業のフロート（余裕時間）として正しいものはどれか。',
    choices: [
      { index: 0, text: 'プロジェクト全体の所要期間と同じ値' },
      { index: 1, text: 'ゼロ（0）' },
      { index: 2, text: '各作業の所要時間の平均値' },
      { index: 3, text: 'PERT の標準偏差値' },
    ],
    answer: 1,
    explanation: 'フロート（スラック）：プロジェクト全体の完了日を遅らせることなく許容できる遅延時間。クリティカルパス上の作業はフロート = 0（遅延がそのままプロジェクト全体の遅延につながる）。トータルフロート = 最遅開始時刻 − 最早開始時刻（または最遅完了 − 最早完了）。フリーフロートは後続作業の最早開始日を遅らせずに許容できる遅延。',
  },
  {
    id: 'PM-056', category: 'project-mgmt', categoryName: 'プロジェクト管理',
    difficulty: 3, year: 2023, tags: ['プロジェクト憲章','立上げ'],
    question: 'プロジェクト憲章（Project Charter）に通常含まれるものとして正しいものはどれか。',
    choices: [
      { index: 0, text: '詳細なスケジュール（ガントチャート）と全作業の工数見積もり' },
      { index: 1, text: 'プロジェクトの目的・スコープ概要・主要マイルストーン・予算・主要ステークホルダ・プロジェクトマネージャの権限' },
      { index: 2, text: 'ソフトウェアのクラス図とシーケンス図' },
      { index: 3, text: 'テスト計画書と不具合管理台帳' },
    ],
    answer: 1,
    explanation: 'プロジェクト憲章（Project Charter）：プロジェクトを正式に承認する文書。立上げプロセス群で作成される。含まれる内容：プロジェクトの目的・目標・高レベルスコープ・主要デリバラブル・主要マイルストーン・予算概算・主要リスク・主要ステークホルダ・プロジェクトマネージャの権限・スポンサー署名。詳細計画はプロジェクトマネジメント計画書で行う。',
  },
  {
    id: 'PM-057', category: 'project-mgmt', categoryName: 'プロジェクト管理',
    difficulty: 2, year: 2021, tags: ['調達管理','契約'],
    question: '固定価格契約（FFP：Firm Fixed Price）の特徴として正しいものはどれか。',
    choices: [
      { index: 0, text: '実際の費用＋固定報酬を支払う契約で、売主（ベンダ）のリスクが最も小さい' },
      { index: 1, text: '契約時に合意した固定価格で購入。超過コストリスクは全て売主が負担する' },
      { index: 2, text: '費用の実費に加えインセンティブを支払う。費用節約の動機が生まれる' },
      { index: 3, text: '購入量に応じた単価×実績数量で支払う契約方式' },
    ],
    answer: 1,
    explanation: '契約タイプとリスク分担：FFP（固定価格契約）：価格固定、超過コストリスクは売主負担→買主リスク最小・売主リスク最大。CPFF（実費＋固定報酬）：実費を買主が全額負担＋固定報酬→買主リスク最大・売主リスク最小。FPIF（固定価格インセンティブ）：固定価格ベースにコスト節約で双方がインセンティブ共有。T&M（実費精算）：工数×単価で支払い、スコープ不明確な案件に使用。',
  },
  {
    id: 'PM-058', category: 'project-mgmt', categoryName: 'プロジェクト管理',
    difficulty: 3, year: 2022, tags: ['変更管理','統合変更管理'],
    question: 'PMBOK の統合変更管理プロセスの説明として正しいものはどれか。',
    choices: [
      { index: 0, text: 'プロジェクト憲章とプロジェクト計画書を最初に作成する立上げプロセス' },
      { index: 1, text: '変更要求をすべて評価・承認・却下し、承認された変更を計画に反映する統合的なプロセス' },
      { index: 2, text: 'プロジェクト終結時にステークホルダの最終承認を得るプロセス' },
      { index: 3, text: 'コストとスケジュールの差異を自動的に修正するツール' },
    ],
    answer: 1,
    explanation: '統合変更管理（Perform Integrated Change Control）：プロジェクトの全変更要求を一元的に評価・承認・却下し、承認済み変更をプロジェクト計画書やベースラインに反映するプロセス。CCB（Change Control Board：変更管理委員会）が審査を担当。スコープクリープを防ぎ、プロジェクトの整合性を維持する。変更のたびに影響分析・リスク評価・コスト見積もりを行う。',
  },

  // ─── ソフトウェア開発 SD-101〜110 ────────────────────
  {
    id: 'SD-101', category: 'software-dev', categoryName: 'ソフトウェア開発',
    difficulty: 2, year: 2022, tags: ['テスト技法','ホワイトボックステスト'],
    question: 'ホワイトボックステストにおける「分岐網羅（ブランチカバレッジ）」の説明として正しいものはどれか。',
    choices: [
      { index: 0, text: '全ての命令文を少なくとも 1 回実行するテスト条件' },
      { index: 1, text: '全ての分岐（真・偽の両方の経路）を少なくとも 1 回実行するテスト条件' },
      { index: 2, text: '入力値の境界値と同値クラスをテストする手法' },
      { index: 3, text: '全ての条件の論理式の真偽組み合わせを網羅するテスト条件' },
    ],
    answer: 1,
    explanation: 'テストカバレッジ基準：命令網羅（C0）：全命令を少なくとも 1 回実行（最も弱い基準）。分岐網羅（C1）：if 文の真・偽の全分岐を実行。条件網羅（C2）：各条件（サブ条件）の真偽を全て実行。複数条件網羅（MCC）：全ての条件の組み合わせを実行（最も強い基準）。ブラックボックステストの技法は同値分割・境界値分析・デシジョンテーブルなど。',
  },
  {
    id: 'SD-102', category: 'software-dev', categoryName: 'ソフトウェア開発',
    difficulty: 3, year: 2023, tags: ['デザインパターン','GoF'],
    question: 'GoF デザインパターンの「Observer（オブザーバ）」パターンの説明として正しいものはどれか。',
    choices: [
      { index: 0, text: 'オブジェクト生成のインタフェースを定義し、サブクラスでインスタンス化するクラスを決定する' },
      { index: 1, text: 'オブジェクトの状態変化を監視し、依存するオブジェクト群に自動通知する' },
      { index: 2, text: '既存クラスに新機能を動的に追加する' },
      { index: 3, text: '複雑なオブジェクト生成のステップを定義し、同じプロセスで異なる表現を生成できるようにする' },
    ],
    answer: 1,
    explanation: 'Observer パターン（MVC の Model→View 通知に使用）：Subject（被観察者）の状態が変化すると、登録済みの Observer（観察者）に自動的に通知する。Subject は Observer を知らなくてよい（疎結合）。Java の EventListener・JavaScript の addEventListener・RxJS（リアクティブプログラミング）が実装例。GoF の行動パターンの一つ。',
  },
  {
    id: 'SD-103', category: 'software-dev', categoryName: 'ソフトウェア開発',
    difficulty: 2, year: 2021, tags: ['バージョン管理','Git'],
    question: 'Git のブランチ戦略「Git Flow」の主な特徴として正しいものはどれか。',
    choices: [
      { index: 0, text: 'mainブランチのみを使用し、プルリクエストで直接マージする最もシンプルな戦略' },
      { index: 1, text: 'main・develop・feature・release・hotfix の各ブランチ役割を明確に定義した戦略' },
      { index: 2, text: '全メンバーが直接 main ブランチに小さなコミットを頻繁にプッシュするトランクベース開発' },
      { index: 3, text: '本番環境ごとに独立したブランチを持ち、環境別にコードを管理する戦略' },
    ],
    answer: 1,
    explanation: 'Git Flow（Vincent Driessen 提唱）：main（本番）・develop（開発統合）・feature/*（機能開発）・release/*（リリース準備）・hotfix/*（緊急修正）の 5 種類のブランチを定義した戦略。大規模プロジェクト・明確なリリースサイクルに向く。GitHub Flow はシンプル版（main＋feature のみ）。トランクベース開発は CI/CD に最適。',
  },
  {
    id: 'SD-104', category: 'software-dev', categoryName: 'ソフトウェア開発',
    difficulty: 3, year: 2022, tags: ['コンテナ','Kubernetes'],
    question: 'Kubernetes（K8s）における「Pod」の説明として正しいものはどれか。',
    choices: [
      { index: 0, text: 'Kubernetes クラスタ全体のコントロールプレーンを担う管理サーバ' },
      { index: 1, text: 'Kubernetes の最小デプロイ単位。1つ以上のコンテナと共有ネットワーク・ストレージを持つ' },
      { index: 2, text: 'コンテナイメージを保存するプライベートレジストリ' },
      { index: 3, text: '外部トラフィックをクラスタ内サービスに転送するロードバランサ' },
    ],
    answer: 1,
    explanation: 'Kubernetes の主要リソース：Pod：最小デプロイ単位。1つ以上のコンテナが同一ネットワーク（同一IPアドレス）・ストレージを共有。Deployment：Pod のレプリカ数管理・ローリングアップデート。Service：Pod 群へのネットワークアクセスの抽象化・ロードバランシング。Ingress：外部 HTTP(S) トラフィックをサービスにルーティング。ConfigMap/Secret：設定情報・秘密情報の管理。',
  },
  {
    id: 'SD-105', category: 'software-dev', categoryName: 'ソフトウェア開発',
    difficulty: 2, year: 2023, tags: ['セキュリティ','OWASP'],
    question: 'OWASP Top 10 に含まれるWebアプリケーションの脆弱性として常に上位に挙げられるものはどれか。',
    choices: [
      { index: 0, text: '色覚バリアフリー対応不足・レスポンシブデザインの欠如' },
      { index: 1, text: 'インジェクション・壊れた認証・セキュリティの設定ミス・XSS（クロスサイトスクリプティング）' },
      { index: 2, text: 'データベースのインデックス設計の問題・クエリの非最適化' },
      { index: 3, text: 'フォントのレンダリング問題・ブラウザ互換性の欠如' },
    ],
    answer: 1,
    explanation: 'OWASP Top 10（2021年版）の主要項目：①壊れたアクセス制御②暗号化の失敗③インジェクション（SQL・OS・LDAP）④安全でない設計⑤セキュリティの設定ミス⑥脆弱で時代遅れなコンポーネント⑦識別と認証の失敗⑧ソフトウェアとデータの整合性の失敗⑨セキュリティログと監視の失敗⑩サーバサイドリクエストフォージェリ（SSRF）。Webアプリ開発で考慮必須のリスト。',
  },
  {
    id: 'SD-106', category: 'software-dev', categoryName: 'ソフトウェア開発',
    difficulty: 3, year: 2021, tags: ['マイクロサービス','API設計'],
    question: 'マイクロサービス間通信の「サーキットブレーカー」パターンの説明として正しいものはどれか。',
    choices: [
      { index: 0, text: '暗号化ライブラリを使ってサービス間の通信を保護する仕組み' },
      { index: 1, text: '障害が発生したサービスへの呼び出しを一定時間遮断し、システム全体への障害伝播を防ぐパターン' },
      { index: 2, text: 'サービス同士が直接通信せずメッセージキューを経由して非同期通信する仕組み' },
      { index: 3, text: 'API のレート制限（Rate Limiting）を実現するためのフロー制御パターン' },
    ],
    answer: 1,
    explanation: 'サーキットブレーカー（Circuit Breaker）パターン：電気回路の安全装置に例えた設計パターン。依存サービスへの呼び出しが連続で失敗すると回路を「オープン」状態にし、一定時間リクエストを即座に失敗として返す（フォールバック処理）→依存サービスの回復時間を確保し、スレッドプール枯渇などのカスケード障害を防ぐ。Netflix Hystrix・Resilience4j が実装例。',
  },
  {
    id: 'SD-107', category: 'software-dev', categoryName: 'ソフトウェア開発',
    difficulty: 2, year: 2022, tags: ['CI/CD','DevOps'],
    question: 'CI（継続的インテグレーション）で自動化されるタスクとして適切でないものはどれか。',
    choices: [
      { index: 0, text: 'コードのコンパイル・ビルド' },
      { index: 1, text: '自動テストの実行（ユニットテスト・統合テスト）' },
      { index: 2, text: '静的コード解析・コードカバレッジ計測' },
      { index: 3, text: '顧客向けプロダクトデモの実施' },
    ],
    answer: 3,
    explanation: 'CI（継続的インテグレーション）で自動化される主なタスク：コードのビルド・コンパイル・ユニットテスト/統合テスト実行・静的コード解析（linting・SAST）・コードカバレッジ計測・依存関係の脆弱性チェック・アーティファクト（ビルド成果物）の生成・通知。顧客向けデモは人間が手動で行うプロセスであり CI の自動化対象ではない。CD（継続的デリバリー/デプロイ）はステージングや本番へのデプロイを含む。',
  },
  {
    id: 'SD-108', category: 'software-dev', categoryName: 'ソフトウェア開発',
    difficulty: 3, year: 2023, tags: ['アーキテクチャ','クリーンアーキテクチャ'],
    question: 'クリーンアーキテクチャの「依存性逆転の原則（DIP）」の説明として正しいものはどれか。',
    choices: [
      { index: 0, text: '上位モジュールが下位モジュールに直接依存し、実装の詳細を直接呼び出す設計' },
      { index: 1, text: '上位モジュールも下位モジュールも抽象（インタフェース）に依存し、実装の詳細に依存しない設計原則' },
      { index: 2, text: 'クラスは 1 つの責務のみを持つべきという設計原則' },
      { index: 3, text: 'ソフトウェアは拡張に対してオープン、修正に対してクローズであるべき設計原則' },
    ],
    answer: 1,
    explanation: 'SOLID 原則の D（依存性逆転の原則：DIP）：①高レベルモジュールは低レベルモジュールに依存すべきでない（両者ともに抽象に依存せよ）②抽象は詳細に依存すべきでない（詳細が抽象に依存せよ）。クリーンアーキテクチャでビジネスロジックが DB・UI・外部 API などの実装詳細に依存しないようにインタフェースを介することで、テスト容易性・交換可能性が向上する。',
  },
  {
    id: 'SD-109', category: 'software-dev', categoryName: 'ソフトウェア開発',
    difficulty: 2, year: 2021, tags: ['オブジェクト指向','継承'],
    question: 'オブジェクト指向の「ポリモーフィズム（多態性）」の説明として正しいものはどれか。',
    choices: [
      { index: 0, text: 'クラスのデータと処理をひとつの単位にまとめて外部から隠す概念' },
      { index: 1, text: '同じメソッド名で、呼び出すオブジェクトに応じた異なる処理が実行される仕組み' },
      { index: 2, text: '既存クラスの属性・メソッドを引き継いで新しいクラスを作る仕組み' },
      { index: 3, text: '複数の属性とメソッドを一つの設計図（クラス）にまとめる概念' },
    ],
    answer: 1,
    explanation: 'OOP の 3 大原則：カプセル化（データと処理を 1 つの単位にまとめ外部からの直接アクセスを制限）・継承（既存クラスの特性を引き継いだサブクラス生成）・ポリモーフィズム（多態性：同一のインタフェース/メソッド名で、実行時の実際のオブジェクト型に応じた異なる処理を呼び出す仕組み）。例：動物クラスの speak() を犬・猫・鳥で上書き（オーバーライド）し、それぞれ異なる動作をする。',
  },
  {
    id: 'SD-110', category: 'software-dev', categoryName: 'ソフトウェア開発',
    difficulty: 3, year: 2022, tags: ['要件定義','ユースケース'],
    question: 'ユースケース図の「アクター」の説明として正しいものはどれか。',
    choices: [
      { index: 0, text: 'システム内部のデータベースやモジュール' },
      { index: 1, text: 'システムの外部にいてシステムと相互作用する人・外部システム・タイマーなど' },
      { index: 2, text: 'システム内部のビジネスルールを記述した仕様書' },
      { index: 3, text: 'ユースケース間の依存関係を示す矢印' },
    ],
    answer: 1,
    explanation: 'ユースケース図の要素：アクター（Actor）：システムの外部に存在し、システムと相互作用する人・組織・外部システム・タイマー。棒人間で表現。ユースケース（Use Case）：システムが提供する機能・サービス（楕円で表現）。関係：関連（アクター↔ユースケース）・包含（≪include≫：必ず実行するサブフロー）・拡張（≪extend≫：条件付きで追加される動作）・汎化（継承）。',
  },

  // ─── データベース DB-108〜117 ────────────────────────
  {
    id: 'DB-108', category: 'database', categoryName: 'データベース',
    difficulty: 2, year: 2022, tags: ['SQL','集計'],
    question: '次の SQL でエラーが起きる理由として正しいものはどれか。\nSELECT department, employee_name, MAX(salary) FROM employees GROUP BY department;',
    choices: [
      { index: 0, text: 'MAX 関数は GROUP BY と一緒に使えない' },
      { index: 1, text: 'GROUP BY に含まれない employee_name を SELECT 列に指定しているため' },
      { index: 2, text: 'FROM 句の前に WHERE 句が必要なため' },
      { index: 3, text: 'MAX(salary) の引数はカラム名ではなく定数でなければならない' },
    ],
    answer: 1,
    explanation: 'GROUP BY の規則：SELECT 列に指定できるのは、①GROUP BY の列②集計関数（MAX・MIN・AVG・SUM・COUNT）のみ。employee_name は GROUP BY に含まれておらず、集計関数でもないため各グループの代表値が一意に決まらずエラーになる。正しい SQL には MAX(employee_name) や HAVING などを使うか、GROUP BY に employee_name を追加する。',
  },
  {
    id: 'DB-109', category: 'database', categoryName: 'データベース',
    difficulty: 3, year: 2023, tags: ['インデックス','実行計画'],
    question: 'データベースでインデックスが使われない（フルスキャンになりやすい）ケースとして正しいものはどれか。',
    choices: [
      { index: 0, text: 'WHERE age = 30 （等値検索）' },
      { index: 1, text: 'WHERE name LIKE \'%田中%\' （前方一致でない LIKE 検索）' },
      { index: 2, text: 'WHERE created_at BETWEEN \'2024-01-01\' AND \'2024-12-31\' （範囲検索）' },
      { index: 3, text: 'WHERE id IN (1, 2, 3) （IN 検索）' },
    ],
    answer: 1,
    explanation: 'インデックスが使われにくいケース：①前方一致でない LIKE（%xxx% や %xxx）②インデックス列を関数・演算で加工（WHERE YEAR(created_at) = 2024 など）③OR 条件④NULL 比較（IS NULL/IS NOT NULL）⑤低カーディナリティ（値の種類が少ない列）。B+木インデックスは前方一致（xxx%）・等値・範囲検索では有効。オプティマイザの実行計画（EXPLAIN）で確認できる。',
  },
  {
    id: 'DB-110', category: 'database', categoryName: 'データベース',
    difficulty: 3, year: 2021, tags: ['トランザクション','ACID'],
    question: 'トランザクションの「原子性（Atomicity）」の説明として正しいものはどれか。',
    choices: [
      { index: 0, text: 'トランザクション実行中、他のトランザクションの中間結果が見えないこと' },
      { index: 1, text: 'トランザクション内の全操作が完全に実行されるか、全く実行されないか（全か無か）の性質' },
      { index: 2, text: 'コミット後のデータ変更が永続的に保存されること' },
      { index: 3, text: 'トランザクション前後でデータが整合性の制約を満たすこと' },
    ],
    answer: 1,
    explanation: 'ACID 特性：A（原子性：Atomicity）：全操作が成功するか全失敗かの「全か無か」。失敗時は ROLLBACK で元に戻す。C（一貫性：Consistency）：制約・外部キーなどのルールがトランザクション前後で満たされる。I（独立性：Isolation）：同時実行トランザクションが互いの中間状態を見られない。D（永続性：Durability）：COMMIT 後のデータはシステム障害後も永続的に保存される。',
  },
  {
    id: 'DB-111', category: 'database', categoryName: 'データベース',
    difficulty: 2, year: 2022, tags: ['NoSQL','MongoDB'],
    question: 'NoSQL データベースの「ドキュメント指向」データベースの特徴として正しいものはどれか。',
    choices: [
      { index: 0, text: 'キーと値のペアのみを格納し、シンプルな構造を持つ' },
      { index: 1, text: 'JSON 形式などの柔軟なスキーマレスドキュメントを格納し、ネストした構造を扱いやすい' },
      { index: 2, text: 'グラフ構造（ノード・エッジ）でデータとその関係を表現する' },
      { index: 3, text: '列族（カラムファミリ）でデータを管理し、大規模な読み書きに最適化されている' },
    ],
    answer: 1,
    explanation: 'NoSQL の主な分類：①ドキュメント指向（MongoDB・Couchbase）：JSON/BSON 形式のドキュメントを格納。スキーマレスで柔軟②キーバリュー（Redis・DynamoDB）：シンプルな KV ペア、高速③ワイドカラム（Cassandra・HBase）：行と列で管理、大規模分散処理④グラフ（Neo4j）：ノードとエッジでの複雑な関係性クエリ。選択はユースケースによる。CAP 定理で AP 優先のものが多い。',
  },
  {
    id: 'DB-112', category: 'database', categoryName: 'データベース',
    difficulty: 3, year: 2023, tags: ['正規化','第3正規形'],
    question: '第 3 正規形（3NF）の条件として正しいものはどれか。',
    choices: [
      { index: 0, text: '繰り返しグループが除去されており、全属性が原子値をもつ（第 1 正規形）' },
      { index: 1, text: '第 2 正規形であり、かつ推移的関数従属が存在しない（非キー属性が他の非キー属性に依存しない）' },
      { index: 2, text: '第 1 正規形であり、かつ全非キー属性が主キーに完全関数従属する' },
      { index: 3, text: '全ての行列式（関数従属の左辺）がスーパーキーである' },
    ],
    answer: 1,
    explanation: '正規化の段階：1NF：繰り返しグループをなくし全属性が原子値。2NF：1NF＋全非キー属性が複合主キーに完全関数従属（部分関数従属の除去）。3NF：2NF＋非キー属性間の推移的関数従属を除去（非キー属性が主キー以外に依存しない）。BCNF：3NF より厳格で全ての関数従属の決定項がスーパーキー。実務では通常 3NF で正規化を止め、性能を考慮して意図的に非正規化することもある。',
  },
  {
    id: 'DB-113', category: 'database', categoryName: 'データベース',
    difficulty: 2, year: 2021, tags: ['レプリケーション','可用性'],
    question: 'データベースレプリケーションの「マスタ・スレーブ構成」の特徴として正しいものはどれか。',
    choices: [
      { index: 0, text: '全ノードへの書き込みが同時に行われ、すべてのノードが書き込みを受け付ける' },
      { index: 1, text: 'マスタへの書き込みがスレーブに非同期または同期で複製される。読み取りはスレーブに分散できる' },
      { index: 2, text: 'シャーディングと同義であり、データを水平分割して複数のノードに格納する' },
      { index: 3, text: '全ノードが対等でリーダー選出なしに書き込み・読み取りを処理する方式' },
    ],
    answer: 1,
    explanation: 'マスタ・スレーブレプリケーション：マスタ（プライマリ）が書き込みを処理し、スレーブ（レプリカ）にデータを複製する。読み取りをスレーブに分散して負荷を軽減できる（Read Scaling）。同期レプリケーション（強整合性・遅延増）vs 非同期レプリケーション（高速・一時的不整合リスク）。マスタ障害時はフェイルオーバーでスレーブを昇格させる（HA 構成）。MySQL・PostgreSQL・Redis に実装。',
  },
  {
    id: 'DB-114', category: 'database', categoryName: 'データベース',
    difficulty: 3, year: 2022, tags: ['ウィンドウ関数','SQL'],
    question: 'SQL のウィンドウ関数（OVER 句）の説明として正しいものはどれか。',
    choices: [
      { index: 0, text: 'GROUP BY と同様にグループをまとめ、集計結果のみを返す' },
      { index: 1, text: 'パーティション内の各行に対して集計結果を計算しつつ、元の行も保持する' },
      { index: 2, text: 'テーブルの一部のカラムのみを表示するビューを作成する' },
      { index: 3, text: 'トランザクション内でのみ使えるキャッシュ機構' },
    ],
    answer: 1,
    explanation: 'ウィンドウ関数（分析関数）：OVER(PARTITION BY col ORDER BY col) で定義されるウィンドウに対して集計を行いつつ、行を削減しない（GROUP BY と異なる点）。主な関数：ROW_NUMBER()（行番号）・RANK()/DENSE_RANK()（順位）・LAG()/LEAD()（前後の行の値）・SUM/AVG/COUNT（累積集計・移動平均）。例：各部門内の給与ランキング付けや前月比計算に活用。',
  },
  {
    id: 'DB-115', category: 'database', categoryName: 'データベース',
    difficulty: 2, year: 2023, tags: ['バックアップ','復旧'],
    question: 'データベースのポイントインタイムリカバリ（PITR）の説明として正しいものはどれか。',
    choices: [
      { index: 0, text: 'データベースを最新の完全バックアップ時点にのみ復元する手法' },
      { index: 1, text: 'フルバックアップとトランザクションログを使って任意の時点の状態に復元する手法' },
      { index: 2, text: 'データベースを別のサーバにリアルタイムでミラーリングする手法' },
      { index: 3, text: 'テーブル単位でのバックアップと個別テーブルの復元を行う手法' },
    ],
    answer: 1,
    explanation: 'ポイントインタイムリカバリ（PITR）：完全バックアップ＋WAL（Write-Ahead Log）やバイナリログを使い、特定の時刻（例：ランサムウェア感染直前）の状態にデータベースを復元する手法。RTO（目標復旧時間）とRPO（目標復旧時点）の両方を最小化できる。PostgreSQL の WAL・MySQL の binlog・SQL Server の LSN（ログシーケンス番号）で実現。',
  },
  {
    id: 'DB-116', category: 'database', categoryName: 'データベース',
    difficulty: 3, year: 2021, tags: ['OLAP','データ分析'],
    question: 'OLAP（Online Analytical Processing）の説明として正しいものはどれか。',
    choices: [
      { index: 0, text: '日常の業務処理（注文・入金・在庫更新など）を高速に処理するシステム（= OLTP）' },
      { index: 1, text: '大量の履歴データを多次元的に分析するシステム。スライス・ダイス・ドリルダウンが主な操作' },
      { index: 2, text: 'リアルタイムでデータをストリーム処理して即座に結果を返すシステム' },
      { index: 3, text: 'ファイルシステムのキャッシュを管理してディスクアクセスを最適化するシステム' },
    ],
    answer: 1,
    explanation: 'OLAP（Online Analytical Processing）：経営意思決定支援向けに大量の履歴データを多次元（ディメンション）で集計・分析するシステム。主な操作：スライス（1次元で絞り込み）・ダイス（複数次元で絞り込み）・ドリルダウン/アップ（詳細化/集約）・ロールアップ・ピボット。MOLAP（多次元キューブ）・ROLAP（リレーショナルDB）・HOLAP（ハイブリッド）の実装形態がある。対比：OLTP（トランザクション処理）。',
  },
  {
    id: 'DB-117', category: 'database', categoryName: 'データベース',
    difficulty: 2, year: 2022, tags: ['SQL','サブクエリ'],
    question: 'SQL の相関サブクエリ（Correlated Subquery）の特徴として正しいものはどれか。',
    choices: [
      { index: 0, text: '外側クエリと独立して一度だけ実行されるサブクエリ' },
      { index: 1, text: '外側クエリの行ごとに参照して実行されるサブクエリ（行ごとに再評価される）' },
      { index: 2, text: 'FROM 句に書くインラインビュー（派生テーブル）のこと' },
      { index: 3, text: 'EXISTS 句でのみ使用できるサブクエリ' },
    ],
    answer: 1,
    explanation: '相関サブクエリ：外側クエリの現在処理中の行の値を参照しながら、外側クエリの各行に対して繰り返し実行されるサブクエリ。例：各社員の給与がその部門の平均給与より高いかを調べる SELECT。パフォーマンス上の問題が起きやすいため、JOIN・ウィンドウ関数・WITH 句（CTE）で書き換えると効率的な場合が多い。EXISTS 句と相性がよい。',
  },

  // ─── アルゴリズム ALG-115〜124 ────────────────────────
  {
    id: 'ALG-115', category: 'algorithm', categoryName: 'アルゴリズム',
    difficulty: 2, year: 2023, tags: ['ソート','選択ソート'],
    question: '選択ソートの時間計算量（最良・平均・最悪）として正しいものはどれか。',
    choices: [
      { index: 0, text: '最良 O(n)、平均 O(n log n)、最悪 O(n²)' },
      { index: 1, text: '最良・平均・最悪ともに O(n²)' },
      { index: 2, text: '最良・平均・最悪ともに O(n log n)' },
      { index: 3, text: '最良 O(1)、平均 O(n)、最悪 O(n²)' },
    ],
    answer: 1,
    explanation: '選択ソート：未整列部分から最小値を選んで先頭と交換する操作を繰り返す。比較回数は常に n(n-1)/2 のため、最良・平均・最悪すべて O(n²)。空間計算量 O(1)（インプレース）。不安定ソート（等しい要素の順序が保たれない）。挿入ソートは最良 O(n)（すでに整列済みの場合）のため、既にほぼ整列済みのデータには挿入ソートが有利。',
  },
  {
    id: 'ALG-116', category: 'algorithm', categoryName: 'アルゴリズム',
    difficulty: 3, year: 2022, tags: ['トポロジカルソート','DAG'],
    question: 'トポロジカルソートが適用できる条件として正しいものはどれか。',
    choices: [
      { index: 0, text: '重み付き無向グラフ' },
      { index: 1, text: '有向非巡回グラフ（DAG：Directed Acyclic Graph）' },
      { index: 2, text: '連結な無向グラフ（全頂点が接続）' },
      { index: 3, text: '負のエッジを含む有向グラフ' },
    ],
    answer: 1,
    explanation: 'トポロジカルソート：DAG（有向非巡回グラフ）の頂点を、辺の方向を壊さない線形順序に並べる操作。アルゴリズム：カーンのアルゴリズム（入次数 0 の頂点をキューで処理）・DFS 後順の逆。用途：タスクの依存関係解決（Makefile・パッケージ管理・コンパイル順序）。閉路（サイクル）があると不可能→閉路検出にも使える。',
  },
  {
    id: 'ALG-117', category: 'algorithm', categoryName: 'アルゴリズム',
    difficulty: 3, year: 2021, tags: ['動的計画法','最長増加部分列'],
    question: '動的計画法で解く「最長増加部分列（LIS）」問題の計算量として正しいものはどれか。',
    choices: [
      { index: 0, text: 'O(n) で解ける' },
      { index: 1, text: 'DP の素朴実装は O(n²)、二分探索を使うと O(n log n) まで改善できる' },
      { index: 2, text: '常に O(2ⁿ) が必要な NP 困難問題' },
      { index: 3, text: 'O(n log n) より良い O(n) アルゴリズムが存在する' },
    ],
    answer: 1,
    explanation: 'LIS（最長増加部分列）：dp[i] = インデックス i で終わる LIS の長さと定義し、各 i に対して j < i かつ a[j] < a[i] を探す素朴 DP は O(n²)。patience sorting + 二分探索を使うと O(n log n) に改善できる。配列の LCS・LIS は競技プログラミング頻出問題。応用：ビリヤードの球並べ・株価の最大利益計算。',
  },
  {
    id: 'ALG-118', category: 'algorithm', categoryName: 'アルゴリズム',
    difficulty: 2, year: 2022, tags: ['文字列','KMP法'],
    question: 'KMP（Knuth-Morris-Pratt）法が解決する問題として正しいものはどれか。',
    choices: [
      { index: 0, text: '文字列を任意のソート順に並べ替える' },
      { index: 1, text: 'テキスト文字列の中からパターン文字列を O(n+m) で効率的に検索する' },
      { index: 2, text: '圧縮されたテキストファイルをデコードする' },
      { index: 3, text: '文字列の最長共通部分列（LCS）を求める' },
    ],
    answer: 1,
    explanation: 'KMP 法（文字列検索アルゴリズム）：パターンの「失敗関数（接頭辞と接尾辞の一致情報）」を事前計算し、不一致時に比較位置を適切にスキップすることでナイーブ法 O(nm) を O(n+m) に改善する。Boyer-Moore 法・Rabin-Karp 法（ハッシュ利用）も文字列検索の代表アルゴリズム。Aho-Corasick 法は複数パターンの同時検索に対応。',
  },
  {
    id: 'ALG-119', category: 'algorithm', categoryName: 'アルゴリズム',
    difficulty: 3, year: 2023, tags: ['グラフ','強連結成分'],
    question: 'Kosaraju アルゴリズムで求められるものとして正しいものはどれか。',
    choices: [
      { index: 0, text: '有向グラフの最短経路' },
      { index: 1, text: '有向グラフの強連結成分（SCC：Strongly Connected Components）' },
      { index: 2, text: '無向グラフの最小全域木' },
      { index: 3, text: 'グラフの彩色問題における最少色数' },
    ],
    answer: 1,
    explanation: '強連結成分（SCC）：有向グラフで任意の 2 頂点間を双方向に到達できる頂点の最大部分集合。Kosaraju アルゴリズム：①元グラフで DFS し終了時刻の降順にスタックに積む②グラフを逆向きにする③スタック順に逆グラフで DFS して各 SCC を識別。時間計算量 O(V+E)。Tarjan のアルゴリズムも同等。用途：ウェブページのリンク解析・パッケージの循環依存検出。',
  },
  {
    id: 'ALG-120', category: 'algorithm', categoryName: 'アルゴリズム',
    difficulty: 2, year: 2021, tags: ['ヒープ','優先度キュー'],
    question: '二分ヒープ（Binary Heap）の主な操作の時間計算量として正しいものはどれか。',
    choices: [
      { index: 0, text: '挿入 O(1)、最小値取得 O(n)、削除 O(log n)' },
      { index: 1, text: '挿入 O(log n)、最小値取得 O(1)、削除 O(log n)' },
      { index: 2, text: '挿入 O(n)、最小値取得 O(1)、削除 O(1)' },
      { index: 3, text: '挿入・取得・削除すべて O(1)' },
    ],
    answer: 1,
    explanation: '二分ヒープ（最小ヒープ）：完全二分木で親ノードが子ノード以下になる性質。配列で効率的に実装できる。挿入：末尾追加→上方向に sift-up → O(log n)。最小値取得：根ノードを参照 → O(1)。削除（最小値取り出し）：根と末尾を交換→末尾削除→下方向 sift-down → O(log n)。ヒープソート・ダイクストラ法の優先度キュー・A* アルゴリズムに使用。',
  },
  {
    id: 'ALG-121', category: 'algorithm', categoryName: 'アルゴリズム',
    difficulty: 3, year: 2022, tags: ['計算量','空間複雑度'],
    question: '再帰によるフィボナッチ数列（メモ化なし）の空間計算量（スタック深さ）として正しいものはどれか。',
    choices: [
      { index: 0, text: 'O(2ⁿ)' },
      { index: 1, text: 'O(n)' },
    ...
    answer: 1,
    explanation: '単純再帰フィボナッチの空間計算量：最大再帰深さは n（fib(n)→fib(n-1)→…→fib(1)）のため O(n)。時間計算量は O(2ⁿ)。スタックは 1 つの呼び出しが終わると解放されるため全ノード数 O(2ⁿ) 分が同時に積まれるわけではない。メモ化 DP（トップダウン）は時間 O(n)・空間 O(n)。ボトムアップ DP は時間 O(n)・空間 O(1)（前 2 値のみ保持）。',
  },
  {
    id: 'ALG-122', category: 'algorithm', categoryName: 'アルゴリズム',
    difficulty: 2, year: 2023, tags: ['ハッシュ','暗号ハッシュ'],
    question: '暗号学的ハッシュ関数に求められる性質として正しいものはどれか。',
    choices: [
      { index: 0, text: 'ハッシュ値からメッセージを容易に復元できること（可逆性）' },
      { index: 1, text: '一方向性（ハッシュ値から元データを求めることが計算量的に困難）と衝突耐性（同じハッシュ値を持つ 2 つの異なる入力を見つけることが困難）' },
      { index: 2, text: '同じ入力に対して毎回異なるハッシュ値を生成すること（ランダム性）' },
      { index: 3, text: '任意の入力に対し固定長でなく可変長のハッシュ値を生成すること' },
    ],
    answer: 1,
    explanation: '暗号学的ハッシュ関数の 3 大性質：①一方向性（Pre-image resistance）：ハッシュ値から元データを計算困難②第 2 原像耐性（Second pre-image resistance）：同じハッシュ値を持つ別のデータを見つけること困難③衝突耐性（Collision resistance）：同じハッシュ値を持つ 2 つの入力を見つけること困難。SHA-256・SHA-3 が現在推奨。MD5・SHA-1 は衝突が見つかり非推奨。',
  },
  {
    id: 'ALG-123', category: 'algorithm', categoryName: 'アルゴリズム',
    difficulty: 3, year: 2021, tags: ['貪欲法','ハフマン符号'],
    question: 'ハフマン符号化の説明として正しいものはどれか。',
    choices: [
      { index: 0, text: '全ての文字に同じビット長を割り当てる固定長符号化' },
      { index: 1, text: '出現頻度の高い文字に短いビット列を割り当て、可変長符号化でデータを圧縮する貪欲アルゴリズム' },
      { index: 2, text: '文字列の繰り返しパターンを辞書に登録して圧縮する手法（LZ77/LZ78）' },
      { index: 3, text: '連続して同じ文字が続く場合に回数と文字をセットで表現する圧縮法' },
    ],
    answer: 1,
    explanation: 'ハフマン符号（David A. Huffman 考案）：出現頻度の高い文字に短いコード・頻度の低い文字に長いコードを割り当てる最適な可変長圧縮。二分木（ハフマン木）を最小ヒープで貪欲構築し、各文字への経路（左=0・右=1）がコードワード。プレフィックスコード（他のコードの接頭辞にならない）のため一意にデコード可能。JPEG・MP3・DEFLATE（zlib/gzip）などで使用。',
  },
  {
    id: 'ALG-124', category: 'algorithm', categoryName: 'アルゴリズム',
    difficulty: 2, year: 2022, tags: ['再帰','分割統治法'],
    question: '分割統治法（Divide and Conquer）の説明として正しいものはどれか。',
    choices: [
      { index: 0, text: '問題を小さな部分問題に分割し、各部分問題の解を記録して再利用する手法（動的計画法）' },
      { index: 1, text: '問題を独立した複数の小問題に分割して再帰的に解き、各解を結合して全体の解を得る手法' },
      { index: 2, text: '現在最善の選択を繰り返すことで最適解に近い解を求める手法（貪欲法）' },
      { index: 3, text: '全候補解を枝刈りしながら探索する手法（バックトラッキング）' },
    ],
    answer: 1,
    explanation: '分割統治法（Divide & Conquer）の 3 ステップ：①分割（Divide）：問題を同種の小問題に分割②解決（Conquer）：各部分問題を再帰的に解く（十分小さければ直接解く）③結合（Combine）：部分解を結合して全体の解を得る。代表例：マージソート（分割→ソート→マージ）・クイックソート・二分探索・行列の高速乗算（Strassen 法）・FFT（高速フーリエ変換）。',
  },

  // ─── ネットワーク NET-115〜124 ────────────────────────
  {
    id: 'NET-115', category: 'network', categoryName: 'ネットワーク',
    difficulty: 2, year: 2022, tags: ['SDN','ネットワーク仮想化'],
    question: 'SDN（Software-Defined Networking）の主な特徴として正しいものはどれか。',
    choices: [
      { index: 0, text: 'ネットワーク機器のハードウェアを完全に廃止してすべてソフトウェアで実現する' },
      { index: 1, text: 'コントロールプレーン（経路制御）とデータプレーン（パケット転送）を分離し、集中的にソフトウェアで制御する' },
      { index: 2, text: '物理ケーブルを廃止し無線通信のみで企業ネットワークを構築する手法' },
      { index: 3, text: '複数の ISP 回線を束ねて帯域を増加させる技術' },
    ],
    answer: 1,
    explanation: 'SDN（ソフトウェア定義ネットワーク）：コントロールプレーン（ルーティング判断）を物理機器から分離してSDNコントローラに集約し、データプレーン（実際の転送）は専用ハードウェア（OpenFlow スイッチ）が行う。中央集権的な制御でネットワークポリシーの動的変更・自動化・可視化が容易になる。NFV（ネットワーク機能仮想化）は VPN・FW・LB 等の機能をVM上に仮想化する関連技術。',
  },
  {
    id: 'NET-116', category: 'network', categoryName: 'ネットワーク',
    difficulty: 3, year: 2023, tags: ['マルチキャスト','IGMPv3'],
    question: 'IP マルチキャストの説明として正しいものはどれか。',
    choices: [
      { index: 0, text: '送信元から宛先 IP アドレスを持つ単一のホストにパケットを届ける通信方式' },
      { index: 1, text: '1 つの送信元から複数の受信者（グループ）にパケットを同時に効率的に配信する通信方式' },
      { index: 2, text: 'ネットワーク内の全ホストにパケットをフラッディングする通信方式' },
      { index: 3, text: '複数の送信元から最も近い受信者にのみ配信するエニーキャスト方式' },
    ],
    answer: 1,
    explanation: 'IP マルチキャスト：1 対多の効率的な配信方式。224.0.0.0〜239.255.255.255 の Class D アドレスを使用。受信者がグループに参加（IGMP：Internet Group Management Protocol）することで受信できる。ルータは PIM（Protocol Independent Multicast）でマルチキャストツリーを形成してパケットを転送。動画ストリーミング・株価配信・ソフトウェア更新の一斉配信に使用。',
  },
  {
    id: 'NET-117', category: 'network', categoryName: 'ネットワーク',
    difficulty: 2, year: 2021, tags: ['プロキシ','リバースプロキシ'],
    question: 'リバースプロキシの説明として正しいものはどれか。',
    choices: [
      { index: 0, text: 'クライアント側に配置され、クライアントの代わりに外部サーバにアクセスするプロキシ' },
      { index: 1, text: 'サーバ側に配置され、クライアントからのリクエストを受け取り内部サーバに振り分けるプロキシ' },
      { index: 2, text: 'DNS の代わりにドメイン名解決を行う専用サーバ' },
      { index: 3, text: 'ネットワーク間の経路情報を交換するルーティングプロトコルの一種' },
    ],
    answer: 1,
    explanation: 'リバースプロキシ：Web サーバの前段に配置されるプロキシ。クライアントはリバースプロキシの IP アドレスのみを知り、内部サーバの構成が隠蔽される。主な機能：SSL/TLS ターミネーション・ロードバランシング・キャッシュ（静的コンテンツ）・圧縮・WAF・レート制限。Nginx・HAProxy・AWS ALB が代表例。フォワードプロキシはクライアント側でクライアントのリクエストを代理する。',
  },
  {
    id: 'NET-118', category: 'network', categoryName: 'ネットワーク',
    difficulty: 3, year: 2022, tags: ['QoS','帯域制御'],
    question: 'QoS（Quality of Service）の「DiffServ（差別化サービス）」モデルの説明として正しいものはどれか。',
    choices: [
      { index: 0, text: 'フロー単位でネットワーク機器がリソースを事前予約するモデル（IntServ）' },
      { index: 1, text: 'IP ヘッダの DSCP フィールドにトラフィッククラスをマークし、クラスに応じてルータが転送優先度を決定するモデル' },
      { index: 2, text: 'ネットワーク全体のデータを均等に分割して全ルートで並列転送するモデル' },
      { index: 3, text: 'TCP の ACK 頻度を調整して帯域利用率を最大化するトランスポート層の仕組み' },
    ],
    answer: 1,
    explanation: 'QoS モデルの比較：IntServ（統合サービス）：フロー単位でネットワークリソースを RSVP プロトコルで予約→スケールしない。DiffServ（差別化サービス）：IP ヘッダの DSCP（6 ビット）でクラス分け（EF/AF/BE 等）→各ルータがクラスに応じてキューイング・スケジューリング・ドロップポリシーを適用→スケーラブル。実用的な企業ネットワーク・ISP では DiffServ が標準。音声（VoIP）は EF（Expedited Forwarding）クラスで最優先処理。',
  },
  {
    id: 'NET-119', category: 'network', categoryName: 'ネットワーク',
    difficulty: 2, year: 2023, tags: ['ゼロトラスト','ZTNA'],
    question: 'ゼロトラストネットワークアクセス（ZTNA）の基本原則として正しいものはどれか。',
    choices: [
      { index: 0, text: '社内ネットワーク内のトラフィックは全て信頼し、外部トラフィックのみを検査する' },
      { index: 1, text: '場所（社内/社外）を問わず全てのアクセスを検証し、最小権限でリソースへのアクセスを許可する' },
      { index: 2, text: 'VPN でトンネリングすれば認証なしに全社内リソースにアクセスできる' },
      { index: 3, text: 'ファイアウォールで境界を守ることで内部ネットワークを安全に保つ境界防御モデル' },
    ],
    answer: 1,
    explanation: 'ゼロトラスト（NIST SP 800-207）：「信頼するな、常に検証せよ（Never Trust, Always Verify）」の原則。すべてのアクセスはデバイス・ユーザ・文脈を検証し最小権限で許可。VPN による全社内アクセス開放を廃止。実装要素：MFA・継続的認証・デバイス健全性検証・マイクロセグメンテーション・CASB・SWG。テレワーク普及でVPNの代替として急速に採用が拡大。',
  },
  {
    id: 'NET-120', category: 'network', categoryName: 'ネットワーク',
    difficulty: 3, year: 2021, tags: ['スイッチ','MACアドレス'],
    question: 'L2 スイッチの MAC アドレステーブル（MACアドレス学習）の動作として正しいものはどれか。',
    choices: [
      { index: 0, text: '宛先 MAC アドレスを見てルーティングテーブルを参照し、適切なポートに転送する' },
      { index: 1, text: '受信フレームの送信元 MAC アドレスとポートを学習し、宛先 MAC アドレスが未知の場合はフラッディングする' },
      { index: 2, text: 'IP アドレスを基準に VLAN の割り当てを決定し、セグメントを分離する' },
      { index: 3, text: '全ポートに受信したフレームを常にコピーして送出する（ハブと同じ動作）' },
    ],
    answer: 1,
    explanation: 'L2 スイッチの動作：①フレーム受信時に送信元 MAC アドレス＋受信ポートを MAC アドレステーブル（CAM テーブル）に記録（学習）②宛先 MAC アドレスがテーブルにあれば該当ポートのみに転送（ユニキャスト）③宛先が不明（エントリなし）またはブロードキャスト/マルチキャストの場合は全ポートにフラッディング。テーブルエントリは一定時間（通常 300 秒）アクセスがなければ削除（エージング）。',
  },
  {
    id: 'NET-121', category: 'network', categoryName: 'ネットワーク',
    difficulty: 2, year: 2022, tags: ['NAT','NAPT'],
    question: 'NAPT（Network Address Port Translation、IPマスカレード）の説明として正しいものはどれか。',
    choices: [
      { index: 0, text: '1つのグローバル IP アドレスを 1 台のホストにのみ 1:1 でマッピングする変換' },
      { index: 1, text: '複数のプライベート IP アドレスを 1 つのグローバル IP アドレス＋ポート番号の組み合わせに変換し、複数端末が同時にインターネット接続できる' },
      { index: 2, text: 'IPv4 アドレスを IPv6 アドレスに変換して移行を支援する技術' },
      { index: 3, text: 'ルーティングテーブルで宛先 IP に基づいて次ホップを決定する処理' },
    ],
    answer: 1,
    explanation: 'NAPT（IPマスカレード）：複数のプライベートIPアドレス（192.168.x.x）を 1 つのグローバル IP アドレスと異なるポート番号の組み合わせに変換する技術。家庭用ルータの標準動作。グローバル IP の節約・内部ホストのアドレス隠蔽によるセキュリティ向上の効果がある。NAT テーブル（変換テーブル）でセッションごとに送信元 IP・ポート↔グローバル IP・変換ポートを管理。',
  },
  {
    id: 'NET-122', category: 'network', categoryName: 'ネットワーク',
    difficulty: 3, year: 2023, tags: ['WAN','SD-WAN'],
    question: 'SD-WAN（Software-Defined WAN）の特徴として正しいものはどれか。',
    choices: [
      { index: 0, text: '専用線（MPLS）のみを使って拠点間を接続する従来型 WAN の最適化手法' },
      { index: 1, text: 'インターネット・LTE・MPLS など複数の回線をソフトウェアで統合制御し、トラフィックを動的に最適配分する' },
      { index: 2, text: 'Wi-Fi のみを使って広域エリアをカバーする無線 WAN 技術' },
      { index: 3, text: '物理ルータを廃止してすべての処理をクラウドで実行するバーチャル WAN' },
    ],
    answer: 1,
    explanation: 'SD-WAN：複数の WAN 回線（インターネット VPN・MPLS・4G/5G LTE）をソフトウェアで一元管理し、アプリケーションの優先度・帯域・レイテンシに応じてトラフィックを自動最適化する技術。MPLS 専用線に比べてコスト大幅削減・クラウドへのダイレクトアクセス向上。中央管理コントローラでポリシーを一括適用・可視化。ゼロトラストアーキテクチャとの統合（SASE）が進んでいる。',
  },
  {
    id: 'NET-123', category: 'network', categoryName: 'ネットワーク',
    difficulty: 2, year: 2021, tags: ['ICMP','ping'],
    question: 'ICMP（Internet Control Message Protocol）の主な用途として正しいものはどれか。',
    choices: [
      { index: 0, text: 'ファイルの転送とリモートディレクトリ操作を行うプロトコル' },
      { index: 1, text: 'ネットワーク通信のエラー報告・疎通確認（ping）・ルート追跡（traceroute）に使用されるネットワーク層プロトコル' },
      { index: 2, text: 'ドメイン名を IP アドレスに変換する名前解決プロトコル' },
      { index: 3, text: 'メールの送受信を担当するアプリケーション層プロトコル' },
    ],
    answer: 1,
    explanation: 'ICMP（RFC 792）：ネットワーク機器間のエラー報告・制御メッセージを運ぶ IP の補助プロトコル（IP ヘッダの Protocol 番号 1）。主なメッセージタイプ：Echo Request/Reply（ping で疎通確認）・Destination Unreachable（宛先到達不可）・Time Exceeded（TTL 超過→traceroute が活用）・Redirect（経路変更通知）。ICMP をブロックするとネットワーク障害調査が困難になる。IPv6 では ICMPv6 が必須プロトコル（NDP・MLD に使用）。',
  },
  {
    id: 'NET-124', category: 'network', categoryName: 'ネットワーク',
    difficulty: 3, year: 2022, tags: ['DDoS','攻撃対策'],
    question: 'DDoS（分散型サービス妨害）攻撃の対策として正しいものはどれか。',
    choices: [
      { index: 0, text: 'HTTPS 化（TLS 導入）のみで全ての DDoS 攻撃を防御できる' },
      { index: 1, text: 'CDN・クリーニングセンター（スクラビングセンター）・レートリミット・Anycast による分散吸収などの多層防御' },
      { index: 2, text: 'サーバの CPU をアップグレードすれば全ての DDoS トラフィックを処理できる' },
      { index: 3, text: 'ファイアウォールで全インバウンドポートを閉じれば DDoS を完全に防御できる' },
    ],
    answer: 1,
    explanation: 'DDoS 対策の多層防御：①CDN（コンテンツ配信ネットワーク）による分散吸収②スクラビングセンター（クリーニングセンター）：攻撃トラフィックをフィルタリングしてクリーンなトラフィックのみ転送③レートリミット・IP レピュテーション④Anycast ルーティング：攻撃を世界中のPoP（接続拠点）で分散吸収⑤BGP Blackhole Routing（緊急遮断）。完全な防御は困難なため、攻撃時のキャパシティ確保と迅速な検知・対応が重要。',
  },

  // ─── 論理 LG-101〜110 ────────────────────────────────
  {
    id: 'LG-101', category: 'logic', categoryName: '論理・数学',
    difficulty: 2, year: 2022, tags: ['2進数','加算'],
    question: '2 進数 1011 と 1101 を加算した 2 進数の結果として正しいものはどれか。',
    choices: [
      { index: 0, text: '10110' },
      { index: 1, text: '11000' },
      { index: 2, text: '11110' },
    ...
    answer: 1,
    explanation: '2 進数の加算：1011（=11）+ 1101（=13）= 11000（=24）。桁ごとに計算：1+1=10（桁上がり）→ 繰り上がりを次の桁へ。または 10 進数で確認：11 + 13 = 24 → 24 を 2 進数に変換：24 = 16 + 8 = 2⁴ + 2³ = 11000。',
  },
  {
    id: 'LG-102', category: 'logic', categoryName: '論理・数学',
    difficulty: 3, year: 2021, tags: ['論理式','カルノー図'],
    question: 'カルノー図（Karnaugh Map）の主な用途として正しいものはどれか。',
    choices: [
      { index: 0, text: '2 進数の乗算結果を視覚的に求めるグラフ' },
      { index: 1, text: '論理関数を視覚的に整理して積和標準形（SOP）や和積標準形（POS）を最簡化するツール' },
      { index: 2, text: '複数の論理ゲートの遅延時間を比較する性能評価ツール' },
      { index: 3, text: 'フリップフロップの動作を状態遷移で記述する図' },
    ],
    answer: 1,
    explanation: 'カルノー図：ブール代数式を視覚的に最小化するツール。隣接するセル（ハミング距離 1 の組み合わせ）をグループ化して共通因子を括り出すことで補語項を削減し、最簡積和標準形（最少ゲート数の論理回路）を導出する。2〜4 変数でよく使用。5 変数以上は Quine-McCluskey 法などのアルゴリズムを使用。',
  },
  {
    id: 'LG-103', category: 'logic', categoryName: '論理・数学',
    difficulty: 2, year: 2022, tags: ['確率','組み合わせ'],
    question: '52 枚のトランプからランダムに 1 枚引く試行でハートのエース（♥A）が出る確率として正しいものはどれか。',
    choices: [
      { index: 0, text: '1/4' },
      { index: 1, text: '1/13' },
      { index: 2, text: '1/52' },
    ...
    answer: 2,
    explanation: '52 枚のトランプに♥A は 1 枚のみ。確率 = 有利な場合の数 / 全場合の数 = 1/52。なお特定スート（例：ハート全て）の確率は 13/52 = 1/4。特定ランク（例：全てのエース）の確率は 4/52 = 1/13。',
  },
  {
    id: 'LG-104', category: 'logic', categoryName: '論理・数学',
    difficulty: 3, year: 2023, tags: ['情報量','エントロピー'],
    question: 'シャノンの情報エントロピー H = −Σ p(x) log₂ p(x) において、各記号の出現確率が等しいとき（p = 1/n）の性質として正しいものはどれか。',
    choices: [
      { index: 0, text: 'エントロピーが最小（0）になる' },
      { index: 1, text: 'エントロピーが最大（log₂ n ビット）になる' },
      { index: 2, text: 'エントロピーは出現確率に依存せず常に一定' },
      { index: 3, text: 'エントロピーは n に依存せず常に 1 ビット' },
    ],
    answer: 1,
    explanation: 'シャノンエントロピー：情報の不確実性（平均情報量）を表す。各記号の確率が等しい（p = 1/n）とき H = log₂(n) ビットで最大値をとる。例：コイン投げ（n=2、p=1/2）→ H = 1 ビット。サイコロ（n=6、p=1/6）→ H ≈ 2.58 ビット。確率が偏っているとエントロピーは低下（完全に決まっている場合 H = 0）。圧縮理論・暗号強度・機械学習（情報利得）に活用。',
  },
  {
    id: 'LG-105', category: 'logic', categoryName: '論理・数学',
    difficulty: 2, year: 2021, tags: ['再帰','漸化式'],
    question: '漸化式 T(n) = 2T(n/2) + n （T(1) = 1）をマスター定理で解いた時間計算量として正しいものはどれか。',
    choices: [
      { index: 0, text: 'O(n)' },
      { index: 1, text: 'O(n log n)' },
      { index: 2, text: 'O(n²)' },
    ...
    answer: 1,
    explanation: 'マスター定理：T(n) = aT(n/b) + f(n) の形の漸化式を解く定理。この問題では a=2、b=2、f(n)=n。n^(log₂ 2) = n^1 = n。f(n) = n = Θ(n^log_b a) なので Case 2（等値）→ T(n) = Θ(n log n)。マージソートの時間計算量 T(n) = 2T(n/2) + n と同じ形。これが O(n log n) になることを定理で証明する。',
  },
  {
    id: 'LG-106', category: 'logic', categoryName: '論理・数学',
    difficulty: 3, year: 2022, tags: ['暗号','RSA'],
    question: 'RSA 暗号の安全性の根拠として正しいものはどれか。',
    choices: [
      { index: 0, text: '量子コンピュータでも解読不可能な数学的問題に基づく' },
      { index: 1, text: '大きな素数 2 つの積（合成数）の素因数分解が計算量的に困難なこと（素因数分解問題）' },
      { index: 2, text: '共通鍵の配送経路を物理的に保護することで安全性を確保する' },
      { index: 3, text: '鍵の長さが長いほど総当たりによる解読が困難になるという原則のみに基づく' },
    ],
    answer: 1,
    explanation: 'RSA 暗号の安全性：2 つの大きな素数 p・q の積 n = p×q を公開鍵に含めるが、n から p と q を求める（素因数分解）ことが現在のコンピュータでは計算量的に困難（多項式時間アルゴリズムが未発見）なことに基づく。鍵長 2048 ビット以上が推奨。量子コンピュータの Shor のアルゴリズムは多項式時間で素因数分解できるため、量子耐性暗号（格子ベース暗号等）への移行が検討されている。',
  },
  {
    id: 'LG-107', category: 'logic', categoryName: '論理・数学',
    difficulty: 2, year: 2023, tags: ['組み合わせ','パスカルの三角形'],
    question: '10 人の中から 3 人を選ぶ組み合わせの数として正しいものはどれか。',
    choices: [
      { index: 0, text: '30' },
      { index: 1, text: '120' },
      { index: 2, text: '720' },
    ...
    answer: 1,
    explanation: 'C(10,3) = 10! / (3! × 7!) = (10 × 9 × 8) / (3 × 2 × 1) = 720 / 6 = 120。組み合わせ C(n,r)：順序を考慮しない選択数。順列 P(n,r) = 10 × 9 × 8 = 720（順序あり）。C(10,3) = P(10,3) / 3! = 720 / 6 = 120。情報系での応用：n 頂点の完全グラフの辺数 = C(n,2)、n 人のコミュニケーションチャネル数 = C(n,2) = n(n-1)/2。',
  },
  {
    id: 'LG-108', category: 'logic', categoryName: '論理・数学',
    difficulty: 3, year: 2021, tags: ['線形代数','行列'],
    question: '機械学習でよく使われる行列演算に関して、行列の積 AB が定義できる条件として正しいものはどれか。',
    choices: [
      { index: 0, text: 'A と B が正方行列（同じサイズ）であること' },
      { index: 1, text: 'A の列数と B の行数が等しいこと' },
      { index: 2, text: 'A と B の全要素が正の数であること' },
      { index: 3, text: 'A と B のランクが等しいこと' },
    ],
    answer: 1,
    explanation: '行列の積 AB：A が m×k 行列、B が k×n 行列のとき積 AB が定義され、結果は m×n 行列となる。A の列数（k）と B の行数（k）が一致することが必要条件。AB ≠ BA（行列の積は一般に非可換）。機械学習では入力ベクトル・重み行列・バイアス行列の積が順伝播計算の基本。転置・逆行列・固有値分解が線形代数の重要演算。',
  },
  {
    id: 'LG-109', category: 'logic', categoryName: '論理・数学',
    difficulty: 2, year: 2022, tags: ['16進数','変換'],
    question: '10 進数 255 を 16 進数に変換した結果として正しいものはどれか。',
    choices: [
      { index: 0, text: 'EF' },
      { index: 1, text: 'FF' },
      { index: 2, text: 'FE' },
    ...
    answer: 1,
    explanation: '255 を 16 進数に変換：255 ÷ 16 = 15 余り 15。商=15（16進数で F）、余り=15（16進数で F）→ FF。確認：F×16 + F = 15×16 + 15 = 240 + 15 = 255。なお 256 = 100₁₆、254 = FE₁₆、127 = 7F₁₆。255 = 0xFF は IPv4 のサブネットマスク（255.255.255.0）やビットマスク（全ビット 1）として頻出。',
  },
  {
    id: 'LG-110', category: 'logic', categoryName: '論理・数学',
    difficulty: 3, year: 2023, tags: ['オートマトン','チューリングマシン'],
    question: '有限オートマトン（FA）とチューリングマシン（TM）の表現能力の違いとして正しいものはどれか。',
    choices: [
      { index: 0, text: '有限オートマトンの方がチューリングマシンより計算能力が高い' },
      { index: 1, text: 'チューリングマシンが最も計算能力が高く、有限オートマトン→プッシュダウンオートマトン→チューリングマシンの順に計算能力が上がる' },
      { index: 2, text: '両者は計算能力が等しく、どちらも同じ言語クラスを受理できる' },
      { index: 3, text: '有限オートマトンはスタックを使うためプッシュダウンオートマトンより表現力が高い' },
    ],
    answer: 1,
    explanation: 'チョムスキー階層：正規言語（FA が認識）⊂ 文脈自由言語（PDA が認識）⊂ 文脈依存言語（線形拘束オートマトン）⊂ 帰納的可算言語（TM が認識）。有限オートマトン（FA）：記憶なし・正規表現で記述可能。プッシュダウンオートマトン（PDA）：スタック 1 つ・文脈自由文法（プログラミング言語の構文解析）。チューリングマシン：無限テープ・計算可能なあらゆる問題を解ける（Church-Turing テーゼ）。',
  },

  // ─── OS OS-111〜120 ──────────────────────────────────
  {
    id: 'OS-111', category: 'os', categoryName: 'OS・ハードウェア',
    difficulty: 2, year: 2022, tags: ['シェル','スクリプト'],
    question: 'Linux のパイプ（|）の説明として正しいものはどれか。',
    choices: [
      { index: 0, text: 'コマンドを並列に実行してどちらかが終了するまで待機する演算子' },
      { index: 1, text: '左のコマンドの標準出力を右のコマンドの標準入力に渡して連結する仕組み' },
      { index: 2, text: 'コマンドの出力をファイルに追記する演算子（>>と同じ）' },
      { index: 3, text: '2 つのコマンドを順番に実行し、最初のコマンドが失敗しても次を実行する演算子（;と同じ）' },
    ],
    answer: 1,
    explanation: 'パイプ（|）：左コマンドの stdout を右コマンドの stdin に接続してコマンドを連結する UNIX の強力な機能。例：ls -la | grep ".txt" | sort（ファイル一覧→.txtを抽出→ソート）。パイプは IPC（プロセス間通信）の一種。なお >> はリダイレクト追記、&& は前のコマンドが成功した場合のみ次を実行（論理 AND）、; は前のコマンドの成否にかかわらず順番に実行。',
  },
  {
    id: 'OS-112', category: 'os', categoryName: 'OS・ハードウェア',
    difficulty: 3, year: 2023, tags: ['セキュリティ','Linux権限'],
    question: 'Linux の setuid ビット（SUID）の説明として正しいものはどれか。',
    choices: [
      { index: 0, text: 'ファイルを誰でも読み取れるようにするパーミッションビット' },
      { index: 1, text: '実行ファイルに設定すると、実行したユーザではなくファイル所有者の権限で実行される特殊パーミッション' },
      { index: 2, text: 'ファイルの削除を所有者のみに制限するスティッキービット' },
      { index: 3, text: 'グループの実行権限を全ユーザに付与するパーミッションビット' },
    ],
    answer: 1,
    explanation: 'SUID（Set User ID on Execution）：実行ファイルに設定すると、実行時に呼び出したユーザの権限ではなくファイル所有者の権限でプロセスが動作する。代表例：/usr/bin/passwd（root 所有・SUID 設定）→一般ユーザが自分のパスワード変更時に /etc/shadow（root のみ書き込み可）を更新できる。セキュリティリスクが高いため乱用厳禁・定期的に find を使って SUID ファイルを監査する。',
  },
  {
    id: 'OS-113', category: 'os', categoryName: 'OS・ハードウェア',
    difficulty: 2, year: 2021, tags: ['仮想化','ハイパーバイザ'],
    question: 'Type 1 ハイパーバイザ（ネイティブ/ベアメタル型）の特徴として正しいものはどれか。',
    choices: [
      { index: 0, text: 'ホスト OS（Windows/Linux）の上で動作するため導入が容易だが性能オーバヘッドが大きい' },
      { index: 1, text: 'ハードウェア上に直接インストールされ、ホスト OS を必要とせずに VM を管理する。性能オーバヘッドが小さく本番環境向き' },
      { index: 2, text: 'コンテナ技術と同等で OS カーネルを複数の VM で共有する' },
      { index: 3, text: 'クラウドプロバイダのサービスとしてのみ提供され、オンプレミスでは使用不可' },
    ],
    answer: 1,
    explanation: 'ハイパーバイザの分類：Type 1（ベアメタル型）：VMware ESXi・Microsoft Hyper-V・KVM（Linux カーネル内蔵）。ハードウェアに直接搭載し高性能・高信頼性→データセンタ・エンタープライズ向け。Type 2（ホスト型）：VMware Workstation・VirtualBox・Parallels Desktop。ホスト OS の上で動作→性能はやや劣るが個人・開発環境で手軽に使える。',
  },
  {
    id: 'OS-114', category: 'os', categoryName: 'OS・ハードウェア',
    difficulty: 3, year: 2022, tags: ['ファイルシステム','NTFS'],
    question: 'ジャーナリングファイルシステムの説明として正しいものはどれか。',
    choices: [
      { index: 0, text: '全データのバックアップをリアルタイムに別のデバイスに作成する仕組み' },
      { index: 1, text: 'ファイルシステムへの変更をジャーナル（ログ）に先行記録し、クラッシュ時にジャーナルを使って整合性を回復する仕組み' },
      { index: 2, text: 'ファイルをジャーナル（日誌形式）で管理し、アクセス履歴を記録するロギング機能' },
      { index: 3, text: 'SSD の書き換え回数を分散させてフラッシュメモリの寿命を延ばすウェアレベリング技術' },
    ],
    answer: 1,
    explanation: 'ジャーナリングファイルシステム：ファイルシステムのメタデータ（またはデータも）変更をジャーナル（WAL：Write-Ahead Log）に先行記録する技術。電源断・クラッシュ時にジャーナルをリプレイして整合性を迅速に回復できる（fsck の大幅短縮）。ext3/ext4・NTFS・HFS+・ZFS が採用。ジャーナリングなしの ext2 はクラッシュ後に長時間の fsck が必要。',
  },
  {
    id: 'OS-115', category: 'os', categoryName: 'OS・ハードウェア',
    difficulty: 2, year: 2023, tags: ['CPU','パイプライン'],
    question: 'CPU のパイプライン処理（Pipeline）の説明として正しいものはどれか。',
    choices: [
      { index: 0, text: '命令を 1 つずつ順番に完全実行してから次の命令を実行する逐次処理方式' },
      { index: 1, text: '命令を複数のステージ（フェッチ・デコード・実行・ライトバック等）に分割し、複数の命令を並列に処理してスループットを向上する方式' },
      { index: 2, text: '複数の CPU コアが同一の命令を並列実行する SIMD 処理方式' },
      { index: 3, text: 'CPU 内のキャッシュメモリを階層化して命令アクセスを高速化する仕組み' },
    ],
    answer: 1,
    explanation: 'パイプライン：命令実行を IF（命令フェッチ）・ID（デコード）・EX（実行）・MEM（メモリアクセス）・WB（ライトバック）の 5 段階に分け、各ステージを異なる命令が同時に使用する。理想的には CPI（1 命令あたりのサイクル数）≒ 1 に近づく。問題点：データハザード（依存関係）・制御ハザード（分岐予測外れ）・構造ハザード（リソース競合）でパイプラインストールが発生。分岐予測・アウトオブオーダー実行で対策。',
  },
  {
    id: 'OS-116', category: 'os', categoryName: 'OS・ハードウェア',
    difficulty: 3, year: 2021, tags: ['ネットワークOS','TCPソケット'],
    question: 'UNIX ドメインソケットと TCP ソケットの違いとして正しいものはどれか。',
    choices: [
      { index: 0, text: 'TCP ソケットは同一マシン内でのみ使用でき、UNIX ドメインソケットはネットワーク越しの通信に使用する' },
      { index: 1, text: 'UNIX ドメインソケットは同一マシン内のプロセス間通信に使用（TCP/IPスタック不使用で高速）。TCP ソケットはネットワーク経由の通信に使用' },
      { index: 2, text: '両者は同一の仕組みであり、ファイルパスを使うか IP アドレスを使うかの違いだけ' },
      { index: 3, text: 'UNIX ドメインソケットは UDP ベース、TCP ソケットは TCP ベースの通信方式' },
    ],
    answer: 1,
    explanation: 'UNIX ドメインソケット（UDS）：ファイルシステムのパス（/tmp/app.sock 等）を識別子としてカーネル内でのみ通信するプロセス間通信（IPC）メカニズム。TCP/IP スタックをバイパスするため localhost より高速・低レイテンシ。使用例：Nginx ⇔ PHP-FPM 連携・Redis の一部構成・Docker Engine ソケット（/var/run/docker.sock）。TCP ソケットはネットワークインタフェースを使い異なるホスト間の通信も可能。',
  },
  {
    id: 'OS-117', category: 'os', categoryName: 'OS・ハードウェア',
    difficulty: 2, year: 2022, tags: ['電力管理','省電力'],
    question: 'CPU の動的電圧・周波数スケーリング（DVFS）の説明として正しいものはどれか。',
    choices: [
      { index: 0, text: 'CPU のコア数を動的に増減させて消費電力を制御する技術' },
      { index: 1, text: 'CPU の動作周波数と電源電圧を負荷に応じてリアルタイムに変化させて性能・省電力を動的バランスする技術' },
      { index: 2, text: '複数 CPU コアの処理を統合して使用コアを削減する仮想化技術' },
      { index: 3, text: 'メモリの電圧を下げてストレージデバイスの消費電力を削減する技術' },
    ],
    answer: 1,
    explanation: 'DVFS（Dynamic Voltage and Frequency Scaling）：CPU 負荷が低いとき周波数と電圧を下げ（P-state 制御）、高負荷時に最大周波数・電圧に上げる省電力技術。消費電力は電圧の 2 乗×周波数に比例するため、電圧低下による省電力効果が大きい。Intel の SpeedStep・AMD の Cool\'n\'Quiet・ARM の big.LITTLE が実装例。モバイル端末のバッテリー持続・データセンタのPUE改善に重要。',
  },
  {
    id: 'OS-118', category: 'os', categoryName: 'OS・ハードウェア',
    difficulty: 3, year: 2023, tags: ['セキュリティ','TEE'],
    question: 'ARM TrustZone や Intel SGX などのハードウェアセキュリティ技術の説明として正しいものはどれか。',
    choices: [
      { index: 0, text: 'ハードウェアのセキュリティチップ（TPM）を使った全ディスク暗号化技術' },
      { index: 1, text: 'CPU ハードウェアが通常の OS から分離した信頼された実行環境（TEE）を提供し、機密コードとデータをOS・VMM からも保護する技術' },
      { index: 2, text: 'ネットワーク通信を暗号化して盗聴を防ぐ TLS/SSL の後継技術' },
      { index: 3, text: 'ソフトウェアで実装された仮想 HSM（ハードウェアセキュリティモジュール）' },
    ],
    answer: 1,
    explanation: 'TEE（Trusted Execution Environment）：ハードウェアが提供する分離された安全実行環境。ARM TrustZone：プロセッサを Normal World（通常 OS）と Secure World（信頼ソフトウェア）に分割。Intel SGX（Software Guard Extensions）：アプリケーションが「エンクレーブ」という隔離領域を作成し、OS・VMM もアクセス不可の秘密データを処理できる。DRM・モバイル決済・鍵管理・プライバシー保護計算に活用。',
  },
  {
    id: 'OS-119', category: 'os', categoryName: 'OS・ハードウェア',
    difficulty: 2, year: 2021, tags: ['ベンチマーク','性能測定'],
    question: 'コンピュータ性能の「スループット」の説明として正しいものはどれか。',
    choices: [
      { index: 0, text: 'リクエストを送信してからレスポンスが返るまでの時間（応答時間）' },
      { index: 1, text: '単位時間あたりに処理できる仕事量（トランザクション数・命令数など）' },
      { index: 2, text: 'システムが停止せずに動作し続けられる時間の割合（稼働率）' },
      { index: 3, text: 'システムの最大同時接続数' },
    ],
    answer: 1,
    explanation: 'スループット（Throughput）：単位時間当たりの処理量（TPS：Transactions Per Second、IOPS：Input/Output Operations Per Second、Mbps：ネットワーク帯域など）。レスポンスタイムとは異なる概念で、負荷が高くなるとレスポンスタイムが悪化してもスループットは上限に近づく。リトルの法則（L = λW：L=滞在数、λ=スループット、W=レスポンスタイム）でシステムの性能モデリングができる。',
  },
  {
    id: 'OS-120', category: 'os', categoryName: 'OS・ハードウェア',
    difficulty: 3, year: 2022, tags: ['BIOS','ブートプロセス'],
    question: 'UEFI セキュアブート（Secure Boot）の説明として正しいものはどれか。',
    choices: [
      { index: 0, text: '起動時にディスク全体を暗号化して不正アクセスを防ぐ機能' },
      { index: 1, text: 'ブートローダやOSカーネルのデジタル署名を検証し、信頼されたソフトウェアのみを起動させる機能' },
      { index: 2, text: '起動パスワードを設定して物理アクセスを制限する BIOS の機能' },
      { index: 3, text: '仮想マシンが実行されているかを検出してブートを中断する機能' },
    ],
    answer: 1,
    explanation: 'UEFI セキュアブート：ブート時にブートローダ・OS カーネル・UEFI ドライバのデジタル署名を UEFI DB（信頼済み証明書データベース）と照合し、署名されていないコードの実行をブロックする機能。ルートキット・ブートキットのようなブートプロセスを狙うマルウェアへの対策。Windows・Linux（Shim 経由）・macOS がサポート。Linux のカスタムカーネルを使う場合は設定が必要。',
  },
];
QUESTIONS_DATA.push(...QUESTIONS_DATA_PHASE18);
